...
static jint JII_AlreadyOneWithVM(JavaVM* vm, JNIEnv** p_env) {
  if (vm == NULL || p_env == NULL) {
    return JNI_ERR;
  }
  Thread* self = Thread::Current();
...
class JII {
 public:
  ...
  static jint AlreadyOneWithVM(JavaVM* vm, JNIEnv** p_env) {
  	return JII_AlreadyOneWithVM(vm, p_env);
  }
