...
ifeq (true,$(strip $(enable_target_debugging)))
  ...
  ADDITIONAL_DEFAULT_PROPERTIES += ro.debuggable=1
  ADDITIONAL_DEFAULT_PROPERTIES += service.adb.root=0
  ADDITIONAL_DEFAULT_PROPERTIES += debug.service.adb.root=0
  else
...
MTK_DEVICE := $(shell find device/ -name "*_$(IC_SETTING)*"| awk -F/ '{print $$1"/"$$2}')
MTK_DEVICE := $(addprefix ./,$(addsuffix %.mk,$(MTK_DEVICE)))

subdir_makefiles := \
	$(shell build/tools/findleaves.py --prune=$(OUT_DIR) --prune=.repo --prune=.git $(subdirs) Android.mk)
subdir_makefiles := $(filter-out $(MTK_DEVICE), $(subdir_makefiles))
subdir_makefiles := $(filter-out ./device/%/basic/Android.mk, $(subdir_makefiles))

ifeq "$(TARGET_IC)" ""
-include $(THIS_ROOT)/output/export_configuration.mak
endif

subdir_makefiles += \
	$(DEVICE_STRING)/Android.mk
...
.PHONY: impdataimage
impdataimage: $(INSTALLED_IMPDATAIMAGE_TARGET)

.PHONY: tclconfigimage
tclconfigimage: $(INSTALLED_TCLCONFIGIMAGE_TARGET)

.PHONY: tvosimage
tvosimage: $(INSTALLED_TVOSIMAGE_TARGET)

.PHONY: tcldataimage
tcldataimage: $(INSTALLED_TCLDATAIMAGE_TARGET)
...
droidcore: files \
	...
	$(INSTALLED_IMPDATAIMAGE_TARGET) \
	$(INSTALLED_TCLCONFIGIMAGE_TARGET) \
	$(INSTALLED_TVOSIMAGE_TARGET) \
	$(INSTALLED_TCLDATAIMAGE_TARGET) \
