...
public class ConnectivityService extends IConnectivityManager.Stub {
    ...
    //to save state for networks' connectting to internet
    private boolean mConnectedInternetState[];
    
    /** Lock for setting dns and route. */
    private Object mInternetLock = new Object();
    ...
    public ConnectivityService(Context context, INetworkManagementService netManager,
        ...
        mConnectedInternetState = new boolean[ConnectivityManager.MAX_NETWORK_TYPE+1];
        ...
        if (mNetworkPreference == -1) {
            for (int n : mPriorityList) {
                if (mNetConfigs[n].isDefault() && ConnectivityManager.isNetworkTypeValid(n)) {
                    mNetworkPreference = 1;
                    log("mNetworkPreference = n:"+mNetworkPreference);
                    break;
                }
            }
            if (mNetworkPreference == -1) {
        ...
        public NetworkStateTracker createTracker(int targetNetworkType, NetworkConfig config) {
            switch (config.radio) {
                ...
                case TYPE_ETHERNET:
                    ...
                    /*EthernetStateTracker tracker = new EthernetStateTracker(mContext, mTrackerHandler);
                    EthernetService ethService = new EthernetService(mContext, tracker);
                    ServiceManager.addService(Context.ETHERNET_SERVICE, ethService);
					*/
					/*PppoeService pppoeService = new PppoeService();
					ServiceManager.addService(PppoeService.PPPOE_SERVICE, pppoeService);
					pppoeService.startMonitoring(mContext, mTrackerHandler);
					*/
                    //return tracker;
                case TYPE_PPPOE:
    ...
    private void enforcePreference() {
        ...
        if (!mNetTrackers[mNetworkPreference].getNetworkInfo().isAvailable())
            return;
    ...
    private void enforceConnectivityInternalPermission() {
		...
		private void NotifyConnectedToInternet(NetworkInfo info, boolean isConnected) {
        ...
        intent.putExtra(ConnectivityManager.EXTRA_INET_CONDITION, isConnected ? 100:0 );
			
		log("zyy:dns: NotifyConnectedToInternet [" + info.getTypeName() + "] is  " + (isConnected ? "connected":"not connected"));	
		...
		//zengyongying: to handle whether connected internet msg
		Handler internetStateHandler = new Handler(){
			public void handleMessage(Message msg){
				int netType = msg.arg1;
				String value;
				
				synchronized(mInternetLock) {
					//just do nothing, if already get right state
					if(mConnectedInternetState[netType] == true) {
						log("zyy:dns: internetStateHandler, type[" + netType + "] is already get right state");
						return;
					}
					
					if(mActiveDefaultNetwork == ConnectivityManager.TYPE_PPPOE) {
						log("zyy:dns: internetStateHandler, type[" + netType + "] return by default is TYPE_PPPOE");
						return;
					}
					
					//try to set dns and route
					if(netType == ConnectivityManager.TYPE_ETHERNET){//ethernet connected to internet
						if(mActiveDefaultNetwork != ConnectivityManager.TYPE_ETHERNET){
							log("zyy:dns: internetStateHandler, set ethernet dns and route");
							
							//remove ethernet default route, then add ethernet default route
							removeDefaultRouteForInterface(ConnectivityManager.TYPE_ETHERNET);
							removeDefaultRouteForInterface(ConnectivityManager.TYPE_WIFI);
					    addDefaultRouteForInterface(ConnectivityManager.TYPE_ETHERNET);
					      	
							//set eth dns
							value = SystemProperties.get("dhcp.eth0.dns1");
							if(!value.equals(""))				SystemProperties.set("net.dns1", value);
							value = SystemProperties.get("dhcp.eth0.dns2");
							if(!value.equals(""))				SystemProperties.set("net.dns2", value);
							mActiveDefaultNetwork = ConnectivityManager.TYPE_ETHERNET;	
						}else{//already set to ethernet, do nothing
							//null
						}
					}else{//wifi connected to internet
						if(mConnectedInternetState[ConnectivityManager.TYPE_ETHERNET] != true){
							log("zyy:dns: internetStateHandler, set wifi dns and route");
							
							//remove ethernet default route, then add wifi default route
							removeDefaultRouteForInterface(ConnectivityManager.TYPE_ETHERNET);
							removeDefaultRouteForInterface(ConnectivityManager.TYPE_WIFI);
					    addDefaultRouteForInterface(ConnectivityManager.TYPE_WIFI);
					      	
							//set wifi dns
							value = SystemProperties.get("dhcp.wlan0.dns1");
							if(!value.equals(""))				SystemProperties.set("net.dns1", value);
							value = SystemProperties.get("dhcp.wlan0.dns2");
							if(!value.equals(""))				SystemProperties.set("net.dns2", value);
							mActiveDefaultNetwork = ConnectivityManager.TYPE_WIFI;	
						}else {//ethernet connected, do not change dns and route
							//null
						}
					}
					
					mConnectedInternetState[netType] = true;
					NotifyConnectedToInternet(mNetTrackers[netType].getNetworkInfo(), true);
				}	
			}
		};
    ...
    private void handleDisconnect(NetworkInfo info) {
        ...
        int prevNetType = info.getType();
				...
				//do nothing while pppoe is using
				if(prevNetType != ConnectivityManager.TYPE_PPPOE
					&& mActiveDefaultNetwork == ConnectivityManager.TYPE_PPPOE) {
					log("zyy:dns:[handledisconnect] remove dns&route for type "+prevNetType+" while pppoe is using!");	
		     
		     	//remove route
		     	removeDefaultRouteForInterface(prevNetType);
		      ...
		      /*//set pppoe dns				
		        ...
		        mConnectedInternetState[prevNetType] = false;
		        NotifyConnectedToInternet(mNetTrackers[prevNetType].getNetworkInfo(), false);
        ...
        mDefaultInetConditionPublished = 0;	//zengyongying
        ...
        if (mActiveDefaultNetwork != -1) {
			...
			synchronized(mInternetLock) {
        if(mNetTrackers[ConnectivityManager.TYPE_ETHERNET].getNetworkInfo().isConnected()){
        		log("zyy:dns: re-set eth route and dns!");	
        		
        		if(mActiveDefaultNetwork != ConnectivityManager.TYPE_ETHERNET) {
		       	addDefaultRouteForInterface(ConnectivityManager.TYPE_ETHERNET);	
						String value = SystemProperties.get("dhcp.eth0.dns1");
						if(!value.equals(""))				SystemProperties.set("net.dns1", value);
						value = SystemProperties.get("dhcp.eth0.dns2");
						if(!value.equals(""))				SystemProperties.set("net.dns2", value);
						handleDnsConfigurationChange(ConnectivityManager.TYPE_ETHERNET);	
						
						mActiveDefaultNetwork = ConnectivityManager.TYPE_ETHERNET;
						log("zyy:dns: re-set eth route and dns! ok");	
					}
        }
       	else if(mNetTrackers[ConnectivityManager.TYPE_WIFI].getNetworkInfo().isConnected()){
        		log("zyy:dns: re-set wifi route and dns!");	
        		
        		if(mActiveDefaultNetwork != ConnectivityManager.TYPE_WIFI) {
		       	addDefaultRouteForInterface(ConnectivityManager.TYPE_WIFI);	
						String value = SystemProperties.get("dhcp.wlan0.dns1");
						if(!value.equals(""))				SystemProperties.set("net.dns1", value);
						value = SystemProperties.get("dhcp.wlan0.dns2");
						if(!value.equals(""))				SystemProperties.set("net.dns2", value);
						...
						mActiveDefaultNetwork = ConnectivityManager.TYPE_WIFI;
						log("zyy:dns: re-set wifi route and dns! ok");	
					}
        }
        
        log("zyy:dns: mActiveDefaultNetwork is " + mActiveDefaultNetwork);	
        if(mActiveDefaultNetwork != -1){
		        //zengyongying: notify app wheather connect to internet
		        boolean isConnectInternet = isConnectedToInternet(mActiveDefaultNetwork);
		        mConnectedInternetState[mActiveDefaultNetwork] = isConnectInternet;
		        NotifyConnectedToInternet(mNetTrackers[mActiveDefaultNetwork].getNetworkInfo(), isConnectInternet);
      	}
      }
    }
...
//                if (currentPriority >= mNetConfigs[checkType].mPriority) continue;
    ...
    private boolean isNewNetTypePreferredOverCurrentNetType(int type) {
		...
		//1: connected internet ok				0: not connected to internet
		private boolean isConnectedToInternet0() {
				boolean bRet = false;
				log("zyy:dns: isConnectedToInternet: begin.....Socket.....");
				
				try{
				    Socket s=new Socket("www.sina.com.cn",80);
				    if(s.isConnected()){
				    	bRet = true;
				    	loge("zyy:dns: connect to sina ok");
				    }
				    else {				    	
				    	loge("zyy:dns: connect to sina fail");
				    }
				    s.close();
				}catch(Exception e){
						loge("zyy:dns: connect to sina fail");
				...
				//try 'baidu' to avoid wrong result
				try{
				    Socket s=new Socket("www.baidu.com",80);
				    if(s.isConnected()){
				    	bRet = true;
				    	loge("zyy:dns: connect to baidu ok");
				    }
				    else {				    	
				    	loge("zyy:dns: connect to baidu fail");
				    }
				    s.close();
				}catch(Exception e){
						loge("zyy:dns: connect to baidu fail");
		...
		private boolean isConnectedToInternetImp1(int netType) {			
				boolean bRet = false;
				//Date now = new Date();
				String iface="null";
				...
				if(ConnectivityManager.TYPE_ETHERNET == netType)
					...
					iface = "wifi";
				else if(ConnectivityManager.TYPE_PPPOE == netType)
					iface = "pppoe";	
					
				
				//System.currentTimeMillis()
				log("zyy:dns: isConnectedToInternet["+iface+"]: begin....sina");
				
				try{
				    InetAddress address =InetAddress.getByName("www.sina.com.cn");
				    //InetAddress address =InetAddress.getByName("218.30.66.101");
				    //now = new Date();
				    log("zyy:dns: isConnectedToInternet["+iface+"]: getByName sina");
				    
				    if(address.isReachable(2*1000)){		//2 seconds
				    	bRet = true;
				    	loge("zyy:dns["+iface+"]: connect to sina ok");
				    }
				    else {				    	
				    	loge("zyy:dns["+iface+"]: connect to sina fail");
				    }
			...
			/*
			 now = new Date();
			 log("zyy:dns: isConnectedToInternet: getByName ok now...." + now);
				...
				try{
				    Socket s=new Socket("www.baidu.com",80);
				    now = new Date();
				    log("zyy:dns: isConnectedToInternet: new Socket now...." + now);
				    if(s.isConnected()){
				    	bRet = true;
				    	loge("zyy:dns: connect to baidu ok");
				    }
				...
				now = new Date();
				log("zyy:dns: isConnectedToInternet: new Socket ok now...." + now);
				//*/
				return bRet;
		...
		private boolean isConnectedToInternetImp2(int netType) {			
				boolean bRet = false;
				//Date now = new Date();
				String iface="null";
				...
				//System.currentTimeMillis()
				log("zyy:dns: isConnectedToInternet["+iface+"]: begin...baidu");
				
				try{
				    Socket s=new Socket("www.baidu.com",80);
				    //now = new Date();
				    log("zyy:dns: isConnectedToInternet["+iface+"]: new Socket ....baidu");
				    if(s.isConnected()){
				    	bRet = true;
				    	loge("zyy:dns["+iface+"]: connect to baidu ok");
				    }
				    else {				    	
				    	loge("zyy:dns["+iface+"]: connect to baidu fail");
				    }
				    s.close();
				}catch(Exception e){
						loge("zyy:dns["+iface+"]: connect to baidu fail");
				...
				//log("zyy:dns: isConnectedToInternet["+iface+"]: new Socket baidu ok");
				//*/
				return bRet;		
		}

		private boolean isConnectedToInternetImp3(int netType) {			
				boolean bRet = false;
				//Date now = new Date();
				String iface="null";
				...
				//System.currentTimeMillis()
				log("zyy:dns: isConnectedToInternet["+iface+"]: begin...tcl" );
				
				try{
				    InetAddress address =InetAddress.getByName("www.tcl.com");
				    //InetAddress address =InetAddress.getByName("218.30.66.101");
				    //now = new Date();
				    log("zyy:dns: isConnectedToInternet["+iface+"]: getByName ...tcl ");
				    
				    if(address.isReachable(2*1000)){		//2 seconds
				    	bRet = true;
				    	loge("zyy:dns["+iface+"]: connect to tcl ok");
				    }
				    else {				    	
				    	loge("zyy:dns["+iface+"]: connect to tcl fail");
				    }
		...
		class IsConnectToInternetThread extends Thread {
				boolean result = false;
				private int netType;
				private int threadNo;
				
        public IsConnectToInternetThread(int nt, int tNo) {//netType & threadNo
            super("IsConnectToInternet");
            netType = nt;
            threadNo = tNo;
        }
        ...
        public void run() {
        		switch(threadNo) {
        			case 1:
								result = isConnectedToInternetImp1(netType);
								break;
        			case 2:
								result = isConnectedToInternetImp2(netType);
								break;
        			case 3:
								result = isConnectedToInternetImp3(netType);
								break;
							default:
								result = false;	
								break;
						...
						if(result == true) {
							...
							loge("zyy:dns["+netType+"]: send msg ok to handler");
							internetStateHandler.sendMessage(msg);
						}
        }
    }

		private boolean isConnectedToInternet(int netType) {
			boolean ret = false;
			
			if (VDBG) {	
		       	for(RouteInfo r : mAddedRoutes) {  
		        		log("zyy:isConnectedToInternet: mAddedRoutes: " + r );
		        }			
   			}
			
				IsConnectToInternetThread myThread1 = new IsConnectToInternetThread(netType,1);	
				myThread1.start();
				
				IsConnectToInternetThread myThread2 = new IsConnectToInternetThread(netType,2);	
				myThread2.start();
				
				IsConnectToInternetThread myThread3 = new IsConnectToInternetThread(netType,3);	
				myThread3.start();
												
				try {
						Thread.sleep(1000);	// 1000 microseconds
						
						if((myThread1.result == true) ||(myThread2.result == true) || (myThread3.result == true))
									return true;
									
						Thread.sleep(1*1000);	// 2 seconds
						ret = ((myThread1.result == true) ||(myThread2.result == true) || (myThread3.result == true));
		...
		private void removeDefaultRouteForInterface(int iface_type) {
				Collection<RouteInfo> cr = mNetTrackers[iface_type].getLinkProperties().getRoutes();
        String ifaceName = mNetTrackers[iface_type].getLinkProperties().getInterfaceName();
        
        LinkProperties curLp = mCurrentLinkProperties[iface_type];
        LinkProperties newLp = null;
        
        updateRoutes(newLp, curLp, mNetConfigs[iface_type].isDefault(),UNEXEMPT);
        
        for(RouteInfo r : cr) {        
		        while(mAddedRoutes.remove(r));
		        if (mAddedRoutes.contains(r) == false) {
		        		log("zyy:dns: Removing " + r + " for interface " + ifaceName);
		            try {
		            		mNetd.removeRoute(ifaceName, r);
		            } catch (Exception e) {
		            		// never crash - catch them all
		                loge("zyy:dns: Exception trying to remove a route: " + e);
		                		return;
		            }
		        } else {
		        		log("zyy:dns: not removing " + r + " for interface " + ifaceName + " as it's still in use");
		        }				
      	}			
		}
		
		private void addDefaultRouteForInterface(int iface_type) {
			//handleConnectivityChange(iface_type,false);
			//return;
			
			  LinkProperties curLp = mCurrentLinkProperties[iface_type];
        LinkProperties newLp = null;
        
        updateRoutes(curLp, newLp, mNetConfigs[iface_type].isDefault(),UNEXEMPT);
        ...
        for(RouteInfo r : cr) {        
            log("zyy:dns: Adding " + r + " for interface " + ifaceName);
            try {
        				if (mAddedRoutes.contains(r) == false) {
                ...
                // never crash - catch them all
                loge("zyy:dns: Exception trying to add a route: " + e);
            }
      	}			//*/
		}

		//return ture: connected.		
		private boolean trySetDefaultRoute(int netType)	{
				int iNetworkNum = 0;
				boolean isConnected = false;
				String value;
				log("zyy:dns: trySetDefaultRoute: begin..........");
				
				//get connected network number
				/*
				for (int t=0; t <= ConnectivityManager.MAX_RADIO_TYPE; t++) {
        //    if (t != mNetworkPreference && mNetTrackers[t] != null &&
            if (	mNetTrackers[t] != null &&
                    mNetTrackers[t].getNetworkInfo().isConnected()) {
                    log("tearing down " + mNetTrackers[t].getNetworkInfo() +
                            " in enforcePreference");
                iNetworkNum ++;
    	...
    	synchronized(mInternetLock){
				//return if only one connected net
				if(!mNetTrackers[ConnectivityManager.TYPE_ETHERNET].getNetworkInfo().isConnected()){
        		log("zyy:dns: trySetDefaultRoute: ethernet not connected");
        		mActiveDefaultNetwork = ConnectivityManager.TYPE_WIFI;
        		return isConnectedToInternet(mActiveDefaultNetwork);
        }
        if(!mNetTrackers[ConnectivityManager.TYPE_WIFI].getNetworkInfo().isConnected()){
        		log("zyy:dns: trySetDefaultRoute: wifi not connected");
        		mActiveDefaultNetwork = ConnectivityManager.TYPE_ETHERNET;
        		return isConnectedToInternet(mActiveDefaultNetwork);
        }
        
        //run here, ethernet and wifi are both in connected state
        //and both route info have been set
        
        //use ethernet if it has connected to internet
        if(mConnectedInternetState[ConnectivityManager.TYPE_ETHERNET]) {
        		...
        		mActiveDefaultNetwork = ConnectivityManager.TYPE_ETHERNET;
        		log("zyy:dns: ethernet is connected to internet!");
        		return true;
        }

				//detect it if new comming is ethernet
				if(netType == ConnectivityManager.TYPE_ETHERNET) {
						//restore ethernet setting as default		
						log("zyy:dns: none connected to internet, re-set ethernet route and dns!");									
						removeDefaultRouteForInterface(ConnectivityManager.TYPE_WIFI);		
						//addDefaultRouteForInterface(ConnectivityManager.TYPE_ETHERNET);				
						...
						if(isConnectedToInternet(ConnectivityManager.TYPE_ETHERNET)){
								log("zyy:dns: ethernet is connected to internet!");
								mConnectedInternetState[ConnectivityManager.TYPE_ETHERNET] = true;
								return true;
						}
						mConnectedInternetState[ConnectivityManager.TYPE_ETHERNET] = false;						
				}
				
				// run here, ethernet is not connected to internet
				
				 {	//new comming is wifi or ethernet is not connected to internet
						//remove ethernet default route, then add wifi default route
						removeDefaultRouteForInterface(ConnectivityManager.TYPE_ETHERNET);
						removeDefaultRouteForInterface(ConnectivityManager.TYPE_WIFI);
				    addDefaultRouteForInterface(ConnectivityManager.TYPE_WIFI);
						...
						if(mConnectedInternetState[ConnectivityManager.TYPE_WIFI]) {
								log("zyy:dns: wifi is connected to internet!");
								return true;
						}
		  			
		  			//new comming is wifi
		      	//if ok to internet, return
		      	if(netType == ConnectivityManager.TYPE_WIFI){
				      	if(isConnectedToInternet(ConnectivityManager.TYPE_WIFI)){
										log("zyy:dns: wifi is connected to internet!");
		
										mConnectedInternetState[ConnectivityManager.TYPE_WIFI] = true;
										return true;
								}
								mConnectedInternetState[ConnectivityManager.TYPE_WIFI] = false;
						}
				}
				
				//run here, neither ethernet nor wifi is connected to internet
										//restore ethernet setting as default		
				log("zyy:dns: none connected to internet, re-set ethernet route and dns!");									
				removeDefaultRouteForInterface(ConnectivityManager.TYPE_WIFI);		
				removeDefaultRouteForInterface(ConnectivityManager.TYPE_ETHERNET);
    ...
    private void handleConnect(NetworkInfo info) {
        ...
        // do NOT accept connection when pppoe already connected
        if (mActiveDefaultNetwork == ConnectivityManager.TYPE_PPPOE) {
            //zengyongying: remove wifi or ethernet dns&route
            //if(newNetType == ConnectivityManager.TYPE_ETHERNET) {
		        		...
		        		//set pppoe dns	
        				PppoeManager pm = (PppoeManager)mContext.getSystemService(Context.PPPOE_SERVICE);
								if(null != pm) {
										String value = pm.getDns1();
										if(value!=null && !value.equals(""))				SystemProperties.set("net.dns1", value);
										value = pm.getDns2();
										if(value!=null && !value.equals(""))				SystemProperties.set("net.dns2", value);
           			}else{
									log("PppoeManager is null");
								} 
								//}
            
            if (DBG) log("refuse connection because pppoe already connected");
            return;
        }else if (newNetType == ConnectivityManager.TYPE_PPPOE) {
             if (DBG) log("remove wifi&eth route while pppoe connected");
        ...
        if (mActiveDefaultNetwork == ConnectivityManager.TYPE_PPPOE){
        		isConnectInternet = isConnectedToInternet(ConnectivityManager.TYPE_PPPOE);
        		isConnectInternet = true;		//zengyongying: it should always connected to internet while pppoe connected
        }else {
		        isConnectInternet = trySetDefaultRoute(newNetType);//zengyongying
		        //zengyongying: notify app wheather connect to internet
		        //if(!isConnectInternet)
		        // 	isConnectInternet = isConnectedToInternet(mActiveDefaultNetwork);
      	}
        mConnectedInternetState[mActiveDefaultNetwork] = isConnectInternet;        
        NotifyConnectedToInternet(mNetTrackers[mActiveDefaultNetwork].getNetworkInfo(), isConnectInternet);
//add by gaoyunpei for setprop activenetwork
        switch (mActiveDefaultNetwork) {
            case ConnectivityManager.TYPE_WIFI:
                SystemProperties.set("net.activenetwork","WIFI");
                break;
            case ConnectivityManager.TYPE_ETHERNET:
                SystemProperties.set("net.activenetwork","ETHERNET");
                break;
            case ConnectivityManager.TYPE_PPPOE:
                SystemProperties.set("net.activenetwork","PPPOE");
                break;
         }
//end
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case NetworkStateTracker.EVENT_CONFIGURATION_CHANGED: {
                    ...
                    //       @see bug/4455071
                    log("invoke handleConnectivityChange 1");
