...
class AlarmManagerService extends IAlarmManager.Stub {
    ...
    static final String TAG_Sarah = "Sarah_AlarmManager_ScreenSaver";
	...
	//{{gaodw@tcl.com patch from ms628.
//added by cuix@tcl.com About standby interface in 2014.1.7
	private boolean isPowerOffTimerSet = false;
	private long poweroffTime = 0;
	private long poweroff_time_delay = 0; //for user setting

	public void setPoweroffTimer(long rel) {
		poweroff_time_delay = rel;
		if (rel >= 0) {
			isPowerOffTimerSet = true;
			poweroffTime = SystemClock.elapsedRealtime() + rel;
			Slog.d(TAG, "###### begin to start timer monitor .... ");
			new Thread(new TimeMonitorThread()).start();
		}
	}

	public void cancelPoweroffTimer() {
		poweroffTime = 0;
		poweroff_time_delay = 0;
		isPowerOffTimerSet = false;
	}

	public long getReminderRelativeTime() {
		return (poweroffTime - SystemClock.elapsedRealtime());
	}

	public long getPowerOffTimer() {
		return poweroff_time_delay;
	}

	public void onLastMinuteWarn() {
		//TODO Auto-generated method stub
		Intent intent = new Intent();
		intent.setAction("com.android.alarm.TIMELAST_MINUTE_WARN");
	...
	private class TimeMonitorThread extends Thread {
		...
		public void run() {
			while(isPowerOffTimerSet) {
				long PowerOffTimeLast;
				boolean LastMinute = false;
				Slog.d(TAG, "isPowerOffTimerSet: " + isPowerOffTimerSet);
				if(isPowerOffTimerSet) {
					Slog.w(TAG, "Power Off time set, begin to calculate last minutes.");
					PowerOffTimeLast = getReminderRelativeTime();
					Slog.w(TAG, "Power Off time set begin to calculate last minutes: " + PowerOffTimeLast);
					if((PowerOffTimeLast < 61 * 1000) && (PowerOffTimeLast >= 60 * 1000)) {
						if(!LastMinute) {
							Slog.d(TAG, " ++++++1 minute last ------");
							onLastMinuteWarn();
							LastMinute = true;
						}
					}
						...
						//shutdown action
						if(PowerOffTimeLast <= 1000) {
							isPowerOffTimerSet = false;
							int eventCode = 26; //powerkey from
							long now = SystemClock.uptimeMillis();
							Slog.d(TAG, " **** begin to send power down key ...");

							//@FIXME, no longer injectKeyEvent in IWindowManager, it should use inputmanager injectInputEvent instead
					...
					SystemClock.sleep(1000); //check in 1 time per s
...
/* Screensaver func */
        private boolean isScreenOffTimerSet = false;
        private boolean isScreenOffTimerSet_ON = false;
        private long screenoffTime = 0;
        private long screenoff_time_delay = 0; //for user setting
        private boolean screensaverON = false;

        public void setScreenoffTimer(long rel) {
                screenoff_time_delay = rel;
                Slog.d(TAG_Sarah, "###### setScreenoffTimer: " + rel);
        }

        public void startScreenoffTimer() {
		/* Get screenoff_time from cloud server */
		String systemtime = SystemProperties.get("persist.sys.screensavertime");
		screenoff_time_delay = Long.parseLong(systemtime);
		Slog.d(TAG_Sarah, "###### get system ScreenoffTimer: " + systemtime + "; str2long=" + screenoff_time_delay);
                if (screenoff_time_delay > 0) {
                        isScreenOffTimerSet = true;
                        isScreenOffTimerSet_ON = true;
                        screenoffTime = SystemClock.elapsedRealtime() + screenoff_time_delay;
                        Slog.d(TAG_Sarah, "###### begin to start timer monitor for Screen saver... ");
                        new Thread(new ScreenSaverTimeMonitorThread()).start();
                } else {
                        Slog.e(TAG_Sarah, "###### fail to begin to start timer monitor for Screen saver...time= " + screenoff_time_delay);
                }

        }
        public void stopScreenoffTimer() {
                screenoffTime = 0;
//              screenoff_time_delay = 0;
                isScreenOffTimerSet = false;
                isScreenOffTimerSet_ON = false;
        }
        ...
        public void EventResetScreenoffTimer() {
                Slog.d(TAG_Sarah, "###Sarah### EventResetScreenoffTimer");
                if(screenoff_time_delay >=0 ) { //screensaver listening
                        screenoffTime = SystemClock.elapsedRealtime() + screenoff_time_delay;
                        Slog.d(TAG_Sarah, "###### EventResetScreenoffTimer:screensaverON= " + screensaverON + "time=" + screenoffTime + "set_on=" + isScreenOffTimerSet_ON + "set=" + isScreenOffTimerSet);
                        if(screensaverON) {//screensaver on
                                isScreenOffTimerSet_ON = true;
                                Slog.d(TAG_Sarah, " **** begin to send left screen saver broadcast ...");
                                Intent intent = new Intent();
                                intent.setAction("com.android.alarm.HASEVENT_LEFT_SCREENSAVER");
                             //   getContext().sendBroadcast(intent);
				mContext.sendBroadcast(intent);
                        } else { //screensaver off
                //              screenoffTime = SystemClock.elapsedRealtime() + poweroff_time_delay;
                        }
                }
        }

        public long getReminderRelativeScreenTime() {
                long _timer = (screenoffTime - SystemClock.elapsedRealtime());
                if(_timer <= 0)
                        _timer = -1;
        ...
        public long getScreenOffTimer() {
                return screenoff_time_delay;
        }

/*      public void onLastMinuteWarnScreen() {
                //TODO Auto-generated method stub
                Intent intent = new Intent();
                intent.setAction("com.android.alarm.TIMELAST_MINUTE_WARN");
                getContext().sendBroadcast(intent);
...
*/
        class ScreenSaverTimeMonitorThread extends Thread {
                public ScreenSaverTimeMonitorThread() {
                        super("AlarmManager");
                }

                public void run() {
                        while(isScreenOffTimerSet) {
                                long ScreenOffTimeLast;
                                boolean LastMinute = false;
                                Slog.d(TAG_Sarah, "isScreenOffTimerSet: " + isScreenOffTimerSet);
                                if(isScreenOffTimerSet_ON) {
                                        Slog.w(TAG_Sarah, "Screen Off time set, begin to calculate last minutes.");
                                        ScreenOffTimeLast = getReminderRelativeScreenTime();
                                        Slog.w(TAG_Sarah, "Screen Saver Off time set begin to calculate last minutes: " + ScreenOffTimeLast);
                                        if((ScreenOffTimeLast < 61 * 1000) && (ScreenOffTimeLast >= 60 * 1000)) {
                                           if(!LastMinute) {
                                                Slog.d(TAG_Sarah, " ++++++1 minute last ------");
                                                //onLastMinuteWarn();
                                                LastMinute = true;
                                          }
                                        }
                                        ...
                                        //shutdown action
                                        if(ScreenOffTimeLast <= 1000) {
                                           isScreenOffTimerSet_ON = false;
                                        //              int eventCode = 26; //powerkey from
                                        //              long now = SystemClock.uptimeMillis();
                                           Slog.d(TAG_Sarah, " **** begin to send enter screen saver broadcast ...");
                                           Intent intent = new Intent();
                                           intent.setAction("com.android.alarm.NOEVENT_ENTER_SCREENSAVER");
//                                         getContext().sendBroadcast(intent);
				           mContext.sendBroadcast(intent);
                                       } 	
                                }
                                ...
                                try {
                                    ...
                                    Slog.e(TAG_Sarah, "Interrupted while waiting for ScreenSaver", iex);
                                    break;
                                }
