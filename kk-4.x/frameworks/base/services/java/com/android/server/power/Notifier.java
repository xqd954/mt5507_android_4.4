...
final class Notifier {
	...
	private static final int MSG_STANDBY = 4;
	private static final int MSG_PANEL_ON = 5;
	private static final int MSG_BGM_START = 6;
	private static final int MSG_START_RESUME_PACKAGE = 7;	
	private static final int MSG_PREPARE_SHUTDOWN = 8;
	
	private String resumePackageName = null;
	...
	/**
	* Called when the device want to send broadcast.
	*/	
	public void sendBgmStart() {
		if (DEBUG) {
			Slog.d(TAG, "sendBgmStart");
		}

		Message msg = mHandler.obtainMessage(MSG_BGM_START);
		msg.setAsynchronous(true);
		mHandler.sendMessage(msg);
	}
	
	private void sendBgmStartbroadcast() {
		if (DEBUG) {
			Slog.d(TAG, "sendBgmStartbroadcast");
		}

		Intent bgmStartIntent = new Intent("mtk.intent.bgm.start");				  
		bgmStartIntent.addFlags(Intent.FLAG_INCLUDE_STOPPED_PACKAGES);
		mContext.sendOrderedBroadcastAsUser(bgmStartIntent, UserHandle.ALL, null, null, mHandler, 0, null, null);
		Slog.d(TAG,"powerManagerservice: send mtk.intent.bgm.start intent!");
	}
	...
	*/	
	public void sendPanelOn() {
		if (DEBUG) {
			Slog.d(TAG, "sendPanelOn");
		}

		Message msg = mHandler.obtainMessage(MSG_PANEL_ON);
		msg.setAsynchronous(true);
		mHandler.sendMessage(msg);
	}
	
	private void sendPanelOnbroadcast() {
		if (DEBUG) {
			Slog.d(TAG, "sendPanelOnbroadcast");
		}

		Intent panelOnIntent = new Intent(Intent.ACTION_PANEL_ON);			  
		panelOnIntent.addFlags(Intent.FLAG_INCLUDE_STOPPED_PACKAGES);
		mContext.sendOrderedBroadcastAsUser(panelOnIntent, UserHandle.ALL, null, null, mHandler, 0, null, null);
		Slog.d(TAG,"powerManagerservice: send ACTION_PANEL_ON intent!");
	}
	...
	*/
	public void sendStandby() {
		if (DEBUG) {
			Slog.d(TAG, "sendStandby");
		}

		Message msg = mHandler.obtainMessage(MSG_STANDBY);
		msg.setAsynchronous(true);
		mHandler.sendMessage(msg);
	}
	
	private void sendStandbybroadcast() {
		if (DEBUG) {
			Slog.d(TAG, "sendStandbybroadcast");
		}

		Intent standbyIntent = new Intent(Intent.ACTION_STANDBY); 		  
		standbyIntent.addFlags(Intent.FLAG_INCLUDE_STOPPED_PACKAGES);
		mContext.sendOrderedBroadcastAsUser(standbyIntent, UserHandle.ALL, null, null, mHandler, 0, null, null);
		Slog.d(TAG,"powerManagerservice: send standby intent!");
	}
	
	/**
	* start intent_to_resolve package or last top package
	*/
	public void startResumePackageMsg(String packageName) {
		if (DEBUG) {
			Slog.d(TAG, "startResumePackageMsg packageName="+packageName);
		}

		resumePackageName = packageName;
		Message msg = mHandler.obtainMessage(MSG_START_RESUME_PACKAGE);
		msg.setAsynchronous(true);
		mHandler.sendMessage(msg);
	}
	
	private void startResumePackage() {
		if (DEBUG) {
			Slog.d(TAG, "startResumePackage, resumePackageName="+resumePackageName);
		}

		Slog.d(TAG, "startResumePackage, resumePackageName="+resumePackageName);
		Intent resumeIntent = mContext.getPackageManager().getLaunchIntentForPackage(resumePackageName);			
		Slog.d(TAG, "startResumePackage, resumeIntent="+resumeIntent);
		if(null!=resumeIntent){
			mContext.startActivity(resumeIntent);
			Slog.d(TAG,"powerManagerservice: startActivity:"+resumePackageName);
		}
	}
	...
	* Called when the device want to send broadcast.
	*/
	public void sendPrepareShutdown() {
		if (DEBUG) {
			Slog.d(TAG, "sendPrepareShutdown");
		}

		Message msg = mHandler.obtainMessage(MSG_PREPARE_SHUTDOWN);
		msg.setAsynchronous(true);
		mHandler.sendMessage(msg);
	}
	
	private void sendPrepareShutdownbroadcast() {
		if (DEBUG) {
			Slog.d(TAG, "sendPrepareShutdownbroadcast");
		}

		Intent preShutdownIntent = new Intent(Intent.ACTION_PREPARE_SHUTDOWN);		  
		preShutdownIntent.addFlags(Intent.FLAG_INCLUDE_STOPPED_PACKAGES);
		mContext.sendOrderedBroadcastAsUser(preShutdownIntent, UserHandle.ALL, null, null, mHandler, 0, null, null);
		Slog.d(TAG,"powerManagerservice: send prepare shutdown intent!");
	}
    ...
    private void sendNextBroadcast() {
		...
		boolean isPowerStateUnkown=false;
        synchronized (mLock) {
            if (mBroadcastedPowerState == POWER_STATE_UNKNOWN) {
                // Broadcasted power state is unknown.  Send wake up.
                isPowerStateUnkown = true;
        ...
        if (powerState == POWER_STATE_AWAKE) {
            sendWakeUpBroadcast(isPowerStateUnkown);
        } else {
            sendGoToSleepBroadcast(goToSleepReason);
        }
    }

    private void sendWakeUpBroadcast(boolean isPowerStateUnkown) {
        if (DEBUG) {
			Slog.d(TAG, "Sending wake up broadcast.isPowerStateUnkown="+isPowerStateUnkown);
        }
        ...
        if (ActivityManagerNative.isSystemReady()) {
			if(!isPowerStateUnkown){
				Slog.d(TAG,"powerManagerservice: send ACTION_SCREEN_ON!");
				
				mContext.sendOrderedBroadcastAsUser(mScreenOnIntent, UserHandle.ALL, null,
                	    mWakeUpBroadcastDone, mHandler, 0, null, null);
			}else{
			   	mContext.sendOrderedBroadcastAsUser(new Intent(), UserHandle.ALL, null,
                	    mWakeUpBroadcastDone, mHandler, 0, null, null);
			}
			
			Slog.d(TAG,"powerManagerservice: send mScreenOnIntent!");
        } else {
    ...
    private void sendGoToSleepBroadcast(int reason) {
        if (DEBUG) {
			Slog.d(TAG, "Sending go to sleep broadcast.reason="+reason);
        }
        ...
        if (ActivityManagerNative.isSystemReady()) {
			Slog.d(TAG,"powerManagerservice: send ACTION_SCREEN_OFF!");
    ...
    private final class NotifierHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
				...
				case MSG_STANDBY:
					sendStandbybroadcast();
					break;

				case MSG_PANEL_ON:
					sendPanelOnbroadcast();
					break;

				case MSG_BGM_START:
					sendBgmStartbroadcast();
					break;

				case MSG_START_RESUME_PACKAGE:
					startResumePackage();
					break;

				case MSG_PREPARE_SHUTDOWN:
					sendPrepareShutdownbroadcast();
					break;
            }
        }
    }
