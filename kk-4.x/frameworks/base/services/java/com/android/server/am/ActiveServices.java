...
public final class ActiveServices {
    ...
    // MStar Android Patch Begin  from ms628.
    static final int BG_START_TIMEOUT = 3*1000;
    ...
    private final String bringUpServiceLocked(ServiceRecord r,
        ...
        if ("true".equals(SystemProperties.get("ro.config.low_ram", "false"))) {
            Slog.v(TAG, "r.killedbyLMK = "+r.killedbyLMK);
            if (r.killedbyLMK) {
                if (!checkServiceRestartMemoryLocked(r)) {
                    Slog.v(TAG, "check "+ r +" restart memory fail");
                    return null;
                }
            }
        }
    ...
    private boolean checkServiceRestartMemoryLocked(ServiceRecord sr) {
        ...
        if (!isRestart) {
            ...
            if (avgFreeMem > mAm.mProcessList.getMemLevel(ProcessList.CACHED_APP_MAX_ADJ) ||
                ...
                Slog.e(TAG, "SVC-DelayRestart: Mem not enough to restart sr=" + sr.shortName + ", wait " + delayTime + "ms, free mem="
                         + avgFreeMem + " mem usage=" + sr.memUsage + " MIN ADJ level=" + mAm.mProcessList.getMemLevel(ProcessList.CACHED_APP_MAX_ADJ));
            }
        }
