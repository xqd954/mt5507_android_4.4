...
//add patch for bluetooth usb dongle hothug issue from alltek by yjw start 20150127
	private boolean has_bt_device = false;
	//private static final String CHECK_BLUETOOTH_DRIVER_NAME="dev/btusb0";
	private static final String CHECK_BLUETOOTH_DRIVER_NAME="proc/driver/btusb/btusb0";
//add patch for bluetooth usb dongle hothug issue from alltek by yjw end 20150127
...
//add patch for bluetooth usb dongle hothug issue from alltek by yjw start 20150127
				new Thread(new Runnable() {
					...
					for(;;) 						  
					{
				  		if (new File(CHECK_BLUETOOTH_DRIVER_NAME).exists())
						{
							//Log.i(TAG, "bt hotplug check : " + CHECK_BLUETOOTH_DRIVER_NAME + " exists ");
							has_bt_device=true;
							if(mEnableExternal==true)
							{
							  if(mState == BluetoothAdapter.STATE_OFF){
							    //sendEnableMsg(mEnableExternal);
							    sendEnableMsg(mQuietEnableExternal);
							  	Log.i(TAG, "bt hotplug check :  bt auto open  £¬mQuietEnableExternal ");
							  }
							}
						}
						 else		 
						{
							//Log.i(TAG, "hotplug " + CHECK_BLUETOOTH_DRIVER_NAME + " not exists ");
							if(has_bt_device==true)
							{
								if(mState == BluetoothAdapter.STATE_ON)
								{
									// sendEnableMsg(false);
									sendDisableMsg();
									Log.i(TAG, "bt hotplug check :  bt auto close ");
								}
								 else
								{
									//Log.i(TAG, "bt hotplug check :  bt not open ");
								}
							}
							has_bt_device=false;
