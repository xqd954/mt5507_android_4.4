...
final class ActivityStack {
    ...
    static boolean readyToKillApp = false;
    private AppWhiteList mAppWhiteList = null;
    ...
    ActivityStack(ActivityManagerService service, Context context, Looper looper, int stackId) {
        ...
        mAppWhiteList = new AppWhiteList(mContext);
    }
    ...
    private void completePauseLocked() {
        ...
        if (!mService.isSleepingOrShuttingDown()) {            
            Slog.d("huoph","resumeTopActivityLocked");
            readyToKillApp = true;
   ...
   private  String getLauncherPackageName(Context context)    {
          ...
          //modify by yangzp@tcl.com for filter the thirdpart launcher app install
		  ...
		  if(res.activityInfo.packageName.equals("android")){
			  ...
			  if(res.activityInfo.packageName.length() < SystemProperties.PROP_VALUE_MAX)
				    SystemProperties.set("sys.currentlauncher", res.activityInfo.packageName);
	...
	//qiyx add 150528 for heletime project
	...
	public String queryClientType() {
		...
		//Slog.d("MiniDataBase-kofi", "queryClientType...,client type: " +  clientType);
		return clientType;
	}

	//add by gaodw. fade mouse. patch from ms918.
	//TCL Patch Begin
	//In TV APP , It should disable pointer
	//autor chenglan@tcl.com
	private static boolean mAppCursorState = false;
	private static final String  mTvPkgName = "com.tcl.tv";
	//TCL Patch End
	//end
...
/*		//qiyx mod 150616
	//add heletime for customer need by gaodw. start
	if(queryClientType().equals(clientType_F3850A)){
		Slog.d(TAG,"isStartUiHeleTime ============================================ ");
		Slog.d(TAG,"bootfromui heletime is "+SystemProperties.get("persist.sys.heletime"));
		boolean isStartUiHeleTime = "true".equals(SystemProperties.get("persist.sys.heletime","true"));
		Slog.d(TAG,"isStartUiHeleTime is "+isStartUiHeleTime);
		if(isStartUiHeleTime){
			return mStackSupervisor.resumeTvHeleTimeActivity();
		}
	}
	//add heletime for customer need by gaodw. end
*/

	//huoph add here for start tv or ui when the system first boot up

	Slog.d(TAG,"bootfromui is "+SystemProperties.get("persist.sys.bootfromui"));

	boolean isStartUi = "true".equals(SystemProperties.get("persist.sys.bootfromui","false"));

	  try
	    {
	    	boolean pModeStatus = TDeviceInfo.getInstance().getFactoryPModeStatus();
		if(pModeStatus){
			isStartUi = false;
			Slog.d(TAG,"PMode is on,set bootfromui false");
		}		
	    }catch (Exception e){
		e.printStackTrace();
	    }
	
	Slog.d(TAG,"isStartUi is "+isStartUi);

	if(!isStartUi){
		return mStackSupervisor.resumeTvActivity();	
	}else{
	...
	//end by huoph
                ...
                Slog.d(TAG,"resumeTopActivty");
     //          if(next.packageName.equals("com.tcl.tv")){
		 if(next.packageName.equals("com.tcl.tv") || next.packageName.equals("com.tcl.common.bootsetwizard")){
                Slog.d(TAG, "set persist.sys.bootformui false ");
                SystemProperties.set("persist.sys.bootfromui","false");
               }else{
		Slog.d(TAG, "set persist.sys.bootformui true ");
                SystemProperties.set("persist.sys.bootfromui","true");
		}
		
		
	if(next.realActivity.getClassName().equals("com.huan.edu.tvplayer.EduPlayerActivity")){
               Slog.d(TAG, "set sys.video.fullscreen true ");
               SystemProperties.set("sys.video.fullscreen","true");
	//	RootSeeker.exec("am broadcast -a 'com.tcl.video.fullscreen' --es state true");
		Intent intent = new Intent("com.tcl.video.fullscreen");
		intent.putExtra("state","true");
		mContext.sendBroadcast(intent);

            }else{
	       Slog.d(TAG,"next.packageName:"+next.packageName);
	       if(next.packageName.startsWith("com.huan.edu") || next.packageName.startsWith("com.edu.child") ){
               Slog.d(TAG, "set sys.video.fullscreen false ");
	  ...
	  //     RootSeeker.exec("am broadcast -a 'com.tcl.video.fullscreen' --es state false");
		...
		//add by gdw:gaodw@tcl.com: xi'an bsp at:2015-03-11
                    try{
				    String curPackageName=next.packageName;
				    Slog.e(TAG, "===david===>>> next.pakageName:  "+  curPackageName  );
				    Intent i=new Intent("com.iflytek.itvs.skipxiri");
				    if( curPackageName.equals("com.tcl.common.mediaplayer") ){
						i.putExtra("skip", true);
						Slog.e(TAG,"======david===== cur mediaplayer  skip true");
	                                }else{
						i.putExtra("skip", false);
						Slog.e(TAG,"======david===== cur not mediaplayer  skip false");
	                                }
        ...
        if(next.packageName.equals(getLauncherPackageName(mContext))){
             if(readyToKillApp){
                  //kill
                  Slog.d("huoph", "App White List called");
                  mAppWhiteList.onKillBackProcess();
        
                  readyToKillApp = false;
             }
        }
        
        /*
        //add patch from ms918 by gaodw.
        //  Log.d(TAG,"resumeTopActivty");
        try{
             Slog.d(TAG,"class name :" + next.realActivity.getClassName());
             if(next.realActivity.getClassName().equals("com.tcl.tv.TVActivity")){
                   Slog.d(TAG, "set sys.pointer.fadestat 1 ");
                   SystemProperties.set("sys.pointer.fadestat","1");
                }else{
               Slog.d(TAG, "set sys.pointer.fadestat 0 ");
                   SystemProperties.set("sys.pointer.fadestat","0");
            }
        }catch(Exception ex){
             ex.printStackTrace();
        }
        ...
        */
		...
		//In TV APP , It should disable pointer
		//autor chenglan@tcl.com
		if(next.packageName.equals(mTvPkgName)){
			if( !mAppCursorState){
				//if (DEBUG_TV) Slog.v(TAG, "TV App Disable Pointer!");
				Slog.w(TAG, "********TV App Disable Pointer!  *********");
				mAppCursorState = true;
				mService.mWindowManager.disablePointer();
			}
		}else{
			if(mAppCursorState){
				//if (DEBUG_TV) Slog.v(TAG, "TV App Enable Pointer!");
				Slog.w(TAG, "********TV App Enable Pointer!  *********");
				mAppCursorState = false;
				mService.mWindowManager.enablePointer();
			}
		}
		//TCL Patch End
        //end
    ...
    final void startActivityLocked(ActivityRecord r, boolean newTask,
                ...
                if (task == r.task) {
                    ...
                    if (!startIt) {
                        ...
                        mWindowManager.addAppToken(task.mActivities.indexOf(r), r.appToken,
                                r.task.taskId, mStackId, r.info.screenOrientation, r.info.displayMode, r.fullscreen,
                                (r.info.flags & ActivityInfo.FLAG_SHOW_ON_LOCK_SCREEN) != 0,
        ...
        if (!isHomeStack() || numActivities() > 0) {
            ...
            mWindowManager.addAppToken(task.mActivities.indexOf(r),
                    r.appToken, r.task.taskId, mStackId, r.info.screenOrientation, r.info.displayMode, r.fullscreen,
    ...
    final boolean finishActivityLocked(ActivityRecord r, int resultCode, Intent resultData,
        ...
        // mtk40449 starts for sync finish activities in fast boot feature
        if (reason.equals("sync-finish"))
        {
            mStackSupervisor.mSyncFinishingActivities.add(r);
        }
        // mtk40449 ends for sync finish activities in fast boot feature        
        ...
        try {
            ...
            if (isInStackLocked(token) != null) {
                if (r.state == ActivityState.DESTROYING) {
                    mStackSupervisor.mSyncFinishingActivities.remove(r);                    
