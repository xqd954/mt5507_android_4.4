...
public final class ActivityStackSupervisor {
    ...
    /** List of activities that are ready to be sync finished, but waiting for the previous activity to
     * settle down before doing so.  It contains ActivityRecord objects. */
    final ArrayList<ActivityRecord> mSyncFinishingActivities = new ArrayList<ActivityRecord>();
...
//huoph add this func for auto start ui
    boolean resumeTvActivity() {
         
        return mService.startTv(mCurrentUser);
    }
//end by huoph

//huoph add this func for auto start ui
    boolean resumeTvHeleTimeActivity() {
         
        return mService.startTvHeleTime(mCurrentUser);
    }
//end by huoph
    ...
    void startTvActivity(Intent intent, ActivityInfo aInfo) {
       // moveHomeToTop();
...
void startTvHeleTimeActivity(Intent intent, ActivityInfo aInfo) {
       // moveHomeToTop();
        startActivityLocked(null, intent, null, aInfo, null, null, 0, 0, 0, null, 0,
                null, false, null);
    ...
    final int startActivityLocked(IApplicationThread caller,
        ...
        if (err == ActivityManager.START_SUCCESS) {
			...
			Slog.i(TAG, "======davild=======set sys.voice.speaker 0 ==========");	
    ...
    final ActivityRecord activityIdleInternalLocked(final IBinder token, boolean fromTimeout,
        ...
        //if (isFrontStack(mHomeStack)) //modified by huoph for start tv as the first Activity,so ignore this.
	{
    ...
    public boolean isReadyToSuspendLocked() {
            int size = mSyncFinishingActivities.size();

            if (size > 0) {
                // Still need to tell some activities to destroy; can't Suspend yet.
                Slog.v(TAG, "Suspend still need to destroy " + size + " activities");
                return false;
            }
