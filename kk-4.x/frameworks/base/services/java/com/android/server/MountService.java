...
class MountService extends IMountService.Stub
	...
	/* Begin to modify by Sarah Zhang(zxia@tcl.com) */
        public static final int VolumeInfoLabelResult          = 113;
        public static final int VolumeInfoFstypeResult         = 114;
        public static final int VolumeInfoUuidResult           = 115;
        public static final int VolumeInfoDevpathResult        = 116;
	...
	//}}
        ...
        public static final int SoundCardInserted              = 640;
        public static final int SoundCardRemoved               = 641;
    ...
    //{{gaodw@tcl.com. patch from ms628.
	private /*final lvh@tcl , resort */ ArrayList<StorageVolume> mVolumes = Lists.newArrayList();
    //}}
	...
	//{{gaodw@tcl.com  patch from ms628.
// Sarah Zhang(zxia@tcl.com) Patch Begin
	/**
	 * Check if we need to simulate this volume as sdcard
	 */	
    boolean checkNeedSimulateSD(String path){
	/*	
        StorageVolume targetVolume = null;
        StorageVolume primaryVolume = null;
		
        if(!mExternalStorageEmulated){
            return false;
        }
        ...
        synchronized (mVolumesLock) {
           ...
           for (StorageVolume volume : mVolumes) {				
            ...
            if(targetVolume == null){
                Slog.d(TAG, "No target volume found for " + targetVolume);
                return false;
            }
			
			//check if current primary storage is mounted		
            for (StorageVolume volume : mVolumes) {
                if (volume.isPrimary()) {
                    //Check if this volume is mounted
					String vpath = volume.getPath();
					String state = mVolumeStates.get(vpath);
                    if(state == Environment.MEDIA_MOUNTED){
                        //We don't need to do anything
                        Slog.d(TAG, "Primary volume " + volume.getPath() + " is already mounted");
                        return false;
                    }
            ...
            //Reach here means currently no primary volume is mounted		
            if(primaryVolume != null){
                primaryVolume.setPrimary(false);
            }					
			
            targetVolume.setPrimary(true);
            Slog.d(TAG, "Volume " + targetVolume.getPath() + " is set to primary, replace " + primaryVolume.getPath());						
			
            //rearrange volume list to make sure primary is fist
			
            ArrayList<StorageVolume> newVolumes = Lists.newArrayList();
            for (StorageVolume volume : mVolumes) {
	            //Do we need to rearrange?				
                if(volume.isPrimary()){
                    newVolumes.add(0, volume);
                }else{
...
// Sarah Zhang(zxia@tcl.com) Patch End
//}}
    /**
			...
			Slog.d(TAG, "volstr =" + volstr);
                       ...
                       /* final StorageVolume volume;
                        ...
                        if (state != null) {
                            ...
                            synchronized (mVolumesLock) {
                                volume = mVolumesByPath.get(path);
                                long maxFileSize_ex = 4096 * 1024L * 1024L;
                                if (volume == null) {
 /*                                   volume = new StorageVolume(new File(path),
                                        primary.getDescriptionId(), false, true, false, 0,
                                        false, 0, null);*/

                                if (path.equals("/storage/sdcard1")) {
                                        Slog.i("zxia_debug", "new StorageVolume is for sdcard1!");
                                        volume = new StorageVolume(new File(path),
                                                111111, false, true, false, 0,
                                                false, maxFileSize_ex, null);
                                } else{

                                        Slog.i("zxia_debug", "new StorageVolume is for usb disk!");
                                        volume = new StorageVolume(new File(path),
                                                222222, false, true, false, 0,
                                                false, maxFileSize_ex, null);
                                }
	...
	Slog.i("Sarah_MountService", "onEvent begin and code=" + code);
        if (DEBUG_EVENTS) {
        ...
        //2012/8/6,added by Daniel Hsiao,for USB Audio detection support
        else if ((code == VoldResponseCode.SoundCardInserted) || (code == VoldResponseCode.SoundCardRemoved))
        {
            Slog.d(TAG, "VoldResponseCode.SoundCard" + 
                           ((code == VoldResponseCode.SoundCardInserted) ? "Inserted" : "Removed"));
            int state = ((code == VoldResponseCode.SoundCardInserted) ? 1 : 0);
            int card = -1;
            int device = -1;

            try {
                String[] rawTok = raw.split(" ");
                card = Integer.parseInt(rawTok[1]);
                device = Integer.parseInt(rawTok[2]);
            } catch (Exception ex) {
                Slog.e(TAG, "Failed to parse card/device", ex);
            }
    ...
    private void notifyVolumeStateChange(String label, String path, int oldState, int newState) {
	    ...
	    Slog.i("Sarah_MountService","notifyVolumeStateChange:getVolumeState " + state);
        }
        if (DEBUG_EVENTS) Slog.i(TAG,"Sarah notifyVolumeStateChange::" + oldState + ", new=" + newState + ", state=" + state);
        ...
        if (newState == VolumeState.Init) {
			...
			//{{gaodw@tcl.com  patch from ms628.
           // Sarah Zhang(zxia@tcl.com) Patch Begin
            Slog.e(TAG, "SARAH notifyVolumeStateChange: nomedia status and call update state");
            updatePublicVolumeState(volume, Environment.MEDIA_UNMOUNTED);
            sendStorageIntent(Intent.ACTION_MEDIA_UNMOUNTED, volume, UserHandle.ALL);
            // Sarah Zhang(zxia@tcl.com) Patch End
			//}}
	    ...
	    // Sarah Zhang(zxia@tcl.com) Patch Begin
            if (oldState == VolumeState.Unmounting) {
                Slog.e(TAG, "Sarah: from Idle to unmounting and unnecessary to addVolume.");
                updatePublicVolumeState(volume, Environment.MEDIA_UNMOUNTED);
		...
		if (DEBUG_EVENTS) Slog.i(TAG, "updating volume state for media bad removal nofs and unmountable");
                synchronized (mVolumesLock) {
                    ...
                    if (volume == null) {
                      /*  volume = new StorageVolume(new File(path),
                            getPrimaryPhysicalVolume().getDescriptionId(), false, true, false, 0,
                            false, 0, null);*/
                        if (path.equals("/storage/sdcard1")) {
                               Slog.i("zxia_debug", "new StorageVolume onevent is for sdcard1!");
                               volume = new StorageVolume(new File(path),
                               111111, false, true, false, 0,
                               false, maxFileSize_ex, null);
                       } else{
                               Slog.i("zxia_debug", "new StorageVolume onevent is for usb disk!");
                               volume = new StorageVolume(new File(path),
                               222222, false, true, false, 0,
                               false, maxFileSize_ex, null);
                       }
	...
	Slog.i("Sarah_MountService", "after new NativeDaemonConnector");
    ...
    public int mountSecureContainer(String id, String key, int ownerUid) {
        ...
        Slog.d(TAG, "mountSecureContainer: id=" + id + " key=" + key + " ownerUid=" + ownerUid);
	...
	//{{gaodw@tcl.com  patch from ms628.
    /* Begin to modify by Sarah Zhang(zxia@tcl.com) */
    public String getVolumeLabel_TCL(String path) {
		try{
            final String[] mLabel = NativeDaemonEvent.filterMessageList(
                    mConnector.executeForList("volume", "label", path),
                    VoldResponseCode.VolumeInfoLabelResult);
            return mLabel[0];
        }catch(NativeDaemonConnectorException e){
            Slog.e(TAG,"Error when get volume label from TCL!");
            return new String();
        }
    }

    public String getVolumeFstype(String path) {
        try{
            final String[] mFstype = NativeDaemonEvent.filterMessageList(
                    mConnector.executeForList("volume", "fstype", path),
                    VoldResponseCode.VolumeInfoFstypeResult);
            return mFstype[0];
        }catch(NativeDaemonConnectorException e){
            Slog.e(TAG,"Error when get volume Fstype from TCL!");
            return new String();
        }
    }
    ...
    public String getVolumeUuid(String path) {
        try{
            final String[] mUuid = NativeDaemonEvent.filterMessageList(
                    mConnector.executeForList("volume", "uuid", path),
                    VoldResponseCode.VolumeInfoUuidResult);
            return mUuid[0];
        }catch(NativeDaemonConnectorException e){
            Slog.e(TAG,"Error when get volume uuid from TCL!");
            return new String();
        }
    }
    public String getVolumeDevpath(String path) {
        try{
            final String[] mDevpath = NativeDaemonEvent.filterMessageList(
                    mConnector.executeForList("volume", "devpath", path),
                    VoldResponseCode.VolumeInfoDevpathResult);
            return mDevpath[0];
        }catch(NativeDaemonConnectorException e){
            Slog.e(TAG,"Error when get volume devpath from TCL!");
            return new String();
        }
    }
    /* End to modify by Sarah Zhang(zxia@tcl.com) */
