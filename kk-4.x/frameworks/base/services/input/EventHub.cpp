...
status_t EventHub::mapKey(int32_t deviceId, int32_t scanCode, int32_t usageCode,
    ...
    if (device) {
        ...
        if (device->keyMap.haveKeyLayout()) {
			...
			//add by dupeng for front pad key 20141024 by lulin 20150108
               if(!strncmp(device->identifier.name.string(), "mtkinp_events_panel",19)){

                 (*outFlags) |= POLICY_FLAG_DISABLE_KEY_REPEAT;
                  ALOGD("dupeng disable the front pad's key repeat.\n");
                }
          	//lulin add end
                return NO_ERROR;
            }
            ...
            if (eventItem.events & EPOLLIN) {
                ...
                if (readSize == 0 || (readSize < 0 && errno == ENODEV)) {
                    ...
                    for (size_t i = 0; i < count; i++) {
                        ...
                        //add by dupeng for fade mouse and mouse key 20140806
                        if(iev.type == EV_REL || (iev.type == EV_KEY && (iev.code == 272 || iev.code == 273 || iev.code == 274))) 
                        {
                            bool bEnable = false;
                            char value[PROPERTY_VALUE_MAX];
                            property_get("sys.pointer.fadestat", value, "0");
                            bEnable = (atoi(value) == 0)?(true):(false);
                            if(!bEnable){
                                ALOGE("fade mouse success.");
                                continue;
                            } 
                            if(false){
                            char fade_mouse[20]; //bool fade_mouse;
                            if( property_get("sys.pointer.fadestat", fade_mouse, "false") > 0) 
                            {
                                ALOGE("get prop sys.pointer.fadestat success :%s.", fade_mouse);
                                if(strstr(fade_mouse, "true") != NULL)
                                {
                                   ALOGE("fade mouse success.");
                                   continue;
                                }
...
status_t EventHub::openDeviceLocked(const char *devicePath) {
    ...
    // Allocate device.  (The device object takes ownership of the fd at this point.)
        //added by chenglan set the deviceId of mouse ops.
    int32_t deviceId;
#ifdef NEED_CHANGE_MODE
    if(strstr(identifier.name,"Hillcrest")){//identifier.name=="Hillcrest Labs Virtual Pointer"||identifier.name=="Hillcrest MotionEngine Cursor Mover: #0"){//Compatible with two kinds of Dongle
		deviceId = SENSOR_MOUSE_ID;
    }else if(identifier.name=="Tcl Smart TV IR Receiver"){
    	deviceId = IR_RC_ID;
    }else{
	...
	//{{lvh@tcl, always show softkeyboard
