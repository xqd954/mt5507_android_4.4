...
static bool skipCursorProcess = false;
static bool gEnableSrcoll = true;
//duanliang end
//end
...
//add by gaodw fade mouse. patch from ms918.
//add by duanliang for cusor fade
void InputReader::setPointerState(bool enablepointer)
{
    bEnablePointer = enablepointer;
	if(bEnablePointer==true){
	    ALOGD("===========forbid mouse event=================");
		//fadePointerLocked();
	}else{
             skipCursorProcess = false;
	     ALOGD("===========resume mouse event=================");
	}
}
//add end
//end
...
#ifdef NEED_CHANGE_MODE
void InputReader::removeFocusEventLocked(RawEvent* rawEvents, size_t count){
		RawEvent* rawEvent = rawEvents;
		int32_t deviceId = IR_RC_ID;
	
		rawEvent->deviceId=deviceId;
		rawEvent->flags=0;
		rawEvent->keyCode=AKEYCODE_MOUSE_WAKE;
		rawEvent->code=SCANCODE_MOUSE_WAKE;
		rawEvent->type=1;
		rawEvent->value=1;
		//down event	
				
		rawEvent[LOCAL_INDEX_SECOND].deviceId=deviceId;
		rawEvent[LOCAL_INDEX_SECOND].flags=0;
		rawEvent[LOCAL_INDEX_SECOND].keyCode=AKEYCODE_MOUSE_WAKE;
		rawEvent[LOCAL_INDEX_SECOND].code=SCANCODE_MOUSE_WAKE;
		rawEvent[LOCAL_INDEX_SECOND].type=1;
		rawEvent[LOCAL_INDEX_SECOND].value=0;
		//up event
		processEventsForDeviceLocked(deviceId, rawEvent, count);
}
#endif
...
#endif
			//added by chenglan  Change the mode between Mouse and Focus 
#ifdef NEED_CHANGE_MODE
			uint32_t classes = mEventHub->getDeviceClasses(deviceId);
			if(SENSOR_MOUSE_ID != deviceId && (classes & INPUT_DEVICE_CLASS_CURSOR)){
#if DEBUG_RAW_EVENTS
					ALOGD("type:%d scancode:%d keycode:%d vlaue:%d flags:%d",
					rawEvent->type,rawEvent->code,rawEvent->keyCode,rawEvent->value,rawEvent->flags);
#endif
			}else{
				...
				RawEvent* rawEventLocal = const_cast<RawEvent*>(rawEvent);
				nsecs_t now;
...
void InputReader::processEventsForDeviceLocked(int32_t deviceId,
	...
	property_get("sys.mtk.keyDisable", s_prop, "0");
    if (strcmp(s_prop, "1") == 0) {
       SLOGI("Key was disabled in Android!!\n");
       return;
    } else {
       //SLOGI("Key was processed in Android!!\n");
    }
    if (device->isIgnored()) {
...
uint32_t CursorButtonAccumulator::getButtonState() const {
    ...
    // gaodw@tcl.com change the mouse key event, right->back, middle-> menu ....
    if (mBtnRight) {
...
void CursorInputMapper::process(const RawEvent* rawEvent) {
    ...
    //ALOGE("[ESTALENT] value: %d ,type: %d, code: %d, bEnablePointer: %d\n", 
    //      rawEvent->value, rawEvent->type, rawEvent->code, bEnablePointer);
    if (skipCursorProcess) {
        return ;
    }
	//add by gaodw fade mouse. patch from ms918.
	//duanliang for cursor fade
    if (rawEvent->type == EV_SYN && rawEvent->code == SYN_REPORT) {
    //	if ((rawEvent->type == EV_SYN && rawEvent->code == SYN_REPORT)&&bEnablePointer==false){
	//duanliang end
	//end
        sync(rawEvent->when);
        ALOGE("[ESTALENT] currentButtonState: %#x\n", mCursorButtonAccumulator.getButtonState());
        if (mCursorButtonAccumulator.getButtonState() == 0 && bEnablePointer) {
            skipCursorProcess = true;
            fadePointer();
        }
    }
}
...
void CursorInputMapper::sync(nsecs_t when) {
    ...
    if (mPointerController != NULL) {
		...
		//add by gaodw fade mouse. patch from ms918.
		//added by chenglan When occured the Wheel Event,The Cursor shouldn't be showed.
		if(!scrolled){
