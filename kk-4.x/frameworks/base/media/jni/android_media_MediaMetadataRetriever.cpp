...
//MTK Begin
static void android_media_MediaMetadataRetriever_setThumbnail(JNIEnv *env, jobject thiz, jboolean isThumbnail)
{
    ALOGV("setThumbnail");
    MediaMetadataRetriever* retriever = getRetriever(env, thiz);
    if (retriever == 0) {
    ...
    process_media_retriever_call(env, retriever->setThumbnail(isThumbnail), "java/lang/RuntimeException", "setThumbnail failed");
}

static void android_media_MediaMetadataRetriever_setRetriveverSvctxPath(JNIEnv *env, jobject thiz, jstring p_svctx_path)
{
    ...
    process_media_retriever_call(env,
		                        retriever->setRetriveverSvctxPath(ptemp_path), 
		                        "java/lang/RuntimeException",
		                        "setRetriveverSvctxPath failed");
}
...
static JNINativeMethod nativeMethods[] = {
		...
		//MTK Begin	
		{
			"_setThumbnail",	 
			"(Z)V",
			(void *)android_media_MediaMetadataRetriever_setThumbnail
		},
		{  
			"native_setRetriveverSvctxPath",
		    "(Ljava/lang/String;)V",
			(void *)android_media_MediaMetadataRetriever_setRetriveverSvctxPath
		},
	    //MTK End 
};
