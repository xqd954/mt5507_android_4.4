...
//add by tank@tcl.com for LONG_VOLUME_MUTE
//add for volume mute key  long press start screen shot by yjw 20141020 start
        private boolean isScreenshotKey =false;
        private boolean isFirstRepeatFlag =true;
        private static final int REPEAT_TIME  = 10 ;
//add for volume mute key  long press start screen shot by yjw 20141020 end
//end tank@tcl.com
    ...
    /**
     * @hide Broadcast intent when the mute state for a particular stream type changes.
     * Includes the stream, the new state and previous state
     */
    @SdkConstant(SdkConstantType.BROADCAST_INTENT_ACTION)
    public static final String MUTE_CHANGED_ACTION = "android.media.MUTE_CHANGED_ACTION";
   ...
   /**
     * @hide The stream type for the mute changed intent.
     */
    public static final String EXTRA_MUTE_STREAM_TYPE = "android.media.EXTRA_MUTE_STREAM_TYPE";


    /**
     * @hide The mute associated with the stream for the mute changed intent.
     */
    public static final String EXTRA_MUTE_STREAM_VALUE =
        "android.media.EXTRA_MUTE_STREAM_VALUE";

    /**
     * @hide The previous mute associated with the stream for the mute changed intent.
     */
    public static final String EXTRA_PREV_MUTE_STREAM_VALUE =
        "android.media.EXTRA_PREV_MUTE_STREAM_VALUE";
 ...
 */
    ...
    public static final int[] DEFAULT_STREAM_VOLUME = new int[] {
        ...
        60, // STREAM_MUSIC
        60,  // STREAM_ALARM
        60,  // STREAM_NOTIFICATION
        60,  // STREAM_BLUETOOTH_SCO
        60,  // STREAM_SYSTEM_ENFORCED
        60, // STREAM_DTMF
        60  // STREAM_TTS
    };
    ...
    public void handleKeyDown(KeyEvent event, int stream) {
        if (stream == USE_DEFAULT_STREAM_TYPE)
			stream = STREAM_MUSIC;
        ...
        switch (keyCode) {
			...
			//[[gaodw@tcl.com. patch from ms628. end]]
                 ...
                 */
                // if current state of stream is mute, unmute it
                if (isStreamMute(stream)) {
                    setStreamMute(stream, false);
                } else {
                //int flags = FLAG_SHOW_UI | FLAG_VIBRATE;
 				int flags = FLAG_SHOW_UI; //FLAG_SHOW_UI | FLAG_VIBRATE; //[[gaodw@tcl.com. patch from ms628. end]]
                ...
                if (mUseMasterVolume) {
				  ...
				  //[[gaodw@tcl.com. patch from ms628. begin]]
					boolean bVolumeUp = (
							...
							//||
                            //keyCode == KeyEvent.KEYCODE_TCL_SWING_R1 ||
                            //keyCode == KeyEvent.KEYCODE_TCL_SWING_R2 ||
                            //keyCode == KeyEvent.KEYCODE_TCL_SWING_R3 ||
						//keyCode == KeyEvent.KEYCODE_TCL_SWING_R4
						)
								? true
								: false;
					if(repeatCount < 10) {
						adjustMasterVolume(bVolumeUp ? ADJUST_RAISE : ADJUST_LOWER, flags);
					} else if(repeatCount >= 10 && event.getRepeatCount() < 20) {
						for(int i = 0; i < 2; i++) {
							adjustMasterVolume(bVolumeUp ? ADJUST_RAISE : ADJUST_LOWER, flags);
						}
		...
		//}} //[[gaodw@tcl.com. patch from ms628. end]]
                ...
                break;
//modify by tank@tcl.com for LONG_VOLUME_MUTE
//modify  for volume mute key change to up active and long press start screen shot by yjw 20140829 start
            case KeyEvent.KEYCODE_VOLUME_MUTE:
//add by tank@tcl.com
/*							//qiyx mod 150918
                                if(event.getRepeatCount() < REPEAT_TIME){
                                        isFirstRepeatFlag = true;
                                }
                                else if(event.getRepeatCount() == REPEAT_TIME){

                                  if(isFirstRepeatFlag){
                                          Intent i = new Intent("com.tcl.action.screenshot");
                                          Log.d(TAG, "send broadcast com.tcl.action.screenshot ");
                   ...
                   case KeyEvent.KEYCODE_VOLUME_MUTE:
                        ...
                        if (event.getRepeatCount() == 0) {
                            if (mUseMasterVolume) {
                                ...
                                isScreenshotKey = false ;
                break;
                //modify  for volume mute key change to up active and long press start screen shot by yjw 20140829 end
//end tank@tcl.com
        }
    }
