...
public class AudioService extends IAudioService.Stub {
    ...
    */
    private static final int[] STEAM_VOLUME_OPS = new int[] {
        AppOpsManager.OP_AUDIO_MEDIA_VOLUME,            // STREAM_VOICE_CALL
        AppOpsManager.OP_AUDIO_MEDIA_VOLUME,            // STREAM_SYSTEM
        AppOpsManager.OP_AUDIO_MEDIA_VOLUME,             // STREAM_RING
        AppOpsManager.OP_AUDIO_MEDIA_VOLUME,            // STREAM_MUSIC
        AppOpsManager.OP_AUDIO_MEDIA_VOLUME,            // STREAM_ALARM
        AppOpsManager.OP_AUDIO_MEDIA_VOLUME,     // STREAM_NOTIFICATION
        AppOpsManager.OP_AUDIO_MEDIA_VOLUME,        // STREAM_BLUETOOTH_SCO
        AppOpsManager.OP_AUDIO_MEDIA_VOLUME,            // STREAM_SYSTEM_ENFORCED
        AppOpsManager.OP_AUDIO_MEDIA_VOLUME,            // STREAM_DTMF
        AppOpsManager.OP_AUDIO_MEDIA_VOLUME,            // STREAM_TTS
    };
    ...
    public AudioService(Context context) {
	   ...
	   //[[modify by gaodw@tcl.com end]]
	   	//[[gaodw@tcl.com patch from ms628.
       	int volume = AudioInvoke.getVolume(mContext);
	   	int tmp =  Math.round(Settings.System.getFloatForUser(
			mContentResolver,
			Settings.System.VOLUME_MASTER,
			-1.0f, 
			UserHandle.USER_CURRENT) * MAX_MASTER_VOLUME);
		Log.i(TAG,"Settings master volume:" + tmp + " ,eeprom master volume:" + volume);
		if(tmp != volume) {
			AudioInvoke.setVolume(volume, mContext);
			AudioSystem.setMasterVolume((float)volume / MAX_MASTER_VOLUME);
			Settings.System.putFloatForUser(
				mContentResolver,
				Settings.System.VOLUME_MASTER,
            	(float)volume / MAX_MASTER_VOLUME,
    ...
    private void readPersistedSettings() {
        ...
        //AudioSystem.setMasterMute(masterMute);//modified by huoph@tcl.com for don't set mute state to the previous state 
    ...
    // UI update and Broadcast Intent - Mute
    private void sendMuteUpdate(int streamType, boolean oldState, boolean state) {
        Intent intent = new Intent(AudioManager.MUTE_CHANGED_ACTION);
        intent.putExtra(AudioManager.EXTRA_MUTE_STREAM_TYPE, streamType);
        intent.putExtra(AudioManager.EXTRA_MUTE_STREAM_VALUE, state);
        intent.putExtra(AudioManager.EXTRA_PREV_MUTE_STREAM_VALUE, oldState);
    ...
    public void setStreamMute(int streamType, boolean state, IBinder cb) {
        ...
        sendMuteUpdate(streamType, isStreamMute(streamType), state);
		AudioSystem.setStreamMute(streamType, state);
        int numStreamTypes = AudioSystem.getNumStreamTypes();
		for (int i = 0; i < numStreamTypes; i++) {
			if ((i != streamType) && (STREAM_VOLUME_ALIAS[i] == streamType)) {
            	sendMuteUpdate(i, isStreamMute(i), state);
				AudioSystem.setStreamMute(i, state);
			}
        }
    }
    ...
    public boolean isStreamMute(int streamType) {
		...
		return AudioSystem.getStreamMute(streamType);
    }
	...
	Log.d(TAG,"call setMasterMute start,state:"+state);
	//[[gaodw@tcl.com patch from ms628. begin]]
	   if (state != isMasterMute()) {
	    Log.d(TAG,"call AudioInvoke.setMute start");
            AudioInvoke.setMute(state, mContext);
         
         
           //  AudioSystem.setMasterMute(state);
	//[[gaodw@tcl.com patch from ms628. end]]
	...
	Log.d(TAG,"enter isMasterMute ");
		//[[gaodw@tcl.com. ptach from ms628. begin]]
        //return AudioSystem.getMasterMute();
        return AudioInvoke.isMute(mContext);
		//[[gaodw@tcl.com. ptach from ms628. end]]
    ...
    public int getMasterVolume() {
       // if (isMasterMute()) return 0; marked by huoph@tcl.com for get real volume in mute mode
	...
	Log.i(TAG, "Entry doSetMasterVolume ======davik=======");
		
               // Volume panel for TV style
        //if (AudioSystem.getMasterMute()) {
         /*
		if (isMasterMute()) {
          	AudioInvoke.setMute(false, mContext);
            //AudioSystem.setMasterMute(false);
            // Post a persist master volume msg
            sendMsg(mAudioHandler, MSG_PERSIST_MASTER_VOLUME_MUTE, SENDMSG_REPLACE, false ? 1 : 0, 0, null, PERSIST_DELAY);
		...
		//[[gaodw@tcl.com. patch from ms628. begin]]
		//zhangyingang for Audio adapter begin
		AudioInvoke.setVolume(newVolume, mContext);	

		//add by huoph@tcl.com for cancel mute if increase volume
		boolean increaseVolume = (newVolume > oldVolume) ? true:false;
		if(increaseVolume && isMasterMute()){
                	AudioInvoke.setMute(false, mContext);
          		//AudioSystem.setMasterMute(false);
            		// Post a persist master volume msg
            		sendMsg(mAudioHandler, MSG_PERSIST_MASTER_VOLUME_MUTE, SENDMSG_REPLACE, false ? 1 : 0, 0, null, PERSIST_DELAY);
		...
		Log.i(TAG, "newVolume:" + newVolume + ", oldVolume:" + oldVolume);
		//zhangyingang for Audio adapter end
		//[[gaodw@tcl.com. patch from ms628. end]]
    ...
    /* no-SystemUI case: start */
	/*
	 * !!NOTES: this RingtonePlayer is a dummy player, which is used to 
	 * pass CTS test due to the removal of SystemUI.
	 * Do not use it because RingtonePlayer class should be customized for specific purpose!!
	 */
	private IRingtonePlayer mDummyRingtonePlayer = new IRingtonePlayer.Stub() {
		private static final String TAG = "DummyRingtonePlayer";

		public void play(IBinder token, Uri uri, int streamType)
		{
			Log.e(TAG, "play(): uri: " + uri + ", streamType: " + streamType);
			return;
		}
		...
		{
			Log.e(TAG, "isPlaying: invoked");
			return false;
		}

		public void playAsync(Uri uri,UserHandle user, boolean looping, int streamType)
		{
			Log.e(TAG, "playAsync(): uri: " + uri + ", looping: " + looping + ", streamType: " + streamType);
