...
public class MediaScanner
{
	...
	//Added by zhaojun for mediascan parameters define
    private static boolean mDisableScan = false;
    private static boolean mBulkInsertEnable = true;
    private static int mBulkInsertBufferSize = 500;
    private static int mBulkInsertAudioBufferSize = 50;
    private static int mScanFileCount =1000;
    private static int mScanCount =50000;
	//Add end
    ...
    public MediaScanner(Context c) {
		...
		//Added by zhaojun for set scan parameters
		String prop_bulk_enable = null;
		String prop_bulk_buffer = null;
		String count = null;
	
		prop_bulk_enable = SystemProperties.get("media.scan.bulkinsertenable");
		if((null != prop_bulk_enable) && ("false".equals(prop_bulk_enable))){
			Log.d(TAG,"#### mediascan BulkInsert disable ... ");
				mBulkInsertEnable = false;
		}else{
			Log.d(TAG,"#### mediascan BulkInsert Enable ... ");
				mBulkInsertEnable = true;
		
			prop_bulk_buffer = SystemProperties.get("media.scan.bulkinsertbuffersize");
			if(null != prop_bulk_buffer && (!"".equals(prop_bulk_buffer))){
					mBulkInsertBufferSize = Integer.parseInt(prop_bulk_buffer);
				Log.d(TAG,"#### mediascan BulkInsert buffer size " + mBulkInsertBufferSize);
			}else{
					mBulkInsertBufferSize = 500;
				Log.d(TAG,"#### mediascan  Default BulkInsert buffer size " + mBulkInsertBufferSize);
			}
			prop_bulk_buffer = SystemProperties.get("media.scan.bulkaudiobuffersize");
			if(null != prop_bulk_buffer && (!"".equals(prop_bulk_buffer))){
					mBulkInsertAudioBufferSize = Integer.parseInt(prop_bulk_buffer);
				Log.d(TAG,"#### mediascan BulkInsert Audio buffer size " + mBulkInsertAudioBufferSize);
			}else{
					mBulkInsertAudioBufferSize = 50;
				Log.d(TAG,"#### mediascan  Default BulkInsert Audio buffer size " + mBulkInsertAudioBufferSize);
			}
		}
	
		count = SystemProperties.get("media.scan.count");
		if(null != count && (!"".equals(count))){
				mScanCount = Integer.parseInt(count);
			Log.d(TAG,"#### mediascan Scan max Count " + mScanCount);
		}else{
				mScanCount = 50000;
			Log.d(TAG,"#### mediascan  Default scan max Count " + mScanCount);
		}
	
		count = SystemProperties.get("media.scan.filecount");
		if(null != count && (!"".equals(count))){
				mScanFileCount = Integer.parseInt(count);
			Log.d(TAG,"#### mediascan ScanFile max Count " + mScanFileCount);
		}else{
				mScanFileCount = 1000;
			Log.d(TAG,"#### mediascan  Default scanFile max Count " + mScanFileCount);
		}
		//Add end
    ...
    private class MyMediaScannerClient implements MediaScannerClient {
        ...
        public void scanFile(String path, long lastModified, long fileSize,
            ...
            // Log.v(TAG, "#####scanFile: "+path + "mScanCount :" + mScanCount + "mScanFileCount :" + mScanFileCount);
			//Added by zhaojun for limit max scan count &max scan file nums
	    	if(mScanCount < 1 || mScanFileCount < 1){
            	Log.v(TAG, "##### scan Will stop mediascan  scanFile: "+path + "mScanCount :" + mScanCount + "mScanFileCount :" + mScanFileCount);
				mDisableScan = true;
                disableScan();
	    	}else{
	    		mScanCount = mScanCount - 1;
            	mFileType = 0;
	    		...
	    		if((isaudio || isvideo || isimage)){
	    			...
	    			mScanFileCount = mScanFileCount - 1;
             		...
             		Log.v(TAG, " not media file ,will not scanFile: "+path);
				}
			}
			//Add end           
            //doScanFile(path, null, lastModified, fileSize, isDirectory, false, noMedia);
        ...
        public Uri doScanFile(String path, String mimeType, long lastModified,
            ...
            //Added by zhaojun for mediascan start/stop
            if(mDisableScan){
				disableScan();
			}
			//Add end
            ...
            try {
                ...
                if (entry != null && (entry.mLastModifiedChanged || scanAlways)) {
                    if (noMedia) {
						...
						//Changed by zhaojun for mediascan close processFile(audio & meida)
						if ((isaudio || isvideo) && scanAlways) {
							Log.d(TAG, "########### 2 scanFile: "+path + scanAlways);
							processFile(path, mimeType, this);
						}
						
						//Changed by zhaojun for mediascan close processImageFile
						if (isimage && scanAlways) {
							Log.d(TAG, "########### 3 scanFile: "+path + scanAlways);
...
//            Log.v(TAG, "scanFile: " + path + " took " + (t2-t1));
        ...
        private Uri endFile(FileEntry entry, boolean ringtones, boolean notifications,
			...
			//Changed by zhaojun for video insert database one by one
	...
	//Added by zhaojun for cleanning datebase when plug out u disk
    public void updatescan(String updatapath, String volumeName)  {
        Log.w(TAG, "-----updatapath is :-------"+updatapath);
        Log.w(TAG, "-----volumeName is :-------"+volumeName);
        initialize(volumeName);
        Cursor c = null;
        String where = null;
        String[] selectionArgs = null;

        Log.d(TAG, "-------mAudioUri is :"+mAudioUri+"--------------");

        // Read existing files from the audio table
        if (updatapath != null) {
            where = MediaStore.Audio.Media.DATA + " like " + "'" + updatapath + "/%'";
        } else {
            where = null;
        }
        Log.w(TAG, "-----mAudioUri where is :-----"+where);

        // Delete the list of files from the content provider
        try{
            if (where != null) {
                Log.w(TAG, "-----delete is :-----"+where);
                mMediaProvider.delete(mPackageName, mAudioUri, where, null);
            }    
...
//Video
        Log.d(TAG, "-------mVideoUri is :"+mVideoUri+"--------------");

        // Read existing files from the audio table
        if (updatapath != null) {
            where = MediaStore.Video.Media.DATA + " like " + "'" + updatapath + "/%'";
        } else {
            where = null;
        }
        Log.w(TAG, "-----mVideoUri where is :-----"+where);

        // Delete the list of files from the content provider
        try{
            if (where != null) {
                Log.w(TAG, "-----delete is :-----"+where);
                mMediaProvider.delete(mPackageName, mVideoUri, where, null);
            }    
        } catch (SQLException e) {
            // this might happen if the SD card is removed while the media scanner is running
            Log.e(TAG, "SQLException in MediaScanner.scan()", e);
...
//Images
        Log.d(TAG, "-------mImagesUri is :"+mImagesUri+"--------------");

        // Read existing files from the audio table
        if (updatapath != null) {
            where = MediaStore.Images.Media.DATA + " like " + "'" + updatapath + "/%'";
        } else {
            where = null;
        }
        Log.w(TAG, "-----mImagesUri where is :-----"+where);

        // Delete the list of files from the content provider
        try{
            if (where != null) {
                Log.w(TAG, "-----delete is :-----"+where);
                mMediaProvider.delete(mPackageName, mImagesUri, where, null);
            }    
        } catch (SQLException e) {
            // this might happen if the SD card is removed while the media scanner is running
            Log.e(TAG, "SQLException in MediaScanner.scan()", e);
...
//Add end
	...
	public static void stopMediaScanner() {
		mDisableScan = true;
		Log.d(TAG, "-----set mDisableScan = true;----- ");
	}

	public static void startMediaScanner() {
		mDisableScan = false;
		Log.d(TAG, "-----set mDisableScan = false;----- ");
	}
	//Add end
    ...
    public void scanDirectories(String[] directories, String volumeName) {
        try {
			...
			//Changed by zhaojun for mediascan bulkinsert open/close
            if (mBulkInsertEnable) {
                // create MediaInserter for bulk inserts
                mMediaInserter = new MediaInserter(mMediaProvider, mPackageName, mBulkInsertBufferSize, mBulkInsertAudioBufferSize);
            }
			//Changed end
