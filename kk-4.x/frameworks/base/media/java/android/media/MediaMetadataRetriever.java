...
public class MediaMetadataRetriever
{
    ...
    public void setDataSource(String path) throws IllegalArgumentException {
        ...
        if(null!=path)
        {
            if(playerType!=-1)//Apeend headers 
            {
                /*
                if(playerType==0)
                {
                    path="mtk://player_type=0"+path;
                }
                else if(playerType==1)
                {
                    path="mtk://player_type=1"+path;
                }
                else if(playerType==2)
                {
                    path="mtk://player_type=2"+path;
                }
                else if(playerType==3)
                {
                    path="mtk://player_type=3"+path;
                }
                else if(playerType==4)
                {
                    path="mtk://player_type=4"+path;
                }
                else if(playerType==5)
                {
                    path="mtk://player_type=5"+path;
                }
                else */ 
                if(playerType==6)//CMPB_PLAYER 
                {
                    path="mtk://player_type=6"+path;
                }
                else if(playerType==7)//MTK_STREAM_PLAYER
                {
                    path="mtk://player_type=7"+path;
                }
            }
        }
        //MTK END
    ...
    //MTK Begin
    /**
     * TRUE: Stagefirght to get thumbnial
     * FALSE: CMPB get thumbnial
     * Default: CMPB to get thumbnial
     */
    public void setThumbnail(boolean isThumbnail) {
        _setThumbnail(isThumbnail);
    }

    private native void _setThumbnail(boolean isThumbnail);

    
    /**
      * API to Set The SVCTX Path of Thumbnail.
      * Please call this API after setDataSource() and before prepare()or prepareAsync().
      * If you do not call setRetriveverSvctxPath(), the default SVCTX path of Player is Main SVCTX.
      * @param p_svctx_path should be following one: "main_svctx" or "sub_svctx" or "THRD_USB".
      * 
      * @hide
      */
    public void setRetriveverSvctxPath(String p_svctx_path) {
        native_setRetriveverSvctxPath(p_svctx_path);
    }

    private native void native_setRetriveverSvctxPath(String p_svctx_path);
    ...
    /**
     * This key retrieves the file director.
     */
    public static final int METADATA_KEY_DIRECTOR = 30;
    /**
     * This key retrieves the file copy right.
     */
    public static final int METADATA_KEY_COPYRIGHT = 31;
}
