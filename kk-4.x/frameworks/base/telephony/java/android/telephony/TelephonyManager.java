...
public class TelephonyManager {
    ...
    private ITelephony getITelephony() {
        if (SystemProperties.get("ro.config.low_ram").equals("true"))
        {
            ...
            //MTK modify, for fix cts fail cases of android.permission
            if(iTelephony == null)
            {   
                Intent in = new Intent(); 
                in.setClassName("com.android.phone", "com.android.phone.BluetoothPhoneService");
                mContext.startService(in);

                long maxSleepTime = 2000;
                long everySleepTime = 200;
                while(iTelephony == null && maxSleepTime > 0)
                {
                    SystemClock.sleep(everySleepTime);
                    maxSleepTime -= everySleepTime;
                    iTelephony = ITelephony.Stub.asInterface(ServiceManager.getService(Context.TELEPHONY_SERVICE));
                    android.util.Log.d(TAG, "wait service ready for " + (2000 - maxSleepTime));
                }
            }
            return iTelephony;
        }
