...
public class WifiStateMachine extends StateMachine {
    ...
    public boolean mScanFromApp = false;
   ...
   /* Set the frequency band */
    ...
    public void startScan(int callingUid, WorkSource workSource) {
    	log("sendMessage cmd_start_scan 222");                            	
        //mScanFromApp = true;
    ...
    private void startScanNative(int type) {
    	log("mWifiP2pManager.getMtkWfdSupport():" + mWifiP2pManager.getMtkWfdSupport());
                    if (null != mWifiP2pManager && !mWifiP2pManager.getMtkWfdSupport()) {
    ...
    private void setScanResults() {
        ...
        WifiSsid wifiSsid = null;
        String scanResults;
        String tmpResults = null;
        StringBuffer scanResultsBuf = new StringBuffer();
        int sid = 0;
	int added_scan_result_count = 0;

		loge("begin to append scan results");
        while (true) {
		if(added_scan_result_count++ > 5) {
			loge("too many to append scan results");	
			break;
		}
	
        	try {//zengyongying: fix out of memory
	            tmpResults = mWifiNative.scanResults(sid);
	            if (TextUtils.isEmpty(tmpResults)) break;
	            loge("to append scan results length :" + tmpResults.length());
	            //loge("to append scanResultsBuf:" + scanResultsBuf);
	            if(scanResultsBuf == null){
	            	loge("to append scanResultsBuf null");
	            	return;
	            }
	            scanResultsBuf.append(tmpResults);
	            scanResultsBuf.append("\n");
        	}catch (Exception e) {
            	loge("Failed to append scan results:" + e);
            	return;
        	}
    ...
    class InitialState extends State {
        ...
        public boolean processMessage(Message message) {
            switch (message.what) {
                case CMD_START_SUPPLICANT:
                    //wait until the driver loaded success                    
                    //waitForDriverReady();//zengyongying: don't wait driver ready befor load driver
            ...
            switch (message.what) {
              ...
              case DhcpStateMachine.CMD_POST_DHCP_ACTION:
                ...
                case CMD_START_SCAN:
				    ...
				    noteScanStart(message.arg1, (WorkSource) message.obj);
		                    startScanNative(WifiNative.SCAN_WITHOUT_CONNECTION_SETUP);
				    mScanFromApp = false;   // add by zengyy     
		 		    break;
        ...
        public boolean processMessage(Message message) {
            switch (message.what) {
               case WifiWatchdogStateMachine.POOR_LINK_DETECTED:
                    ...
                    transitionTo(mVerifyingLinkState);
                    break;
              case CMD_START_SCAN://add by zengyy. to stop scan if not from app
				   //if(mScanFromApp == false) {
				   	loge("do not scan if has connected and not from app, mScanFromApp = " + mScanFromApp);
				   	return HANDLED;  
				   //}
	      	  default:
                    return NOT_HANDLED;
            ...
            switch (message.what) {
                case CMD_NO_NETWORKS_PERIODIC_SCAN:
                    ...
                    if (message.arg1 == mPeriodicScanToken &&
						...
						log("sendMessage cmd_start_scan 111");                            	
    ...
    //        return 2; //current is wlan0 (Ralink)
            return WifiNative.wifi0Check();
        }
     public int wifiIsDongleIn() {
        log("[WifiStateMachine][wifiIsDongleIn]:wifiIsDongleIn.");
        return WifiNative.wifiIsDongleIn();
    }
      public int wifiIsDongleOut() {
        log("[WifiStateMachine][wifiIsDongleOut]:wifiIsDongleOut.");
        return WifiNative.wifiIsDongleOut();
    }
        public boolean loadDriver(){
        log("[WifiStateMachine][loadDriver]:loadDriver.");
         return WifiNative.loadDriver();
    }
        
    public boolean startSupplicant(boolean p2pSupported){
        log("[WifiStateMachine][startSupplicant]:startSupplicant.");
         return WifiNative.startSupplicant(p2pSupported);
        }

    public boolean connectToSupplicant(){
          log("[WifiStateMachine][connectToSupplicant]:connectToSupplicant.");
             return mWifiNative.connectToSupplicant();
            }
    public boolean scan(){
        log("[WifiStateMachine][scan]:scan.");
        return mWifiNative.scan(WifiNative.SCAN_WITH_CONNECTION_SETUP);
            }
    public String scanResults(){
        log("[WifiStateMachine][scanResults]:scanResults.");
        return mWifiNative.scanResults();
    }
    ...
    public void waitForDriverReady(){
        ...
        while(true){
            try{
                if(checkTimes > 5) break; // 30s did not loaded driver, breakout 
                if(!WifiNative.isDriverLoaded()){                               
                    if (DBG) log("WifiDriver has not loaded successly, wait 5s here, trytimes="+checkTimes); 
                    checkTimes ++;
                    Thread.sleep(5000); // sleep 5s
                }else{                               
                    if (DBG) log("WifiDriver loaded success"); 
                    break;
                }
            }catch(InterruptedException e)
            {
                // ignore
                if (DBG) log("exception " + e.getStackTrace()); 
                break;
            }
        }
    }
