...
public class WifiP2pService extends IWifiP2pManager.Stub {
	...
	public static final int PEER_CONNECTION_DELAY	        =	1000;
...
private String PeerGOip=null;
private boolean mWfdSupported;
private boolean mJoinGroup = false;
private String mInterfaceName = "";
private String mAddress = "";
private boolean mReStartGO;


    private boolean mTempoarilyDisconnectedWifi = false;

	private String confDirectory = "/data/misc/wifi/p2p_supplicant.conf";
	...
	/**
	 * Mod the Group ssid or/and password that Create by User
	 * This API is used by WifiP2pManager
	 * @return : no
	 * add by cuix@tcl.com 2014.5.8
	 */
	public void ConfigGroupInfo(String mssid, String passphase) {
		boolean setsuc = false;
		if(mssid == null && passphase == null) {
			Slog.d(TAG, "Mod Group info, para null");
			return;
		} else if(mssid != null) {
			Slog.d(TAG, "---Mod Group info, ssid: " + mssid);
			setsuc = mWifiNative.setNetworkVariable(
								0,
								WifiConfiguration.ssidVarName,
								mssid);
			if(!setsuc) {
				Slog.d(TAG, "---set ssid error");
			}
		} else if(passphase != null) {
			Slog.d(TAG, "---Mod Group info, pw: " + passphase);
			setsuc = mWifiNative.setNetworkVariable(
								0,
								WifiConfiguration.pskVarName,
								passphase);
			if(!setsuc) {
				Slog.d(TAG, "---set pw error");
			}
		}
		Slog.d(TAG, "---saveConfig");

		if(1 == WifiNative.modify_psk(confDirectory, passphase)) {
			Slog.d(TAG, "---saveConfig S");
		} else {
			Slog.d(TAG, "---saveConfig fail");
		}
	}
    ...
    public String getMtkPeerIpAddress(String mac,WifiP2pGroup mWifiP2pGroup){
		enforceAccessPermission();
        enforceChangePermission();
        if (mWifiP2pGroup == null) {
            mWifiP2pGroup = mP2pStateMachine.mGroup;
        }
        Slog.d(TAG, "getPeerIpAddress,mac address is  " + mac);
		//judge whether ourself is GO,if it returns true means ourself is GO  
		if(mWifiP2pGroup !=null && mWifiP2pGroup.isGroupOwner()){
			Slog.d(TAG, "p2pGetPeerIpFromMac, ip is  "+ WifiNative.p2pGetPeerIpFromMac(mac));
			return (WifiNative.p2pGetPeerIpFromMac(mac));
		}else{
		  Slog.d(TAG, "Peer is GroupOwner and ourself is gc, peer ip is " + PeerGOip);
          return PeerGOip;
		}
    }
	...
	public void setMiracastEnable(boolean enable) {
        enforceConnectivityInternalPermission();
        mP2pStateMachine.setMiracastEnable(enable);
    }
    
    public void setMtkWfdSupport(boolean iswfd){
    	enforceAccessPermission();
        enforceChangePermission();
        Slog.d(TAG, "setWfdSupport:" +iswfd);
    	mWfdSupported = iswfd;
    }
    public void setMtkP2pListen(){
    	enforceAccessPermission();
        enforceChangePermission();
        Slog.d(TAG, "p2pListen");
        mP2pStateMachine.setMtkP2pListen();
    }
     public boolean getMtkWfdSupport() {
        return mWfdSupported;
    }
    
     public void p2pGroupAdd(int netid) {
	 	  Slog.d(TAG, "09207----p2pGroupAdd:"+" netid="+netid+" mReStartGO="+mReStartGO);
		  mP2pStateMachine.p2pGroupAdd(netid);
    }
    
     public void reStartGO(boolean restart) {
	 	Slog.d(TAG, "09207----reStartGO:"+"set restart="+restart);
        mReStartGO = restart;
     }
	
	public boolean mtkP2pGroupAdd(){
			enforceAccessPermission();
        enforceChangePermission();
      //  mPersistGroup = true;
        boolean groupadd = true;//WifiNative.p2pGroupAdd(false);
				 if (groupadd) {
						 Slog.d(TAG, "WifiNative.p2pGroupAdd() : " + groupadd); 
          } else {
							Slog.d(TAG, "WifiNative.p2pGroupAdd() : " + groupadd); 
		   }
		   return groupadd;
    ...
    private class P2pStateMachine extends StateMachine {
      ...
      public void setMtkP2pListen(){
        ...
        public void p2pGroupAdd(int netid) {
			Slog.d(TAG, "09207----p2pGroupAdd:"+" netid="+netid+" mReStartGO="+mReStartGO);
			if(mReStartGO)
             mWifiNative.p2pGroupAdd(netid);
        }
        
       public void setMiracastEnable(boolean enable) {
			   mWifiNative.setWfdEnable(enable);
		   }
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiMonitor.P2P_DEVICE_FOUND_EVENT:
                    ...
                    logd(" P2P_DEVICE_FOUND_EVENT"+device.deviceAddress+"mThisDevice.deviceAddress:"+mThisDevice.deviceAddress);
                ...
                case WifiMonitor.P2P_DEVICE_LOST_EVENT:
                    ...
                    logd(" P2P_DEVICE_LOST_EVENT"+device.deviceAddress);
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiMonitor.P2P_GO_NEGOTIATION_REQUEST_EVENT:
					removeMessages(PEER_CONNECTION_DELAY);
                    ...
                    mJoinGroup = false;
                    mAddress = mSavedPeerConfig.deviceAddress;
                    sendP2pGoNegotiationRequestBroadcast(mSavedPeerConfig);
                  ...
                  //  }
                    break;
                case WifiMonitor.P2P_WFD_DEV_GO_NEGOTIATION_REQUEST_EVENT:
                	WifiP2pWfdPort wfdport = (WifiP2pWfdPort) message.obj;
                	 logd(getName() + "P2P_WFD_DEV_GO_NEGOTIATION_REQUEST_EVENT wfdport:"+wfdport);
                	 sendP2pAsGOPortBroadcast(wfdport.mCtrlPort);
                	break;
                case WifiMonitor.P2P_INVITATION_RECEIVED_EVENT:
                   ...
                   //         mSavedPeerConfig)) {
                    	sendP2pGoNegotiationRequestBroadcast(mSavedPeerConfig);
                    	  if (DBG) logd(getName() + "auto accept P2P_INVITATION_RECEIVED_EVENT " + mSavedPeerConfig);
...
//                    }
                ...
                case WifiMonitor.P2P_PROV_DISC_SHOW_PIN_EVENT:
                	logd(getName() + "We let the supplicant handle the provision discovery response"+message.toString());
                    //We let the supplicant handle the provision discovery response
                    //and wait instead for the GO_NEGOTIATION_REQUEST_EVENT.
                    //Handling provision discovery and issuing a p2p_connect before
                    //group negotiation comes through causes issues
                     Message msg = Message.obtain();
                     msg.what = PEER_CONNECTION_DELAY;
                     msg.arg1 = message.what;
					 msg.obj = message.obj;
                    sendMessageDelayed(msg,200);
                    break;
				case PEER_CONNECTION_DELAY:
					logd(getName() + "PEER_CONNECTION_DELAY");
					WifiP2pProvDiscEvent provDisc = (WifiP2pProvDiscEvent) message.obj;
					mSavedPeerConfig = new WifiP2pConfig();
					mSavedPeerConfig.deviceAddress = provDisc.device.deviceAddress;
					if (message.arg1 == WifiMonitor.P2P_PROV_DISC_ENTER_PIN_EVENT) {
						mSavedPeerConfig.wps.setup = WpsInfo.KEYPAD;
					} else if (message.arg1 == WifiMonitor.P2P_PROV_DISC_SHOW_PIN_EVENT) {
						mSavedPeerConfig.wps.setup = WpsInfo.DISPLAY;
					...
					if (isConfigInvalid(mSavedPeerConfig)) {
                        loge("Dropping invitation request " + mSavedPeerConfig);
                        break;
                    }  
    ...
    class GroupNegotiationState extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiMonitor.P2P_GROUP_STARTED_EVENT:
                    mGroup = (WifiP2pGroup) message.obj;
                    if (DBG) logd(getName() + " group started"+mGroup+" mJoinGroup:"+mJoinGroup);
                    if (!mJoinGroup) {
                       mInterfaceName = mGroup.getInterface();
                       mWifiNative.connectWfdToSupplicant(mInterfaceName);
                     }
                    sendP2pGroupStartedBroadcast(mGroup);
                    if (mGroup.getNetworkId() == WifiP2pGroup.PERSISTENT_NET_ID) {
                        /*
                    ...
                    if (mGroup.isGroupOwner()) {
                    	...
                    	sendP2pAsGcConnectSuccessBroadcast(mGroup);
                        mWifiNative.setP2pGroupIdle(mGroup.getInterface(), GROUP_IDLE_TIME_S);
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiMonitor.AP_STA_CONNECTED_EVENT:
                    ...
                    sendP2pAsGOConnectSuccessBroadcast(device.interfaceAddress);
                    sendP2pConnectionChangedBroadcast();
                    if (mJoinGroup  && !mAutonomousGroup) {
                        logd(getName() + " ap sta reconnected mInterfaceName:"+mInterfaceName+"  mAddress:"+mAddress);
                        mWifiNative.setP2premove(mInterfaceName,mAddress);
                        mJoinGroup = false;
                    }
                    break;
                case WifiMonitor.AP_STA_DISCONNECTED_EVENT:
                    ...
                    if (deviceAddress != null) {
                        ...
                        if (mGroup.removeClient(deviceAddress)) {
                            if (DBG) logd("Removed client " + deviceAddress+" mReStartGO:"+mReStartGO+" mAutonomousGroup:"+mAutonomousGroup);
                            if (!mAutonomousGroup && mGroup.isClientListEmpty() && !mReStartGO) {
                ...
                case DhcpStateMachine.CMD_POST_DHCP_ACTION:
                    ...
                    if (message.arg1 == DhcpStateMachine.DHCP_SUCCESS &&
                      ...
                      //  setWifiP2pInfoOnGroupFormation(dhcpInfo.serverAddress);
                       ...
                       // if (DBG) logd("DhcpInfo: " + dhcpInfo);
                       // setWifiP2pInfoOnGroupFormation(dhcpInfo.serverAddress);
                        PeerGOip=dhcpResults.serverAddress.getHostAddress();
                        logd("[WifiP2pService]:when we are gc ,the peer GO's Ip address is " + PeerGOip);
                        sendP2pConnectionChangedBroadcast();
                ...
                case WifiMonitor.P2P_GROUP_REMOVED_EVENT:
                    ...
                    handleGroupRemoved();
					 ...
					 WifiP2pGroup mP2pGroup = (WifiP2pGroup) message.obj;
					 logd("09207----- group removed then create go isGroupAdd:"+mP2pGroup.isGroupAdd());
					 if(mP2pGroup.isGroupAdd()){
						p2pGroupAdd(0);
					 }
                    transitionTo(mInactiveState);
                    break;
                ...
                case WifiMonitor.P2P_PROV_DISC_ENTER_PIN_EVENT:
                case WifiMonitor.P2P_PROV_DISC_SHOW_PIN_EVENT:
                    Slog.d(TAG, "P2P_PROV_DISC_PBC_REQ_EVENT");
                    if (isWifiP2pConnect() || mAutonomousGroup) {
                        mJoinGroup = true;
                    WifiP2pProvDiscEvent provDisc = (WifiP2pProvDiscEvent) message.obj;
                    Slog.d(TAG, "P2P_PROV_DISC_PBC_REQ_EVENT mSavedProvDiscDevice"+mSavedProvDiscDevice);
                    mSavedPeerConfig = new WifiP2pConfig();
                    mSavedPeerConfig.deviceAddress = provDisc.device.deviceAddress;
                  ...
                  //  if (!sendConnectNoticeToApp(mSavedProvDiscDevice, mSavedPeerConfig)) {
                    	 Slog.e(TAG, "P2P_PROV_DISC_PBC_REQ_EVENT, transitionTo mUserAuthorizingJoinState");
                ...
                case WifiMonitor.P2P_WFD_DEV_GO_NEGOTIATION_REQUEST_EVENT:
                	WifiP2pWfdPort wfdport = (WifiP2pWfdPort) message.obj;
                    /////add 0320
                    Slog.d(TAG, "P2P_WFD_DEV_PROV_DISC_REQUEST_EVENT mSavedPeerConfig"+mSavedPeerConfig);
                    sendP2pAsGOPortBroadcast(wfdport.mCtrlPort);
                    ////
                	break;
                case WifiMonitor.P2P_GROUP_STARTED_EVENT:
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case PEER_CONNECTION_USER_ACCEPT:
                    ...
                    mWifiNative.p2pStopFind();
                    if (mSavedPeerConfig.wps.setup == WpsInfo.PBC) {
                        loge("PEER_CONNECTION_USER_ACCEPT mJoinGroup: " + mJoinGroup+"mInterfaceName:"+mInterfaceName);
                        if (mJoinGroup) {
                            mWifiNative.startWpsPbc(mInterfaceName, null);
                        }else {
                            mWifiNative.startWpsPbc(mGroup.getInterface(), null); 
                        }
                    } else {
                        loge("PEER_CONNECTION_USER_ACCEPT startWpsPinKeypad mJoinGroup: " + mJoinGroup+"mInterfaceName:"+mInterfaceName);
                        if (mJoinGroup) {
                            mWifiNative.startWpsPinKeypad(mInterfaceName,
                    ...
                    loge("PEER_CONNECTION_USER_ACCEPT mSavedPeerConfig: " + mSavedPeerConfig);
                   // mWifiNative.p2pConnectWithSecond(mSavedPeerConfig, true);
//                    mSavedPeerConfig = null;
	...
	private void sendP2pGoNegotiationRequestBroadcast(WifiP2pConfig mWifiP2pConfig) {
        final Intent intent = new Intent(WifiP2pManager.MTK_WIFI_P2P_GO_NEGOTIATION_REQUEST_ACTION);
		intent.putExtra(WifiP2pManager.MTK_EXTRA_WIFI_P2P_CONFIG, mWifiP2pConfig);
        mContext.sendBroadcast(intent);
    }
    private void sendP2pGroupRemovedBroadcast(){
        final Intent intent = new Intent(WifiP2pManager.MTK_WIFI_P2P_GROUP_REMOVED_ACTION);
        mContext.sendBroadcast(intent);
	}
	private void sendP2pGroupStartedBroadcast(WifiP2pGroup mWifiP2pGroup){
			final Intent intent = new Intent(WifiP2pManager.MTK_WIFI_P2P_GROUP_STARTED_ACTION);
			intent.putExtra(WifiP2pManager.MTK_EXTRA_WIFI_P2P_GROUP_STARTED, mWifiP2pGroup);
			mContext.sendBroadcast(intent);
		}
		private void sendP2pAsGcConnectSuccessBroadcast(WifiP2pGroup mWifiP2pGroup){
			final Intent intent = new Intent(WifiP2pManager.MTK_WIFI_P2P_AS_GC_CONNECT_SUCCESS_ACTION);
			intent.putExtra(WifiP2pManager.MTK_EXTRA_WIFI_P2P_GROUP, mWifiP2pGroup);
			mContext.sendBroadcast(intent);
		}

	private void sendP2pAsGOConnectSuccessBroadcast(String interfaceAddress){
			final Intent intent = new Intent(WifiP2pManager.MTK_WIFI_P2P_AS_GO_CONNECT_SUCCESS_ACTION);
			intent.putExtra(WifiP2pManager.MTK_EXTRA_WIFI_P2P_INTERFACE_ADDRESS, interfaceAddress);
intent.putExtra("mJoinGroup", mJoinGroup);
			mContext.sendBroadcast(intent);
		}
		
		private void sendP2pAsGOPortBroadcast(String rtspport){
			final Intent intent = new Intent(WifiP2pManager.MTK_WIFI_P2P_AS_GO_PORT_ACTION);
			intent.putExtra(WifiP2pManager.MTK_EXTRA_WIFI_P2P_PORT, rtspport);
			mContext.sendBroadcast(intent);
		}
	//////
    ...
    private void updatePersistentNetworks(boolean reload) {
        ...
        logd("updatePersistentNetworks. listStr=" + listStr);
    ...
    private boolean isWifiP2pConnect() {
    ...
    private boolean isConfigInvalid(WifiP2pConfig config) {
        ...
        if (mPeers.get(config.deviceAddress) == null){
            logd("mPeers.get(config.deviceAddress) == null add device");
            WifiP2pDevice device = new WifiP2pDevice();
...
private void initializeWfdSettings(){
        logd("[WifiP2pService]Start call WifiNative.setWfdEnable(true)");
	    if(mWifiNative.setWfdEnable(true)){
        logd("[WifiP2pService]set Wfd start");
		logd("[WifiP2pService]After call WifiNative.setWfdEnable(true),set wfd parameters");
		mWifiNative.setWfdDeviceType(1);//1:sink;
		mWifiNative.setWfdSourceCoup(0);
		mWifiNative.setWfdSinkCoup(0);
		mWifiNative.setWfdSessionAvail(true);
		mWifiNative.setWfdRtspPort(7236);
		mWifiNative.setWfdMaxThroughput(100);
		//mWifiNative.setP2pGoIntent(2);
		mWifiNative.setWfdContentProtect(true);
		}
	}
    ...
    private void handleGroupRemoved() {
        ...
        mWifiNative.setP2pGroupIdle(mInterfaceName, 0);
        mWifiNative.closeWfdSupplicantConnection();
