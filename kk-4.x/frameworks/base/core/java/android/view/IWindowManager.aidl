...
interface IWindowManager
{
    ...
    void activeHomeKey(boolean bHomeKeyAvailabe);
    boolean isHomeKeyActive();
    //add by dupeng for 667 au 20141203
    void activeKeyByKeycode(int Keycode, boolean isActive);
    //add by tank@tcl.com 
    void activeKeyByKeycodeCnPn(int num, in int[] Keycode, boolean isActive, in ComponentName ComName, String packageName);
    //end tank@tcl.com
    boolean isThisKeyActive(int Keycode);
    //add by dupeng end
    void setHomeKeyInterval(int iInterval);
    boolean isHomeKeyPressIgnore();
    void showSanDUI(boolean bShowSanDUI);
    boolean isShowSanDUI();
    boolean getKeyEventState();
    void setKeyEventState(boolean bKeyEventState);
    //End by TCL
