...
public class Environment {
    ...
    public static class UserEnvironment {
        ...
        private /*final*/ File[] mExternalDirsForVold; // gaodw@tcl , remove final patch from ms628.
        /** External storage dirs, as visible to apps */
        private /*final*/ File[] mExternalDirsForApp; // gaodw@tcl , remove final  patch from ms628.
        /** Primary emulated storage dir for direct access */
        private final File mEmulatedDirForDirect;

	//{{ gaodw@tcl patch from ms628.
	...
	//}}
		...
		//{{gaodw@tcl , we use usb disk  patch from ms628.
			mSdcardStorage = rawExternalStorage;
		//}}
        ...
        public File[] getExternalDirsForVold() {
            buildExternalDirs(false); // lvh@tcl
            return mExternalDirsForVold;
        }

        public File[] getExternalDirsForApp() {
            buildExternalDirs(true); // lvh@tcl
	...
	//{{ gaodw@tcl  patch from ms628.
		private void buildExternalDirs(boolean isApp /* not use */) {
			isApp = false;
			ArrayList<File> externalForVold = Lists.newArrayList();
            ArrayList<File> externalForApp = Lists.newArrayList();

			externalForVold.add(new File(mSdcardStorage));
            externalForApp.add(new File(mSdcardStorage));
			do {
				try {
					IMountService mountService = IMountService.Stub.asInterface(ServiceManager.getService("mount"));
					if(mountService == null) break;

					final StorageVolume[] volumes = mountService.getVolumeList();
					if (volumes == null) break;
					//Log.d(TAG, "lvh@tcl, buildExternalDirs start count = " + volumes.length);
					for (int i = 0; i < volumes.length; i++) {
						externalForVold.add(new File(volumes[i].getPath()));
						externalForApp.add(new File(volumes[i].getPath()));
						//Log.d(TAG, "lvh@tcl, buildExternalDirs " + volumes[i].getPath());
					}
				} catch (Exception e) {
					Log.e(TAG, "(lvh@tcl),couldn't talk to MountService", e);
				}
			} while(false);
			mExternalDirsForVold = externalForVold.toArray(new File[externalForVold.size()]);
            mExternalDirsForApp = externalForApp.toArray(new File[externalForApp.size()]);
