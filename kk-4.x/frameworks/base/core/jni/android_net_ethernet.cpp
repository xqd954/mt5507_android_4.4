...
namespace android {
    ...
    static interface_info_t *find_info_by_index(unsigned int index)
    {
		...
		ALOGD("find_info_by_index : index : %d",index);
        while (info) {
		ALOGD("find_info_by_index : info->i : %d",info->i);
            if (info->i == index)
                break;
            info = info->next;
        }
        ...
        if ((len = recvmsg(nl_socket_poll, &msg, 0)) >= 0) {
            ...
            for (nh = (struct nlmsghdr *) buff; NLMSG_OK (nh, len);
                ...
                if (nh->nlmsg_type == NLMSG_DONE) {
                    ALOGD("Did not find useful eth interface information");
                    goto error;
                }

                if (nh->nlmsg_type == NLMSG_ERROR) {
                    /* Do some error handling. */
                    ALOGD("Read device name failed");
                    goto error;
                }

                ALOGD(" event :%d  found", nh->nlmsg_type);
                einfo = (struct ifinfomsg *)NLMSG_DATA(nh);
                ALOGD("the device flag :%X", einfo->ifi_flags);
            ...
            ALOGD("Done parsing");
            rbuf[4096 - left] = '\0';
            ALOGD("poll state :%s, left:%d", rbuf, left);
        }
    ...
    static void free_int_list()
    {
        ...
        if (total_int) {
            ALOGD("Wrong interface count found");
            total_int = 0;
        }
    }
        ...
        if ((netdir = opendir(SYSFS_CLASS_NET)) != NULL) {
             while ((de = readdir(netdir))) {
                if ((!strcmp(de->d_name, ".")) || (!strcmp(de->d_name, "..")) ||
					...
					(!strcmp(de->d_name, "pan0")) || (!strcmp(de->d_name, "gre0")) ||
					(!strcmp(de->d_name, "tunl0")) || (!strcmp(de->d_name, "sit0")))
                    continue;
                ...
                if ((ifidx = fopen(path, "r")) != NULL) {
                    ...
                    if (fgets(idx,MAX_FGETS_LEN, ifidx) != NULL) {
                        ...
                        ALOGD("Can not read %s", path);
                        continue;
                    }
                } else {
                    ALOGD("Can not open %s for read", path);
                    continue;
                }
                /* make some room! */
                intfinfo = (interface_info_t *)malloc(sizeof(interface_info_t));
                if (intfinfo == NULL) {
                    ALOGD("malloc in netlink_init_interfaces_table");
                    goto error;
                }
                /* copy the interface name (eth0, eth1, ...) */
                intfinfo->name = strndup((char *) de->d_name, SYSFS_PATH_MAX);
                intfinfo->i = index;
                ALOGD("interface %s:%d found", intfinfo->name, intfinfo->i);
                                                        ...
                                                        jobject clazz)
    {
        ...
        if (nl_socket_msg <= 0) {
            ALOGD("Can not create netlink msg socket");
            goto error;
        }
        if (bind(nl_socket_msg, (struct sockaddr *)(&addr_msg),
                 sizeof(struct sockaddr_nl))) {
            ALOGD("Can not bind to netlink msg socket");
            goto error;
        }

        nl_socket_poll = socket(AF_NETLINK,SOCK_RAW,NETLINK_ROUTE);
        if (nl_socket_poll <= 0) {
            ALOGD("Can not create netlink poll socket");
            goto error;
        }
        ...
        if (bind(nl_socket_poll, (struct sockaddr *)(&addr_poll),
                sizeof(struct sockaddr_nl))) {
            ALOGD("Can not bind to netlink poll socket,%s", strerror(errno));

            goto error;
        }

        if ((ret = netlink_init_interfaces_list()) < 0) {
            ALOGD("Can not collect the interface list");
            goto error;
        }
                                                         ...
                                                         jint index)
    {
        int i = 0;
        interface_info_t *info;
        ALOGD("User ask for device name on %d, list:%X, total:%d",
             index, (unsigned int)interfaces, total_int);
        info = interfaces;
        if (total_int != 0 && index <= (total_int - 1)) {
            while (info) {
                if (index == i) {
                    ALOGD("Found: %s", info->name);
    ...
    int register_android_net_ethernet_EthernetManager(JNIEnv* env)
    {
        ...
        ALOGD("Loading ethernet jni class");
