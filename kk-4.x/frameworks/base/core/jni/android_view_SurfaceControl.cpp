...
/**add by 40641*///set surfaceFlinger compose with 3D modle
static void nativeSetComposing3D(
        JNIEnv* env, jobject clazz, jint display, jint compose3DType)
{
    LOGE("JNI Surface_setComposing3D ***********STARS**************");
    int err = SurfaceComposerClient::setComposing3D(display, compose3DType);
    if (err < 0) {
		LOGE("JNI Surface_setComposing3D, err < 0 %d throw exception ***********STARS**************",err);
         doThrowIAE(env);
    }
}
...
static JNINativeMethod sSurfaceControlMethods[] = {
    ...
    {"nativeSetComposing3D",		"(II)V", 
    		(void*)nativeSetComposing3D},
