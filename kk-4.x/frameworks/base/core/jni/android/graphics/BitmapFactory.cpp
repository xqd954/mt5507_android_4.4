...
jfieldID gOptions_frameNumberID;
jfieldID gOptions_bitmapFieldID;
jfieldID gOptions_is4K2KID;
jfieldID gOptions_decodeThumbnailID;
...
static bool optionsIs4k2k(JNIEnv* env, jobject options) {
	return options != NULL && env->GetBooleanField(options, gOptions_is4K2KID);
}
...
#define BYTES_TO_BUFFER 512*1024

static jobject doMtkDecode(JNIEnv* env, SkStreamRewindable* stream, jobject padding,
        jobject options, bool allowPurgeable, jintArray frameDelayArray, bool forcePurgeable ) {
    ...
    if (options != NULL) {
        sampleSize = env->GetIntField(options, gOptions_sampleSizeFieldID);
		bThumb = env->GetBooleanField(options, gOptions_decodeThumbnailID);
        if (optionsJustBounds(env, options)) {
            mode = SkImageDecoder::kDecodeBounds_Mode;
        }

		if (optionsIs4k2k(env, options)){
			mode = SkImageDecoder::kDecode4k2k_Mode;
		}
        ...
        env->SetIntField(options, gOptions_heightFieldID, -1);
        env->SetObjectField(options, gOptions_mimeFieldID, 0);
		env->SetIntField(options, gOptions_frameNumberID, 1);
    ...
    decoder->setPreferQualityOverSpeed(preferQualityOverSpeed);
    decoder->setRequireUnpremultipliedColors(requireUnpremultiplied);
	decoder->setDisplayThumbnail(bThumb);

    mtkSkBitmap* outputBitmap = NULL;
    unsigned int existingBufferSize = 0;
    if (javaBitmap != NULL) {
        outputBitmap = (mtkSkBitmap*) env->GetIntField(javaBitmap, gBitmap_nativeBitmapFieldID);
        if (outputBitmap->isImmutable()) {
            ALOGW("Unable to reuse an immutable bitmap as an image decoder target.");
            javaBitmap = NULL;
            outputBitmap = NULL;
    ...
    SkAutoTDelete<mtkSkBitmap> adb(outputBitmap == NULL ? new mtkSkBitmap : NULL);
    if (outputBitmap == NULL) outputBitmap = adb.get();
    ...
    mtkSkBitmap decodingBitmap;
	if (decoder->getFormat() == SkImageDecoder::kGIF_Format)
	{
		
	    if (!decoder->decodeMtkGif(stream, &decodingBitmap, prefConfig, decodeMode)) {
	        return nullObjectReturn("decoder->decode mtkgif returned false");
	    }
	}
	else
	{
    ...
    if (options != NULL) {
        env->SetIntField(options, gOptions_widthFieldID, scaledWidth);
        env->SetIntField(options, gOptions_heightFieldID, scaledHeight);
		env->SetIntField(options, gOptions_frameNumberID,decodingBitmap.getFrameNumber());
        env->SetObjectField(options, gOptions_mimeFieldID,
                getMimeTypeString(env, decoder->getFormat()));
		if (frameDelayArray != NULL)
		{
			int *p = decodingBitmap.getFrameDelay();
			env->SetIntArrayRegion(frameDelayArray, 0, decodingBitmap.getFrameNumber(), p);
		}
    }
...
static jobject doDecodeAnimationGif(JNIEnv* env, SkStreamRewindable* stream, jobject padding,
        jobject options, bool allowPurgeable, bool forcePurgeable, int frameIdx) {
    ...
    mtkSkBitmap decodingBitmap;
	if (!decoder->decodeAnimationGif(stream, &decodingBitmap, prefConfig, decodeMode, frameIdx)) {
        return nullObjectReturn("decoder->decode returned false");
    }
...
static jobject nativeMtkDecodeStream(JNIEnv* env, jobject clazz, jobject is, jbyteArray storage,
        jobject padding, jobject options, jintArray frameDelay) {
    ...
    if (stream.get()) {
        ...
        // SkImageDecoder_libwebp.
        // FIXME: Get this number from SkImageDecoder
        SkAutoTUnref<SkStreamRewindable> bufferedStream(SkFrontBufferedStream::Create(stream, 512*1024));
        SkASSERT(bufferedStream.get() != NULL);
        // for now we don't allow purgeable with java inputstreams
        bitmap = doMtkDecode(env, bufferedStream, padding, options, false, frameDelay, false);
    }
    return bitmap;
}

static jobject nativeDecodeAnimationGifStream(JNIEnv* env, jobject clazz,
                                  jobject is,       // InputStream
                                  jbyteArray storage,   // byte[]
                                  jobject padding,
                                  jobject options,
                                  int frameIdx) {  // BitmapFactory$Options
    jobject bitmap = NULL;
    SkAutoTUnref<SkStream> stream(CreateJavaInputStreamAdaptor(env, is, storage));
    ...
    if (stream.get()) {
        ...
        SkASSERT(bufferedStream.get() != NULL);
        // for now we don't allow purgeable with java inputstreams
        bitmap = doDecodeAnimationGif(env, bufferedStream, padding, options, false, false, frameIdx);
    }
    return bitmap;
}
...
static JNINativeMethod gMethods[] = {
	...
	{
   		"nativeDecodeAnimationGifStream",
		"(Ljava/io/InputStream;[BLandroid/graphics/Rect;Landroid/graphics/BitmapFactory$Options;I)Landroid/graphics/Bitmap;",
        (void*)nativeDecodeAnimationGifStream
    },
    
    {   "nativeDecodeStream",
        "(Ljava/io/InputStream;[BLandroid/graphics/Rect;Landroid/graphics/BitmapFactory$Options;[I)Landroid/graphics/Bitmap;",
        (void*)nativeMtkDecodeStream
    },
};
...
int register_android_graphics_BitmapFactory(JNIEnv* env) {
	...
	jclass mtkoptions_class = env->FindClass("android/graphics/BitmapFactory$MtkOptions");
    SkASSERT(mtkoptions_class);
	gOptions_frameNumberID = getFieldIDCheck(env, mtkoptions_class, "frameNumber", "I");
	gOptions_is4K2KID = getFieldIDCheck(env, mtkoptions_class, "is4k2k", "Z");
	gOptions_decodeThumbnailID = getFieldIDCheck(env, mtkoptions_class, "isDecodeThumbnail", "Z");
