...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode {
    ...
    private WindowManager.LayoutParams getNavigationBarLayoutParams() {
        ...
        /* MTK Added */
        if(1 == SystemProperties.getInt("ro.mtk.hide_navigation_bar", 1)){
            if(mNavBarHide) {
                lp.alpha = 0;
            } else {
                lp.alpha = 1.0f;
...
//            Log.d(TAG, "size=" + newIconSize + " padding=" + newIconHPadding);
    ...
    /* MTK Added */
    private static boolean mNavBarHide = true;

    public void setSystemUiBarHight(int height) {
        Slog.v(TAG,"setSystemUiBarHeight:" + height);
        mNavBarHide = (height == 0);
        final WindowManager.LayoutParams lp
                = getNavigationBarLayoutParams();
        if(null == lp || null == mNavigationBarView) {
            return;
        }
        mWindowManager.updateViewLayout(mNavigationBarView, lp);
		if(1 == SystemProperties.getInt("ro.mtk.hide_navigation_bar",1)){
