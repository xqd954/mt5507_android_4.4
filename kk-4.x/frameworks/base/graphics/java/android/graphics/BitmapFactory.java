...
public class BitmapFactory {
	...
	public static class MtkOptions extends Options{
	...
	public static Bitmap[] decodeAnimationGifFile(String pathName, Options opts, int frameNumber) {
        Bitmap[] bm = new Bitmap[frameNumber];
        InputStream stream = null;
		...
		for (i = 0; i < frameNumber; i++)
		{
	        try {
	            stream = new FileInputStream(pathName);
	            bm[i] = decodeAnimationGifStream(stream, null, opts, i);
	        } catch (Exception e) {
	            /*  do nothing.
	                If the exception happened on open, bm will be null.
	            */
    ...
    public static Bitmap decodeStream(InputStream is, Rect outPadding, Options opts) {
        ...
        Bitmap bm = null;
		MtkOptions mOpts = null;
		if (opts != null && (opts instanceof MtkOptions))
		{
			mOpts = (MtkOptions)opts;
		}
        ...
        try {
            if (is instanceof AssetManager.AssetInputStream) {
            	...
            	if (opts != null && (opts instanceof MtkOptions) && mOpts != null)
				{
					if (mOpts.frameDelayArray == null) 
						mOpts.frameDelayArray = new int[100];
				}
				...
				if (opts != null && (opts instanceof MtkOptions)  && mOpts != null)
				{
					...
					bm = nativeDecodeStream(is, tempStorage, outPadding, opts, mOpts.frameDelayArray);
				}
				else
				{
	...
	public static Bitmap decodeAnimationGifStream(InputStream is, Rect outPadding, Options opts, int frameIndex) {
		// we don't throw in this case, thus allowing the caller to only check
		// the cache, and not force the image to be decoded.
		if (is == null) {
			return null;
		...
		try{
			...
			if (opts != null) tempStorage = opts.inTempStorage;
			if (tempStorage == null) tempStorage = new byte[16 * 1024];
			bm = nativeDecodeAnimationGifStream(is, tempStorage, outPadding, opts, frameIndex);
	...
	private static native Bitmap nativeDecodeStream(InputStream is, byte[] storage,
            Rect padding, Options opts, int[] frameDelay);
	private static native Bitmap nativeDecodeAnimationGifStream(InputStream is, byte[] storage,
            Rect padding, Options opts, int frameIndex);
}
