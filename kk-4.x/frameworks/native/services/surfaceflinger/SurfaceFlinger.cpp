...
// --- static function ---
#if defined(TIME_MEASUREMENT) || defined(BOOT_C_LOG)
static void BootTimeUtil_addTimeStamp(const char *pMsg)
{
	// TOOD: use property
	int file_proc_boottime = -1;
	if (NULL == pMsg) return;
	
	file_proc_boottime = open("/proc/boottime", O_RDWR);
	if (file_proc_boottime != -1)
	{		
		int ret = 0;
		ret = write(file_proc_boottime, pMsg, strlen(pMsg));
		if (ret < 0)
		{	
			LOGD("********%s: write error:", __FUNCTION__);
			close(file_proc_boottime);
			return;
		}
		close(file_proc_boottime);
		file_proc_boottime = -1;
	}
	else
	{
		LOGD("********%s: open file NG", __FUNCTION__);	
	}
}
#endif
...
{
#if defined(TIME_MEASUREMENT) || defined(BOOT_C_LOG)
	LOGI("MT_SF: constructor");
	BootTimeUtil_addTimeStamp("SF: constructor");
#endif
...
#if defined(TIME_MEASUREMENT) || defined(BOOT_C_LOG)
	LOGI("MT_SF: bF(): to exit bootanim");
	BootTimeUtil_addTimeStamp("SF: bF(): to exit bootanim");
#endif		
    // stop boot animation
...
#if defined(TIME_MEASUREMENT) || defined(BOOT_C_LOG)
	LOGI("MT_SF: init() to start");
	BootTimeUtil_addTimeStamp("SF: init() to start");
#endif
...
#if defined(TIME_MEASUREMENT) || defined(BOOT_C_LOG)
	LOGI("MT_SF: to startBootAnim");
	BootTimeUtil_addTimeStamp("SF: to startBootAnim");
#endif
            ...
            for (size_t i=0 ; i<dc ; i++) {
                ...
                if (j < 0) {
                    ...
                    if (disp != NULL) {
						...
						//40641 add for stereo 3D
						if (state.dspDevStateComposing3D != draw[i].dspDevStateComposing3D)
							{
							LOGD("SF::handle transaction::composing 3D changed. %d -> %d", draw[i].dspDevStateComposing3D, state.dspDevStateComposing3D);
							disp->setComposing3D(state.dspDevStateComposing3D);
							}
						//end
                    }
                }
...
/*add mtk40641*/
status_t SurfaceFlinger::setComposing3D(int dpy, int threeD)
{
    
	LOGW("SF::setComposing3D: threeD: %d", threeD);
    Mutex::Autolock _l(mStateLock);
	if (CC_UNLIKELY(uint32_t(dpy) > mDisplays.size()))
        return BAD_VALUE;
	for (size_t dpys=0 ; dpys<mDisplays.size() ; dpys++){
		DisplayDeviceState& disp(mCurrentState.displays.editValueAt(dpys));	
		
		if((dpy==0) && disp.isMainDisplay())
		{
			LOGW("SF::setComposing3D: hw->getDisplayType() is %d", dpys);
			disp.dspDevStateComposing3D = threeD;
			setTransactionFlags(eDisplayTransactionNeeded);
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch (code) {
        ...
        case UNBLANK:
        {
			if (code == BOOT_FINISHED)
			{
#if defined(TIME_MEASUREMENT) || defined(BOOT_C_LOG)
				LOGI("MT_SF: onTransact(): B_F");
				BootTimeUtil_addTimeStamp("SF: onTransact(): B_F");
#endif
			}
...
SurfaceFlinger::DisplayDeviceState::DisplayDeviceState(DisplayDevice::DisplayType type)
    : type(type), layerStack(DisplayDevice::NO_LAYER_STACK), dspDevStateComposing3D(0), orientation(0) {
