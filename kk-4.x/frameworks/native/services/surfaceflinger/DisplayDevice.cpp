      ...
      mOrientation()
{
	...
	//initialize the compose 3D type
	mComposing3DType = DisplayState::eComposingNormal;
...
//40641 add for 3D
int DisplayDevice::getComposing3DType() const  {
	return mComposing3DType;
}
...
//add by 40641 for stereo 3D
status_t DisplayDevice::setComposing3D(uint32_t threeDType)
{
    int fd = open("/dev/graphics/fb0", O_RDWR, 0);
	
    	if (fd >= 0) {
       		 if (ioctl(fd, /*FBIO_SET_UIMODE*/0x4800, threeDType) == -1) {
            	LOGE("[DisplayDevice::setComposing3D] [%s] FBIO_SET_UIMODE failed. errno:%d", __func__, errno);
       		 }
			 else {
			 	mComposing3DType = threeDType;
				LOGW_IF(true, "[DisplayDevice::setComposing3D] After call ioctl composing3DType = %d \n", mComposing3DType);
			 }
        	close(fd);
    	} else {
        	LOGE("[DisplayDevice::setComposing3D] [%s] open fb failed. errno:%d", __func__, errno);
    	}
    return NO_ERROR;
}
