...
#if 0 //#ifdef JUST_QUERY_FROM_VDP
// [mtk40134] lock buffer which is used by VDP
void BufferQueue::setConsumeBufferFifoSize(int mSize)
{
    resolvedSlotQueueSize = mSize;
}

bool BufferQueue::isBufLockedInVDP(int slotId, int w, int h, int format, int usage)
{
    if (resolvedSlotQueueSize != DEFAULT_MODE)
    {
        ...
        const sp<GraphicBuffer>& buffer(mSlots[slotId].mGraphicBuffer);
        if ((buffer == NULL) ||
        ...
        {
            //LOGE("(yjdbg) isBufLockedInVDP run in reallocate flow!!! bufSlot=%d",slotId);
            return false;
        }
        else
        {
            bool keepByVDP;
            buffer->isBufferKeepByVDP(&keepByVDP);
            //LOGE("(yjdbg) isBufLockedInVDP query from VDP!!! bufSlot=%d, keep=%d",slotId, keepByVDP);
            return keepByVDP;
        }
    }
    return false;
...
void BufferQueue::printBufferStatus()
{
    String8* info = new String8("BufferQueue:: ");
    char ch[20];
    snprintf(ch, 20, "slot= %p ", mSlots);
    info->append(ch);
    int maxBufferCount = getMaxBufferCountLocked(false);
    snprintf(ch, 20, "{%d} ", maxBufferCount);
    info->append(ch);
    for (int i = 0; i < maxBufferCount; i++)
    {
        if (BufferSlot::DEQUEUED != mSlots[i].mBufferState)
        {
            char buffer[128];
            snprintf(buffer, 128, "[%d]=(%d) ", i,mSlots[i].mBufferState);
            info->append(buffer);
        }
    }
    LOGI("%s",info->string());
...
status_t BufferQueue::dequeueBuffer(int *outBuf, sp<Fence>* outFence, bool async,
    ...
    { // Scope for the lock
        ...
        #if 0 //#ifdef JUST_QUERY_FROM_VDP
        // [mtk40134] lock buffer which is used by VDP
        if ((usage & GraphicBuffer::USAGE_HW_TO_FBM)  && (usage & GraphicBuffer::USAGE_HW_COMPOSER))
        {
            //LOGE("(yjdbg) TV_VDP_KEEP_MODE");
            setConsumeBufferFifoSize(TV_VDP_KEEP_MODE);
        }
        else
        {
            //LOGE("(yjdbg) DEFAULT_MODE");
            setConsumeBufferFifoSize(DEFAULT_MODE);
        }
        #endif
        int found = -1;
        bool tryAgain = true;
        ...
        while (tryAgain) {
            ...
            for (int i = 0; i < maxBufferCount; i++) {
                ...
                switch (state) {
                    ...
                    if (found < 0 || isOlder) {
                        // [mtk40134] lock buffer which is used by VDP
                        #if 0//#ifdef JUST_QUERY_FROM_VDP
                        if(isBufLockedInVDP(i, w, h, format, usage))
                        {
                            lockedCount++;
                        }
                        else
            ...
            #if 0 //#ifdef JUST_QUERY_FROM_VDP
            if((resolvedSlotQueueSize != DEFAULT_MODE) && (found == INVALID_BUFFER_SLOT) && (lockedCount != 0))
            {
                //LOGE("(yjdbg) lockedCount: %d", lockedCount);
                tryAgain = false;
            }
            #endif
            if (tryAgain) {
            ...
            if((resolvedSlotQueueSize != DEFAULT_MODE) && (found == INVALID_BUFFER_SLOT) && (lockedCount != 0))
            {
                //printBufferStatus();
                tryAgain = true;
                //LOGI("Wait Here Until Some Other Buffer Is Consumed");
                mMutex.unlock();
                usleep(5000);
                mMutex.lock();
            }
