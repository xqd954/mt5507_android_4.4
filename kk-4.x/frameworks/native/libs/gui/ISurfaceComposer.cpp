...
public:
	...
	/**40641*/
	virtual status_t setComposing3D(int dpy, int threeDType)
	 {
		 Parcel data, reply;
		 data.writeInterfaceToken(ISurfaceComposer::getInterfaceDescriptor());
		 data.writeInt32(dpy);
		 data.writeInt32(threeDType);
		 remote()->transact(BnSurfaceComposer::SET_COMPOSING3D, data, &reply);
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch(code) {
		...
		/**40641*/
		case SET_COMPOSING3D: {
            CHECK_INTERFACE(ISurfaceComposer, data, reply);
            int dpy = data.readInt32();
            int threeDType = data.readInt32();
            reply->writeInt32( setComposing3D(dpy, threeDType) );
			return NO_ERROR;
        }
