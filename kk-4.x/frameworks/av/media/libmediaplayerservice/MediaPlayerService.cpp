...
Mutex MediaPlayerService::mGetPlayerTypeLock;
        ...
        int audioSessionId, uid_t uid)
{
    ...
    mOutputPath = OUTPUT_VIDEO_Default;
    mRMPriority = 0;
#endif
    
    int useCmpbInGallery;
    #define PROPERTY_VALUE_MAX 92
    char value[PROPERTY_VALUE_MAX];
    
    /*property_get("use.cmpb.in.gallery", value, "-1");
    useCmpbInGallery = atoi(value);
    if (useCmpbInGallery == 1)
        {
            mPlayerType = CMPB_PLAYER; //use cmpb player
    }*/
    property_get("use.cmpb.in.videoview", value, "-1");
    useCmpbInGallery = atoi(value);
    if (useCmpbInGallery > 0 && useCmpbInGallery <= MTK_STREAM_PLAYER)
        {
        mPlayerType = useCmpbInGallery; //use cmpb player
    }
    
    LOGI("MediaPlayerService::Client() use playe type = %d ", mPlayerType);    
    ///////////////////////////////////
	//MTK END
        ...
        const char *url, const KeyedVector<String8, String8> *headers)
{
    ...
    if (headers)
    {
        ...
        mUriHeaders = *headers;
         ...
         //PlayerType
         index2 = mUriHeaders.indexOfKey(String8("X-tv-PlayerType"));
        if (index2 >= 0) 
        {
             value = mUriHeaders.valueAt(index2);
             ALOGD("MediaPlayerService::X-tv-PlayerType %s\n",value.string());
             
             if(!strcasecmp(value.string(),"STAGEFRIGHT_PLAYER"))
             {
                 mPlayerType = STAGEFRIGHT_PLAYER;
             }
             else if(!strcasecmp(value.string(),"CMPB_PLAYER"))
             {
                 mPlayerType = CMPB_PLAYER;
             }
             else if(!strcasecmp(value.string(),"MTK_STREAM_PLAYER"))
             {
                 mPlayerType = MTK_STREAM_PLAYER;
             }
         }
    }
    #endif  
    ...
    if (strncmp(url, "content://", 10) == 0) {
		...
		//MTK BEGIN  PIC:Guanghui.Wang
        if(0 == strncasecmp(url,"/system/media/audio/ui/VideoRecord.ogg", 38) ||
            0 == strncasecmp(url,"/system/media/audio/ui/camera_click.ogg", 39))
        {
            LOGI("It's camera video, use android default player");
            mPlayerType = 0;
        }

        MediaPlayerService::mGetPlayerTypeLock.lock();
        LOGI("MediaPlayerService: get player type locked...1!!\n");
        //MediaPlayerFactory::IFactory::setCmpbPlayer(mPlayerType == 0 ? false : true);
        
        MediaPlayerFactory::IFactory::setPlayerType(mPlayerType);
        
        player_type playerType = MediaPlayerFactory::getPlayerType(this, url);
        MediaPlayerService::mGetPlayerTypeLock.unlock();
        LOGI("MediaPlayerService: get player type unlocked...1!!\n");
		//MTK END
	    LOGI("MediaPlayerService::Client::setDataSource. mPlayerType=%d,getplayertype=%d", mPlayerType,(int)playerType);
    ...
    #if 0// Test Code =>OK    
    int ret=0;
    ALOGD("MediaPlayerService::AssignSWVideoDecoder\n");
    ret=assignSWVideoDecoder(mSWVideoDecoder);
    ALOGD("MediaPlayerService::assignSWVideoDecoder ret= %d\n",ret);
#endif        
     if (headers)
     {
         ssize_t index2=0;
         ...
         //assignSWVideoDecoder
         index2 = mUriHeaders.indexOfKey(String8(k_assignSWVideoDecoder)); 
          
          ALOGD("MediaPlayerService::AssignSWVideoDecoder\n");
          if (index2 >= 0) {
              value = mUriHeaders.valueAt(index2);
              ALOGD("MediaPlayerService::assignSWVideoDecoder %s\n",value.string());
              
              mSWVideoDecoder = (bool) atoi(value.string());
              ret=assignSWVideoDecoder(mSWVideoDecoder);
              ALOGD("MediaPlayerService::assignSWVideoDecoder ret= %d\n",ret);
          }
          
          //AssignSWAudioDecoder
          index2 = mUriHeaders.indexOfKey(String8(k_assignSWAudioDecoder));
          if (index2 >= 0) {
              value = mUriHeaders.valueAt(index2);
              ALOGD("MediaPlayerService::AssignSWAudioDecoder %s\n",value.string());
     
              mSWAudioDecoder = (bool) atoi(value.string());
              ret=assignSWAudioDecoder(mSWAudioDecoder);
              ALOGD("MediaPlayerService::assignSWAudioDecoder ret= %d\n",ret);
          }
          
         //OutputPath
         index2 = mUriHeaders.indexOfKey(String8(k_outputPath)); 
         if (index2 >= 0) 
         {
             value = mUriHeaders.valueAt(index2);
             ALOGD("MediaPlayerService::OutputPath value %s\n",value.string());
             
             if(!strcasecmp(value.string(),"OUTPUT_VIDEO_MAIN"))
             {
                 mOutputPath = OUTPUT_VIDEO_MAIN;
             }
             else if(!strcasecmp(value.string(),"OUTPUT_VIDEO_SUB"))
             {
                 mOutputPath = OUTPUT_VIDEO_SUB;
             }
             else if(!strcasecmp(value.string(),"OUTPUT_GRAPHIC"))
             {
                 mOutputPath = OUTPUT_GRAPHIC;
             }
             else
             {
                 mOutputPath = OUTPUT_VIDEO_Default;
             }
             ret=setSurfaceType(mOutputPath);
              ALOGD("MediaPlayerService::setSurfaceType ret= %d\n",ret);
         }
         
          //RMPriority
          index2 = mUriHeaders.indexOfKey(String8(k_RMPriority));
          if (index2 >= 0) {
              value = mUriHeaders.valueAt(index2);
              ALOGD("MediaPlayerService::RMPriority %s\n",value.string());
          
              mRMPriority = (int) atoi(value.string());
              
              ret=setRMPriority(mRMPriority);
              ALOGD("MediaPlayerService::setRMPriority ret= %d\n",ret);
          }
     }
...
status_t MediaPlayerService::Client::setDataSource(int fd, int64_t offset, int64_t length)
{
    ...
    LOGI("aPathIn=%s, aFilePath=%s, ret=%d",aPathIn,aFilePath, ret);
    if(ret > 0 && ret < 64)
    {
        if(0 == strncasecmp(aFilePath,"/system/media/audio/ui/VideoRecord.ogg", 38) ||
           0 == strncasecmp(aFilePath,"/system/media/audio/ui/camera_click.ogg", 39))
        {
            LOGI("It's camera video, use android default player");
            mPlayerType = 0;
        }
    ...
    ////////////////////////////////
    MediaPlayerService::mGetPlayerTypeLock.lock();
    LOGI("MediaPlayerService: get player type locked...2!!\n");
    //MediaPlayerFactory::IFactory::setCmpbPlayer(mPlayerType == 0 ? false : true);
    MediaPlayerFactory::IFactory::setPlayerType(mPlayerType);
    ...
    player_type playerType = MediaPlayerFactory::getPlayerType(this,
                                                               ...
                                                               length);
    MediaPlayerService::mGetPlayerTypeLock.unlock();
    LOGI("MediaPlayerService: get player type unlocked...2!!\n");
...
status_t MediaPlayerService::Client::setDataSource(
    ...
    // create the right type of player
    MediaPlayerService::mGetPlayerTypeLock.lock();
    LOGI("MediaPlayerService: get player type locked...3!!\n");
    //MediaPlayerFactory::IFactory::setCmpbPlayer(mPlayerType == 0 ? false : true);
    MediaPlayerFactory::IFactory::setPlayerType(mPlayerType);
    player_type playerType = MediaPlayerFactory::getPlayerType(this, source);
    MediaPlayerService::mGetPlayerTypeLock.unlock();
    LOGI("MediaPlayerService: get player type unlocked...3!!\n");
        ...
        const sp<IGraphicBufferProducer>& bufferProducer)
{
    ...
    //add by kangjp,for video fullscreen properties [begin]
    /*
    int outWidth = 0;
    int outHeight = 0;
    ANativeWindow* anw_ptr = NULL;
    anw_ptr = anw.get();
    if(anw_ptr != NULL) {
        anw->query(anw_ptr, NATIVE_WINDOW_WIDTH, &outWidth);
        anw->query(anw_ptr, NATIVE_WINDOW_HEIGHT, &outHeight);
    }
    if ((outHeight == 1080 && outWidth == 1920) ||
        (outHeight == 2160 && outWidth == 3840)) {
        isNativeWindowFullScreen = true;
    } else {
        isNativeWindowFullScreen = false;
    }
    //add by kangjp,for video fullscreen properties [end]
    */
...
status_t MediaPlayerService::Client::start()
{
    ...
    p->setLooping(mLoop);
    //add by kangjp,for video fullscreen properties [begin]
    status_t result = p->start();
    /*
    if(isNativeWindowFullScreen) {
        property_set("sys.video.fullscreen", "true");
    }
    */
    return result;
    //add by kangjp,for video fullscreen properties [end]
...
status_t MediaPlayerService::Client::stop()
{
    ...
    if (p == 0) return UNKNOWN_ERROR;
   //add by kangjp,for video fullscreen properties [begin]
    status_t result = p->stop();
    /*
    isNativeWindowFullScreen = false;
    property_set("sys.video.fullscreen", "false");
    */
    return result;
    //add by kangjp,for video fullscreen properties [end]
}
...
/*MTK Begin*/
status_t MediaPlayerService::Client::setSurfaceType(int i4Type)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ALOGD("setSurfaceType %d",i4Type);
    
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return false;
    return p->setSurfaceType(i4Type);
}

status_t MediaPlayerService::Client::setPlayerType(int i4PlayerType)
{
#ifdef CC_SUPPORT_TV_STAGEFRIGHT
    
    LOGI("Client::setPlayerType(%d)", i4PlayerType);
    if (i4PlayerType > 0 && i4PlayerType <= MTK_STREAM_PLAYER)
    {
        mPlayerType = i4PlayerType;
    }
    else
    {
        mPlayerType = STAGEFRIGHT_PLAYER;
...
#endif
#else
    LOGI("Client::setPlayerType(), use cmpb as default");
    mPlayerType = CMPB_PLAYER;
#endif
    return OK;
}

status_t MediaPlayerService::Client::setVideoRect(int i4left, int i4top, int i4right, int bottom)
{
	// Do nothing here, SurfaceView will handle it.
    return INVALID_OPERATION;
    /*
    LOGV("Client::setVideoRect()");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == NULL) return UNKNOWN_ERROR;
    return p->setVideoRect(i4left, i4top, i4right, bottom);*/
}

status_t MediaPlayerService::Client::setSvctxPath(const char *pu1_svctx_path)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ...
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return false;
    return p->setSvctxPath(pu1_svctx_path);
}

bool MediaPlayerService::Client::setSubtitleDataSource(const char *url)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ALOGD("url %s",url);
    ...
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return false;
    return p->setSubtitleDataSource(url);
}

bool MediaPlayerService::Client::setSubtitleDataSourceEx(const char *urlSub, const char *urlIdx)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ALOGD("urlSub %s",urlSub);
    ALOGD("urlIdx %s",urlIdx);
    
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return false;
    return p->setSubtitleDataSourceEx(urlSub,urlIdx);
}

status_t MediaPlayerService::Client::setSpeed(int speed)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ALOGD("speed %d",speed);
...
bool MediaPlayerService::Client::setTS(int index)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ALOGD("index %d",index);
    ...
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return false;
    return p->setTS(index);
}

bool MediaPlayerService::Client::canDoTrick(int speed)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ALOGD("speed %d",speed);
    ...
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return false;
    return p->canDoTrick(speed);
}

bool MediaPlayerService::Client::canDoSeek(int speed)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ALOGD("speed %d",speed);
    ...
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return false;
    return p->canDoSeek(speed);
}

bool MediaPlayerService::Client::step(int amount)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ALOGD("step %d",amount);
    ...
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return false;
    return p->step(amount);
}

bool MediaPlayerService::Client::setPlayerRole(int eType)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ...
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return false;
    return p->setPlayerRole(eType);
}

bool MediaPlayerService::Client::setABRepeat(int type)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ...
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return false;
    return p->setABRepeat(type);
}

bool MediaPlayerService::Client::seekToPosition(unsigned long long position)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ALOGD("seekToPosition %lld",position);
    
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return false;
    return p->seekToPosition(position);
}

bool MediaPlayerService::Client::getSpeed(int &speed)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
...
bool MediaPlayerService::Client::getSubtitleTrackCount(int &count)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ...
    b_ret  = p->getSubtitleTrackCount(count); 
    
    ALOGD("getSubtitleTrackCount %d",count);
    
    return b_ret ;
}

bool MediaPlayerService::Client::getSubtitleTrackInfo(int trackIndex,
                          int &SubtitleType,
                          ...
                          String8 &SubtitleTitle)
{
    ...
    b_ret  = p->getSubtitleTrackInfo(trackIndex,
                                  SubtitleType,
                                  SubtitleCodeType,
                                  SubtitleLanguage,
                                  SubtitleTitle);
    
    LOGI("SubtitleLanguage %s %d %s",__FILE__,__LINE__,SubtitleLanguage.string());
    LOGI("SubtitleTitle %s %d %s",__FILE__,__LINE__,SubtitleTitle.string());
    return b_ret;
}


bool MediaPlayerService::Client::getAudioTrackCount(int &count)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ...
    b_ret  = p->getAudioTrackCount(count); 
    
    ALOGD("getAudioTrackCount %d",count);
    
    return b_ret ;
}

bool MediaPlayerService::Client::getAudioTrackInfo(bool IsAudio,
                           int trackIndex,
                           int & AudioTrackNum,
                           int & BitRate,
                           int & TotalPlayTime,
                           ...
                           String8 &Language)
{
    ...
    b_ret  = p->getAudioTrackInfo(IsAudio,
                                  trackIndex,
                                  AudioTrackNum,
                                  BitRate,
                                  TotalPlayTime,
    ...
    //LOGI("Year %s %d %s",__FILE__,__LINE__,Year.string());
    
    return b_ret;
}

bool MediaPlayerService::Client::setSubtitleAttr(int eType, void * prAttr)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ...
    b_ret  = p->setSubtitleAttr(eType,prAttr); 
    
    ALOGD("E_Subtitle_Attr_Type %d",eType);
    
    return b_ret ;
}

bool MediaPlayerService::Client::flushSubtitleAttr()
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ...
    b_ret  = p->flushSubtitleAttr(); 
    
    return b_ret ;
}

bool MediaPlayerService::Client::setAudioTrack(int track)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ...
    b_ret  = p->setAudioTrack(track); 
    
    return b_ret ;
}

bool MediaPlayerService::Client::setSubtitleTrack(int track)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ...
    b_ret  = p->setSubtitleTrack(track); 
    
    return b_ret ;
}

bool MediaPlayerService::Client::getBufferPercent(int &OutPercent)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ...
    b_ret  = p->getBufferPercent(OutPercent); 
    
    return b_ret ;
}

bool MediaPlayerService::Client::onSubtitleTrack()
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ...
    b_ret  = p->onSubtitleTrack(); 
    
    return b_ret ;
}

bool MediaPlayerService::Client::offSubtitleTrack()
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ...
    b_ret  = p->offSubtitleTrack(); 
    
    return b_ret ;
}

bool MediaPlayerService::Client::ifMP3Media(bool & fgIsMP3Media)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ...
    b_ret  = p->ifMP3Media(fgIsMP3Media); 
    
    ALOGD("ifMP3Media %d",fgIsMP3Media);
    
    return b_ret ;
}

status_t MediaPlayerService::Client::getCurrentBytePosition(int *mposition)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ...
    int tmp = 0; 
    b_ret  = p->getCurrentBytePosition(&tmp); 
    *mposition = tmp;
    ALOGD("getCurrentBytePosition %d",tmp);
    
    return b_ret ;
}

status_t MediaPlayerService::Client::getDownloadRate(double *rate)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ...
    if(p == 0)   return NO_INIT;
    double tmp = 0; 
    b_ret  = p->getDownloadRate(&tmp); 
    *rate = tmp;
    ALOGD("getDownloadRate %lf",tmp);
    
    return b_ret ;
}

#ifdef CC_SUPPORT_TV_STAGEFRIGHT
status_t MediaPlayerService::Client::assignSWVideoDecoder(bool useSWDec)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ...
    ret  = p->assignSWVideoDecoder(useSWDec); 
    
    ALOGD("useSWDec %d",useSWDec);
    
    return ret ;
}
status_t MediaPlayerService::Client::assignSWAudioDecoder(bool useSWDec)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
    ...
    ret  = p->assignSWAudioDecoder(useSWDec); 
    
    ALOGD("useSWDec %d",useSWDec);
    
    return ret ;
}
status_t MediaPlayerService::Client::setRMPriority(int i4value)
{
    ALOGD("%s %s %d",__FUNCTION__,__FILE__,__LINE__);
                                       ...
                                       const sp<IMemoryHeap>& heap, size_t *pSize)
{
    ...
    player_type playerType =
        MediaPlayerFactory::getPlayerType(NULL /* client */, url);/*not use cmpb player*/
    ALOGV("player type = %d", playerType);
