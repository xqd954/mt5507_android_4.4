...
NuPlayer::NuPlayer()
    ...
    char prop_no_audio[PROPERTY_VALUE_MAX];
    int iNoAudio = 0;
    property_get("mtk.NuPlayer.noAudio", prop_no_audio, "0");
    mDebugNoAudio = atoi(prop_no_audio);
    ALOGD("mtk.NuPlayer.noAudio =%d",mDebugNoAudio);
    if(mDebugNoAudio)
    {
    ...
    {
        //ALOGD("disable audio. Now video Only!");
    }
#endif    
}
...
NuPlayer::~NuPlayer() {
    ...
    if(!strcmp(value, "1")  || strcasecmp(value, "true") == 0)  {
        ...
        ALOGD("~NuPlayer force stop hwc_enable_vdp status = %d",status);
    ...
    else
    {
        ...
        if (mNativeWindow != NULL) {
           if(mNativeWindow->getNativeWindow()->query(
               mNativeWindow->getNativeWindow().get(), NATIVE_WINDOW_CONSUMER_USAGE_BITS,
               &usageBits) !=OK)
           {
               if ((usageBits & GRALLOC_USAGE_HW_COMPOSER) && (usageBits & GRALLOC_USAGE_HW_TO_FBM))
               {
                    int status = hwc_enable_vdp(0, 0);
                    ALOGD("~NuPlayer hwc_enable_vdp status = %d",status);
               }
           }
        }     
    }
...
void NuPlayer::setUrlBeforeSeekAync(const char *url) {
	mSource->setUrlBeforeSeek(url);
}
...
void NuPlayer::setDataSourceAsync(int fd, int64_t offset, int64_t length) {
    ...
    if(err != OK){
        ...
        ALOGE("setDataSource MEDIA_ERROR Line:%d",__LINE__);
        ALOGE("setDataSource source init check fail err=%d",err);
        source = NULL;
        msg->setObject("source", source);
    ...
    switch (msg->what()) {
        ...
        case kWhatGetTrackInfo:
        {
            ...
            if (mSource != NULL) {
                ...
                if(err!=OK)
                {
                    ALOGE("err %d, Line:%d", err,__LINE__);
                }
            }
...
#ifndef ANDROID_DEFAULT_CODE
            mPlayState = PLAYING;
            mRenderer->onCheckRenderBufferStatus();
#endif
            break;
        }
        ...
        case kWhatScanSources:
        {
            ...
            if (needScanAgain) {     //scanning source is not completed, continue
                ...
                if (mPlayState == PLAYING || mPlayState == PAUSED)
                {
                    ...
                    if (mVideoAbsent) {
                        ...
                        ALOGE("MEDIA_SET_VIDEO_SIZE Line:%d",__LINE__);
                       }
            }
#else
            ...
            if (mAudioSink != NULL) {
                if(mDebugNoAudio)
                {
                    ALOGD("Not initAudioDecoder for disable audio");
                }
                else
                {
            ...
            if ((err = mSource->feedMoreTSData()) != OK) {
                if (mAudioDecoder == NULL && mVideoDecoder == NULL) {
                    ...
                    if (err == ERROR_END_OF_STREAM) {
                        ...
                        ALOGE("MEDIA_ERROR Line:%d",__LINE__);
                        notifyListener(MEDIA_ERROR, MEDIA_ERROR_UNKNOWN, err);
                    }
                }
                break;
        ...
        case kWhatRendererNotify:
        {
            ...
            }else if (what == Renderer::kWhatBufferNotify) {
                int32_t rate;
                CHECK(msg->findInt32("bufRate", &rate));
                ALOGI(" buffering rate %d mAudioEOS = %d mVideoEOS == %d",rate,mAudioEOS,mVideoEOS);
                
                if (rate == 0) { 
                    if ((mRenderer != NULL) && !(mAudioEOS && mVideoEOS))
                    {
        ...
        case kWhatSeek:
        {
            ...
            int64_t TimeMin=seekTimeUs/(1000000ll * 60ll);
            int64_t TimeSec=(seekTimeUs/1000000ll)%60;
            
            ALOGD("kWhatSeek seekTimeUs=%lld us,(%lld m: %lld s)", 
                seekTimeUs,TimeMin, TimeSec);
        ...
        case kWhatResume:
        {
            ...
            if (mSource == NULL)
            {
                ALOGD("kWhatResume, mSource is NULL");
                notifyListener(MEDIA_PLAY_COMPLETE, DEAD_OBJECT, 0);
                break;
            }
#endif
            //if (mSource->play() == OK)
...
{
    // Youxu Meng Modified --> MTK71193-2014/1020 for DTV00608682/DTV00611422
...
status_t NuPlayer::instantiateDecoder(bool audio, sp<Decoder> *decoder) {
    ...
    *decoder = audio ? new Decoder(notify) :
    ...
    if((mDataSourceType == SOURCE_HttpLive) && (!audio))
    {
        format->setInt32("max-width", 1920);
        format->setInt32("max-height", 1080);
        ALOGI("format set max width and height");
    }
...
status_t NuPlayer::feedDecoderInputData(bool audio, const sp<AMessage> &msg) {
    ...
    if ((audio && IsFlushingState(mFlushingAudio))
        ...
        {
            reply->setInt32("err", FAKE_INFO_DISCONTINUITY);
        }
        reply->post();
        return OK;
    }
    ...
    do {
        ...
        //DTV00638665
            ...
            if (err == INFO_DISCONTINUITY) {
                ...
                if (formatChange && (mDataSourceType == SOURCE_HttpLive))
                {   
                   formatChange = 0;
                   ALOGI("discontinuity seamless formatChange(%d) audio(%d).", formatChange, audio);
                }
...
#ifndef ANDROID_DEFAULT_CODE
                if (formatChange) //skip flush while seek/timechange
#else                
...
void NuPlayer::renderBuffer(bool audio, const sp<AMessage> &msg) {
  ...
  int64_t mediaTimeUs;
  CHECK(buffer->meta()->findInt64("timeUs", &mediaTimeUs));
  ALOGV("queue buffer %s %.2f", audio ? "audio":"\t\tvideo", mediaTimeUs / 1E6);
...
void NuPlayer::performSeek(int64_t seekTimeUs) {
    ...
    sp<AMessage> status_change = new AMessage(kWhatSeek, id());
    status_change->setInt32("status", MEDIA_SEEK_COMPLETE);

    mRenderer->onNotifyStatusChange(status_change);
#else
...
void NuPlayer::finishSeek() {
    ...
    if (mDriver != NULL) {
        ...
        if (driver != NULL) {
            ...
            int64_t TimeMin=mSeekTimeUs/(1000000ll * 60ll);
            int64_t TimeSec=(mSeekTimeUs/1000000ll)%60;
            
            ALOGI("seekTo(%.2f) (%lld m: %lld s) completed", mSeekTimeUs / 1E6,TimeMin, TimeSec);
        }
    }
    mSeekTimeUs = -1;    
}
