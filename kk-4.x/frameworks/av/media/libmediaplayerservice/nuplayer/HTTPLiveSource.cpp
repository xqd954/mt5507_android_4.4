...
void NuPlayer::HTTPLiveSource::setUrlBeforeSeek(const char *url){
 	mLiveSession->setUrlBeforeSeek(url);
}

void NuPlayer::HTTPLiveSource::reset() {
	if (mLiveSession != NULL) {
	   	ALOGD("HTTPLiveSource:disconnect web!");
        mLiveSession->disconnect();
        mLiveSession.clear();
...
sp<AMessage> NuPlayer::HTTPLiveSource::getFormat(bool audio) {
    ...
    if (err != OK) {
        ALOGD("%s getFormat is NULL!", audio?"audio":"video");
        return NULL;
    }
    ALOGD("HTTPLiveSource::getFormat return OK!");
...
{	
    ...
    bool tracksPresent = OK;//mLiveSession->allTracksPresent();
