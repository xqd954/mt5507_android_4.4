...
bool isSeekGap=false;
bool static isSpecail=false;
static int64_t resumPosTimeUS=0;
...
status_t NuPlayerDriver::setDataSource(
    ...
    do
    {
        // "http://chyd-wsvod.wasu.tv/data11/ott/344/2014-09/15/1410751214259_193023/playlist.m3u8"
        index_1 = mUri.find("http://chyd-wsvod.wasu.tv/data11/ott/344/2014-09/15/");
        
        index_2 = mUri.find("playlist.m3u8");
        
        if (0==index_1&&
            index_2>0)
        {
            //CTS
            ALOGI("Kling setDataSource_l Specail Line %d",__LINE__);
...
status_t NuPlayerDriver::stop() {
    //To keep last pos for resme
    resumPosTimeUS =mPositionUs;
#ifndef ANDROID_DEFAULT_CODE
    Mutex::Autolock autoLock(mLock);
...
status_t NuPlayerDriver::seekTo(int msec) {
    ...
    ALOGD("seekTo(%d ms) (%lld m: %lld s) mState = %d", msec,TimeMin, TimeSec, (int)mState);
#endif 
    do
    {
        //New begin,
        ...
        //Not resme case,
        if(resumPosTimeUS<=1000ll)
        {
            ALOGD("Kling Normal seek. Line: %d",__LINE__);
            mHasBeenSeek = false;
            mLastSeekPosUs =seekTimeUs;
            break;
        }
        ...
        //Seek gap step1
        //DTV00637691
        //Seek same with last min => Resume case,back seek always right
        if(TimeMin==
           (resumPosTimeUS/(1000000ll * 60ll)))
        
        {
            if(isSpecail)//50 gap issue
            {
                seekTimeUs = seekTimeUs-60000000ll;
            }
            else 
            {
                seekTimeUs = seekTimeUs-20000000ll;
            }
            //Resume seek special
            mHasBeenSeek = true;
            isSeekGap = true;
            //reset
            resumPosTimeUS = 0;
            ALOGD("Kling Resume Case. Line:%d",__LINE__);
        }
        else
        {
            mHasBeenSeek = true;
...
status_t NuPlayerDriver::getCurrentPosition(int *msec) {
    ...
    if (mPositionUs < 0) {
        *msec = 0;
        ALOGI("Kling getCurrentPosition 0!!! Line %d",__LINE__);
    } 
    else 
    {
        //Seek gap step2
        //DTV00639762, seek gap issue
        if(isSeekGap)
        {
            timeGap = mPositionUs-mLastSeekPosUs;
            
            //ALOGI("Kling timeGap %d Line %d",timeGap,__LINE__);
            
            if(timeGap>=10000000ll)//Gap 10s
            {
                *msec = (mLastSeekPosUs + 3000000ll) / 1000; //split gap to step 3s
                mLastSeekPosUs = mLastSeekPosUs + 3000000ll;//Keep step
                
                int64_t TimeMin=*msec/(1000ll * 60ll);
                int64_t TimeSec=(*msec/1000ll)%60;
                //ALOGI("Kling (%lld m: %lld s), Line %d",TimeMin, TimeSec,__LINE__);
            }
            else
            {
                isSeekGap = false;
        ...
        {
            //Dont't use this logic
            if(mHasBeenSeek)
            {
                //Over Duration issue
                if(mPositionUs+60000000ll*4>=mDurationUs)
                {
                    mHasBeenSeek = false;
                    *msec = (mPositionUs + 500ll) / 1000;
                    return OK;
                }
                //DTV00638848, play pos < seek
                if(mPositionUs<mLastSeekPosUs)
                {
                    mLastSeekPosUs = mLastSeekPosUs + 1000000ll/5;//Step: seektime +1/5 s

                    mPositionUs = mLastSeekPosUs;

                   ALOGI("Kling Issue Handle Line %d",__LINE__);
                }
                else
                {
                    //mHasBeenSeek = false;
    ...
    ALOGD("Kling getCurrentPosition: %ds (%d m: %d s)", 
                                  sec,TimeMin, TimeSec);
#endif
    return OK;
}
...
#if 0//Wait confirm
		case INVOKE_ID_SET_URL_BEFORE_SEEK:
		{
			int trackIndex = request.readInt32();
			char *url = (char *)trackIndex;
			ALOGI("INVOKE_ID_SET_URL_BEFORE_SEEK ('%s')", url);
			mPlayer->setUrlBeforeSeekAync(url);
		}
#endif
        default:
        {
