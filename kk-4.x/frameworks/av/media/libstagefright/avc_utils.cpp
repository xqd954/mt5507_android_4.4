...
int decodeVOLHeader(const uint8_t *vol, size_t size, struct MPEG4Info *s) {
    ...
    if (!br.getBits(1)) {
        LOGW("missing marker before vop_time_increment_resolution");
    }
    ...
    if (!br.getBits(1)) {
        LOGW("missing marker before fixed_vop_rate");
    }
...
int decodeShortHeader(const uint8_t *vol, size_t size, struct MPEG4Info* s) {
    ...
    if (tmpvar !=  SHORT_VIDEO_START_MARKER) {
        LOGE("bad short header %x", tmpvar);
        return -1;
    }
    ...
    /* Marker Bit */
    if (!br.getBits(1)) {
        LOGE("bad market bit in PTYPE");
        return -1;
    }

    /* Zero Bit */
    if (br.getBits(1)) {
        LOGE("bad zero bit in PTYPE");
        return -1;
    }
    ...
    switch (br.getBits(3)) {
        ...
        case 7:
            LOGI("extended PTYPE signaled");
            extended_PTYPE = true;
            break;

        default:
            LOGE("bad H.263 source format");
            return -1;
    }
    ...
    if (extended_PTYPE == false) {
        ...
        if (UFEP == 1) {
            ...
            if (br.getBits(4) != 8) {
                LOGE("bad reserved 4 bits, not 0x1000");
                return -1;
            }
        }
        ...
        if (UFEP == 0 || UFEP == 1) {
            ...
            if (br.getBits(3) != 1) {
                LOGE("bad reserved 3 bits, not 0x001");
                return -1;
            }
        } else {
            LOGE("bad UFEP %d", UFEP);
            return -1;
        }
        ...
        if (custom_PFMT == 1 && UFEP == 1) {
            ...
            tmpvar = br.getBits(4);
            if (tmpvar == 0) {
                LOGE("bad aspect ratio %d", tmpvar);
                return -1;
            }
            /* Extended PAR */
            if (tmpvar == 0xF) {
            ...
            /* marker bit */
            if (!br.getBits(1)) {
                LOGE("bad marker bit after width");
                return -1;
            }
            tmpvar = br.getBits(9);
            if (tmpvar == 0) {
