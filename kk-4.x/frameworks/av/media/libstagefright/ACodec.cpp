...
struct CodecObserver : public BnOMXObserver {
    ...
    virtual void onMessage(const omx_message &omx_msg) {
        ...
        switch (omx_msg.type) {
            ...
            {
                ALOGV("event(%d,%d,%d)", omx_msg.u.event_data.event,
                    omx_msg.u.event_data.data1, omx_msg.u.event_data.data2);
            ...
            case omx_message::EMPTY_BUFFER_DONE:
            {
                ...
                #ifdef ACODE_DEBUG_SUPPORT  //For debug
                static int32_t EMPTY_BUFFER_DONE =0;
                
                ++EMPTY_BUFFER_DONE;
                
                ALOGD("FRM_NUM_DBG_1: EMPTY_BUFFER_DONE %d",EMPTY_BUFFER_DONE);
                #endif
            ...
            case omx_message::FILL_BUFFER_DONE:
            {
                ...
                ALOGD("FRM_NUM_DBG_1: FILL_BUFFER_DONE %d",FILL_BUFFER_DONE);
                #endif
                msg->setPointer(
...
status_t ACodec::configureCodec(
	...
	ALOGD("+ configureCodec L(%d)", __LINE__);
    ...
    if (err != OK) {
		ALOGE("[%s] setComponentRole err = %d", mComponentName.c_str(), err);
        return err;
    }
    ...
    if (encoder && strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_FLAC)
        ...
        ALOGE("[%s] FLAC doesn't need a bitrate", mComponentName.c_str());
    ...
    if (video) {
        if (encoder) {
			...
			if (OK != err)
			{
				ALOGE("[%s] setupVideoEncoder failed err = %d", mComponentName.c_str(), err);
			}
        } else {
            ...
            if (!msg->findInt32("width", &width)
				...
				if (OK != err)
				{
					ALOGE("[%s] get width or height failed err = %d", mComponentName.c_str(), err);
				}				
            } else {
                err = setupVideoDecoder(mime, width, height);
				if (OK != err)
				{
					ALOGE("[%s] setupVideoDecoder failed err = %d", mComponentName.c_str(), err);
				}				
            }
        }
        ...
        if (!msg->findInt32("channel-count", &numChannels)
			...
			if (OK != err)
			{
				ALOGE("[%s] setupRawAudioFormat failed err = %d", mComponentName.c_str(), err);
			}			
        }
        ...
        if (!msg->findInt32("channel-count", &numChannels)
			...
			if (OK != err)
			{
				ALOGE("[%s] get numChannels or sampleRate failed err = %d", mComponentName.c_str(), err);
			}			
        } else {
			...
			if (OK != err)
			{
				ALOGE("[%s] setupAACCodec failed err = %d", mComponentName.c_str(), err);
			}			
        }
		...
		if (OK != err)
		{
			ALOGE("[%s] setupAMRCodec false failed err = %d", mComponentName.c_str(), err);
		}		
    } else if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_AMR_WB)) {
        err = setupAMRCodec(encoder, true /* isWAMR */, bitRate);
		if (OK != err)
		{
			ALOGE("[%s] setupAMRCodec true failed err = %d", mComponentName.c_str(), err);
		}		
        ...
        if (!msg->findInt32("channel-count", &numChannels)) {
			...
			if (OK != err)
			{
				ALOGE("[%s] get numChannels failed err = %d", mComponentName.c_str(), err);
			}			
        } else {
            err = setupG711Codec(encoder, numChannels);
			if (OK != err)
			{
				ALOGE("[%s] setupG711Codec failed err = %d", mComponentName.c_str(), err);
			}			
        }
        ...
        if (encoder &&
            ...
            else
            {
                ...
                if(msg->findBuffer("flacinfo", &buffer))
                {
					...
					if (OK != err)
					{
						ALOGE("[%s] setParameter OMX_IndexParamAudioFlac failed err = %d", mComponentName.c_str(), err);
					}									
					...
					if (OK != err)
					{
						ALOGE("[%s] setParameter OMX_IndexParamPortDefinition failed err = %d L(%d)", mComponentName.c_str(), err, __LINE__);
					}					
					...
					if (OK != err)
					{
						ALOGE("[%s] setParameter OMX_IndexParamPortDefinition failed err = %d", mComponentName.c_str(), err);
					}					
                }
            }
#endif         			
    ...
    if (err != OK) {
		ALOGE("[%s] setup failed err = %d", mComponentName.c_str(), err);
        return err;
    }
...
status_t ACodec::setVideoPortFormatType(
    ...
    for (;;) {
        ...
        if (err != OK) {
			ALOGD("setVideoPortFormatType getParameter OMX_IndexParamVideoPortFormat failed err = %d", err);
            return err;
        }
    ...
    if (!found) {		
		ALOGE("setVideoPortFormatType failed found == false");
	...
	if (OK != err)
	{
		ALOGE("setVideoPortFormatType setParameter OMX_IndexParamVideoPortFormat failed err = %d", err);
	}
...
status_t ACodec::setupVideoDecoder(
    ...
    if (err != OK) {
		ALOGD("GetVideoCodingTypeFromMime failed err = %d", err);
        return err;
    }
    ...
    if (err != OK) {
		ALOGD("setVideoPortFormatType input failed err = %d", err);
        return err;
    }

    err = setSupportedOutputFormat();
    if (err != OK) {
		ALOGD("setSupportedOutputFormat output failed err = %d", err);
        return err;
    }
    ...
    if (err != OK) {
		ALOGD("setVideoFormatOnPort input failed err = %d", err);
        return err;
    }
    ...
    if (err != OK) {
		ALOGD("setVideoFormatOnPort output failed err = %d", err);
        return err;
    }
...
status_t ACodec::setupHEVCEncoderParameters(const sp<AMessage> &msg) {
    ...
    if (hevctype.eProfile == OMX_VIDEO_HEVCProfileBaseline) {
        ...
        hevctype.nBFrames = 0;
        hevctype.nPFrames = setPFramesSpacing(iFrameInterval, frameRate);
...
#ifndef ANDROID_DEFAULT_CODE
    // check implementation from wcp2 , leon
...
#ifdef CC_SUPPORT_TV_STAGEFRIGHT
            if (mIsPreempted && kPortIndexOutput == portIndex
                && info->mStatus == BufferInfo::OWNED_BY_DOWNSTREAM)
            {
                ALOGD("[%s] is preempted => force change Buffer %p on port %ld to us",
                        mComponentName.c_str(),
                        info->mBufferID, portIndex);
                info->mStatus = BufferInfo::OWNED_BY_US;
                continue;
            }
#endif
...
bool ACodec::BaseState::onOMXMessage(const sp<AMessage> &msg) {
    ...
    switch (type) {
        ...
        case omx_message::EMPTY_BUFFER_DONE:
        {
            ...
            ALOGD("FRM_NUM_DBG_2: EMPTY_BUFFER_DONE %d",EMPTY_BUFFER_DONE);
            #endif
        ...
        case omx_message::FILL_BUFFER_DONE:
        {
            ...
            ALOGD("FRM_NUM_DBG_2: FILL_BUFFER_DONE %d",FILL_BUFFER_DONE);
            #endif
            IOMX::buffer_id bufferID;
            CHECK(msg->findPointer("buffer", &bufferID));
...
void ACodec::BaseState::onInputBufferFilled(const sp<AMessage> &msg) {
	...
	ALOGV("[%s] onInputBufferFilled %p", mCodec->mComponentName.c_str(), bufferID);
        ...
        case RESUBMIT_BUFFERS:
        {
            if (buffer != NULL && !mCodec->mPortEOS[kPortIndexInput]) {
                ...
                if (flags & OMX_BUFFERFLAG_CODECCONFIG) {
                    ...
                    ALOGI("[%s] calling emptyBuffer %p w/ time %.2f  s",
                         mCodec->mComponentName.c_str(), bufferID, timeUs / 1E6);
#else
...
void ACodec::BaseState::onOutputBufferDrained(const sp<AMessage> &msg) {
	...
	ALOGV("[%s] onOutputBufferDrained %p", mCodec->mComponentName.c_str(), bufferID);
...
#else
    if (!mCodec->mIsPreempted)
    {
        CHECK_EQ((int)info->mStatus, (int)BufferInfo::OWNED_BY_DOWNSTREAM);
    }
    else if ((int)info->mStatus != (int)BufferInfo::OWNED_BY_DOWNSTREAM)
    {
        ALOGD("buffer(%d) with OWNED_BY_DOWNSTREAM is canceled by preemption",
            index);
        return;
    }
#endif
...
bool ACodec::UninitializedState::onAllocateComponent(const sp<AMessage> &msg) {
...
#ifdef CC_SUPPORT_TV_STAGEFRIGHT
        ALOGE("%s(L%d) componentName=%s", __FUNCTION__, __LINE__, componentName.c_str());
#endif
...
#ifdef CC_SUPPORT_TV_STAGEFRIGHT
        ALOGE("%s(L%d) mime=%s", __FUNCTION__, __LINE__, mime.c_str());
#endif
...
#ifdef CC_SUPPORT_TV_STAGEFRIGHT
                OMXCodec::kPreferHardwareCodecs |
#endif
                0,     // flags
                &matchingCodecs);
    }
    ...
    for (size_t matchIndex = 0; matchIndex < matchingCodecs.size();
        ...
        ALOGI("[Try to Allocate decoder %s]", componentName.c_str());
        status_t err = omx->allocateNode(componentName.c_str(), observer, &node);
        androidSetThreadPriority(tid, prevPriority);

        if (err == OK) {
            ALOGI("Success to allocate decoder %s", componentName.c_str());
            break;
        }
...
bool ACodec::LoadedToIdleState::onOMXEvent(
        OMX_EVENTTYPE event, OMX_U32 data1, OMX_U32 data2) {
    ALOGI("[%s] LoadedToIdleStat::onOMXEvent(%d,%d,%d)", mCodec->mComponentName.c_str(), event, data1, data2);
    switch (event) {
...
bool ACodec::IdleToExecutingState::onOMXEvent(
        OMX_EVENTTYPE event, OMX_U32 data1, OMX_U32 data2) {   
	ALOGI("[%s] IdleToExecutingState::onOMXEvent(%d,%d,%d)", mCodec->mComponentName.c_str(), event, data1, data2);
    switch (event) {
        case OMX_EventCmdComplete:
        {
            CHECK_EQ(data1, (OMX_U32)OMX_CommandStateSet);
...
bool ACodec::ExecutingState::onOMXEvent(
        OMX_EVENTTYPE event, OMX_U32 data1, OMX_U32 data2) {
	ALOGI("[%s] ExecutingState::onOMXEvent(%d,%d,%d)", mCodec->mComponentName.c_str(), event, data1, data2);
    switch (event) {
        ...
        {			
            ALOGI("[%s] ExecutingState: OMX_EventBufferFlag received\n", mCodec->mComponentName.c_str());
            return true;
        }
...
bool ACodec::OutputPortSettingsChangedState::onOMXEvent(
        OMX_EVENTTYPE event, OMX_U32 data1, OMX_U32 data2) {
    ALOGI("[%s] OutputPortSettingsChangedState::onOMXEvent(%d,%d,%d)", mCodec->mComponentName.c_str(), event, data1, data2);
    switch (event) {
        case OMX_EventCmdComplete:
        {
...
bool ACodec::ExecutingToIdleState::onOMXEvent(
    ...
    switch (event) {
        ...
        case OMX_EventBufferFlag:
        {
            ...
            ALOGI("[%s] Executing->Idle : OMX_EventPortSettingsChanged or OMX_EventBufferFlag received!", mCodec->mComponentName.c_str());
            return true;
        }
...
bool ACodec::IdleToLoadedState::onOMXEvent(
        OMX_EVENTTYPE event, OMX_U32 data1, OMX_U32 data2) {
    ALOGI("[%s] IdleToLoadedState::onOMXEvent(%d,%d,%d)", mCodec->mComponentName.c_str(), event, data1, data2);
    switch (event) {
        case OMX_EventCmdComplete:
        {
            CHECK_EQ(data1, (OMX_U32)OMX_CommandStateSet);
