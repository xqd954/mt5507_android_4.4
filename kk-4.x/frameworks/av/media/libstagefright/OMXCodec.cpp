...
#if !defined(ANDROID_DEFAULT_CODE) && defined(CC_SUPPORT_TV_STAGEFRIGHT)
const static int64_t kBufferFilledEventTimeOutNs = 50000000000LL;//set large to disable it
#else
...
#ifdef MTK_VIDEO_HEVC_SUPPORT
    if (!strncmp("OMX.mtk.", componentName, 8)) {
        return true;
    }
#endif
...
#ifdef CC_SUPPORT_TV_STAGEFRIGHT
// MTK DSP decoder > MTK SWIP decoder > google SW decoder > others
static int CompareHardwareCodecsFirst(
        const OMXCodec::CodecNameAndQuirks *elem1,
        const OMXCodec::CodecNameAndQuirks *elem2) {
    bool isOMXMTKDsp1 = !strncmp(elem1->mName.string(), "OMX.MTK.AUDIO.DECODER.DSP", 25);
    bool isOMXMTKDsp2 = !strncmp(elem2->mName.string(), "OMX.MTK.AUDIO.DECODER.DSP", 25);        
    bool isOMXMTK1 = !strncmp(elem1->mName.string(), "OMX.MTK.", 8);
    bool isOMXMTK2 = !strncmp(elem2->mName.string(), "OMX.MTK.", 8);
    bool isOMXSW1  = !strncmp(elem1->mName.string(), "OMX.google.", 11);
    bool isOMXSW2  = !strncmp(elem2->mName.string(), "OMX.google.", 11);

    if (isOMXMTKDsp1) {
        if (!isOMXMTKDsp2) { return -1; }
        return 0;
    }
...
#ifdef CC_SUPPORT_TV_STAGEFRIGHT
bool OMXCodec::isMtkHwDec(void)
{
    return !strncmp(mComponentName, "OMX.MTK.", 8);
}
#endif

// static
static bool mOggCamer;
static bool mIsSpecial;
void OMXCodec::findMatchingCodecs(
    ...
    for (;;) {
        ...
        if (!strncmp(componentName, "OMX.MTK.AUDIO.DECODER.DSP", 25)) 
        {               
            char value[PROPERTY_VALUE_MAX];
            if (property_get("omx.EnableDspDecoder", value, NULL) &&
                (!strcmp(value, "0") || !strcasecmp(value, "false"))) {
                ALOGD("Qireng : DSP Decoder disabled %s !!", componentName);
                continue;
            } else {
                ALOGD("Qireng : DSP Decoder enabled %s !!", componentName);
            }
        }
        ...
        if (((flags & kSoftwareCodecsOnly) &&   IsSoftwareCodec(componentName)) ||
            ...
            //entry->mName.string() is OMX.google.h264.decoder
            if(0==strcasecmp(entry->mName.string(), "OMX.google.h264.decoder"))
            {
                ALOGD("OMX.google.h264.decoder Sepcial case!\n");
                mIsSpecial = true;
            }
            else
            {
    ...
    if (flags & kPreferHardwareCodecs) {
        ALOGE("Prefer Hardware Codecs!\n");
        matchingCodecs->sort(CompareHardwareCodecsFirst);
    }
    
    if (flags & kSourceTypeOggCamera) 
    {
        mOggCamer = true;
    }
    else
    ...
    char value[PROPERTY_VALUE_MAX];
    if (property_get("omx.swfirst", value, NULL) &&
        (!strcmp(value, "1") || !strcasecmp(value, "true")))
    {
        ALOGE("omx.swfirst value(%s) !\n", value);
        matchingCodecs->sort(CompareSoftwareCodecsFirst);
    }
#endif
}
...
sp<MediaSource> OMXCodec::Create(
    ...
    if (mime)
        ...
        ALOGI("OMXCodec::Create mime=NULL");
    if (matchComponentName)
        ALOGI("OMXCodec::Create matchComponentName=%s", matchComponentName);
    else
        ALOGI("OMXCodec::Create matchComponentName=NULL");
#endif
    Vector<CodecNameAndQuirks> matchingCodecs;
    findMatchingCodecs(
    ...
    if(mIsSpecial)
    {
        ALOGD("Return for OMX.google.h264.decoder Sepcial case!\n");
        return NULL;
    }
#endif    
    ...
    for (size_t i = 0; i < matchingCodecs.size(); ++i) {
        ...
        ALOGI("OMXCodec::Create Attempting to allocate OMX node '%s'", componentName);
        ...
        if (!createEncoder
            ...
            if (strncmp(componentName, "OMX.SEC.", 8)) {
                ...
                ALOGW("OMXCodec::Create Component '%s' does not give the client access to "
        ...
        if (err == OK) {
            ALOGI("OMXCodec::Create Successfully allocated OMX node '%s'", componentName);
        ...
        {
            ALOGE("OMXCodec::Create fail to allocate OMX node '%s', err=%x", componentName, err);
        }
#endif
    }
...
status_t OMXCodec::configureCodec(const sp<MetaData> &meta) {
    ...
    if (!(mFlags & kIgnoreCodecSpecificData)) {
        ...
        if (meta->findData(kKeyESDS, &type, &data, &size)) {
            ...
            if (meta->findData(kKeyVorbisComment, &type, &data, &size)) {
                addCodecSpecificData(data, size);
            }
            ...
            ALOGW("Added WMVC codec specific data setting!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
            addCodecSpecificData(data, size);
        }
#endif
    }
...
#if defined(CC_SUPPORT_TV_STAGEFRIGHT) && !defined(ANDROID_DEFAULT_CODE)
    if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_FLAC, mMIME))
    {
        uint32_t type;
        typedef struct {
...
//Need aac owner review
        ...
        if (!strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.AAC")) {
            ...
            if (errAAC != OK) {
                SXLOGD("setAACFormat() failed (errAAC = %d)", errAAC);
                return errAAC;
            }
        }
        else
        ...
        if ((!strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.G711")) ||
            (!strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.DSPLPCM"))) {
            bool IsG711=false;
...
#if !defined(ANDROID_DEFAULT_CODE) && defined(CC_SUPPORT_TV_STAGEFRIGHT)
    else if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_ADPCM_MS, mMIME)
            || !strcasecmp(MEDIA_MIMETYPE_AUDIO_ADPCM_IMA, mMIME))
    {
        OMX_AUDIO_ADPCMMODETYPE ADPCMMode;
        if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_ADPCM_MS, mMIME)) {
            ADPCMMode = OMX_AUDIO_ADPCMModeMS;
        }
        else if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_ADPCM_IMA, mMIME) ) {
            ADPCMMode = OMX_AUDIO_ADPCMModeIMA;
        }
        OMX_AUDIO_PARAM_ADPCMTYPE profileADPCM;
        InitOMXParams(&profileADPCM);
        profileADPCM.nPortIndex = kPortIndexInput;
        status_t errADPCM = mOMX->getParameter(
            mNode, OMX_IndexParamAudioAdpcm, &profileADPCM, sizeof(profileADPCM));
        CHECK_EQ((status_t)OK, errADPCM);
        int32_t nChs=0, sR=0;
        int32_t block_align=0, frame_size=0;

        meta->findInt32(kKeyChannelCount, &nChs);
        meta->findInt32(kKeySampleRate, &sR);
        meta->findInt32(kKeyBlockAlign, &block_align);
        meta->findInt32(kKeyFrameSize, &frame_size);

        profileADPCM.nChannels = nChs;
        profileADPCM.nSampleRate = sR;
        profileADPCM.eAdpcmMode = ADPCMMode;
        profileADPCM.nBlockAlign = block_align;
        profileADPCM.nFrameSize = frame_size;
        errADPCM = mOMX->setParameter(
            mNode, OMX_IndexParamAudioAdpcm, &profileADPCM, sizeof(profileADPCM));
        if (errADPCM != OK) {
            CODEC_LOGE("setADPCMFormat() failed (errADPCM = %d)", errADPCM);
            return errADPCM;
        }
    }
#endif
    ...
    {
        OMX_AUDIO_PARAM_DTSTYPE profileDts;
        int32_t IsWavDts = 0;
        profileDts.nPortIndex = kPortIndexInput;
        status_t errDts = mOMX->getParameter (
                                mNode, OMX_IndexParamAudioDts, &profileDts, sizeof(profileDts));
        CHECK_EQ((status_t)OK, errDts);
        meta->findInt32(kKeyDtsInfo, &IsWavDts);
        if (IsWavDts)
        {
            ALOGE ("Is wave-dts!!!\n");
            profileDts.bIsWaveContainer = 1;
            profileDts.bEndian = 0;
        }
        else
        {
            profileDts.bIsWaveContainer = 0;
            profileDts.bEndian = 0;
        }
        errDts = mOMX->setParameter(
                            mNode, OMX_IndexParamAudioDts, &profileDts, sizeof(profileDts));
    }
#endif
#if !defined(CC_SUPPORT_TV_STAGEFRIGHT) || defined(ANDROID_DEFAULT_CODE) // TODO:  check why don't we need this block?
        ...
        )
    {
        ...
        if (meta->findData(kKeyWMAC, &type, &data, &size)) {
            ALOGE ("addCodecSpecificData for WMA");
            const uint8_t *ptr = (const uint8_t *)data;
    ...
    if (!strncasecmp(mMIME, "video/", 6)) {
        ...
        if (mIsEncoder) {
            ...
            int32_t width, height, frameRate = 0;
            ...
            //success = success && meta->findInt32(kKeyHeight, &height);
            //CHECK(success);
            err = setVideoOutputFormat(mMIME, meta);
            if (err != OK) {
                return err;
            }
        }
...
void OMXCodec::setVideoInputFormat(
...
#if defined (CC_SUPPORT_TV_STAGEFRIGHT) && !defined(ANDROID_DEFAULT_CODE)
#ifndef CC_SUPPORT_TV_STAGEFRIGHT   // Morris Yang add for RM
    } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_REAL_VIDEO, mime)) {
        compressionFormat = OMX_VIDEO_CodingRV;
#else
    } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_WMV, mime)) {
        compressionFormat = OMX_VIDEO_CodingWMV;
    } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_MPEG1, mime)) {
    ...
    switch (compressionFormat) {
        ...
        {
            CHECK_EQ(setupHEVCEncoderParameters(meta), (status_t)OK);
            break;
        }
#endif
#endif
...
#if !defined(ANDROID_DEFAULT_CODE) && defined(CC_SUPPORT_TV_STAGEFRIGHT)
#ifdef MTK_VIDEO_HEVC_SUPPORT
status_t OMXCodec::setupHEVCEncoderParameters(const sp<MetaData>& meta) {
    int32_t iFramesInterval, frameRate, bitRate;
    bool success = meta->findInt32(kKeyBitRate, &bitRate);
    success = success && meta->findInt32(kKeyFrameRate, &frameRate);
    success = success && meta->findInt32(kKeyIFramesInterval, &iFramesInterval);
    ...
    status_t err = mOMX->getParameter(
            mNode, OMX_IndexParamVideoHevc, &hevctype, sizeof(hevctype));
    CHECK_EQ(err, (status_t)OK);
    ...
    // Check profile and level parameters
    CodecProfileLevel defaultProfileLevel, profileLevel;
    defaultProfileLevel.mProfile = hevctype.eProfile;
    defaultProfileLevel.mLevel = hevctype.eLevel;
    err = getVideoProfileLevel(meta, defaultProfileLevel, profileLevel);
    if (err != OK) return err;
    hevctype.eProfile = static_cast<OMX_VIDEO_HEVCPROFILETYPE>(profileLevel.mProfile);
    hevctype.eLevel = static_cast<OMX_VIDEO_HEVCLEVELTYPE>(profileLevel.mLevel);

    // XXX

    if (hevctype.eProfile == OMX_VIDEO_HEVCProfileBaseline) {
        hevctype.nSliceHeaderSpacing = 0;
        hevctype.bUseHadamard = OMX_TRUE;
        hevctype.nRefFrames = 1;
        hevctype.nBFrames = 0;
        hevctype.nPFrames = setPFramesSpacing(iFramesInterval, frameRate);
        if (hevctype.nPFrames == 0) {
            hevctype.nAllowedPictureTypes = OMX_VIDEO_PictureTypeI;
        }
        hevctype.nRefIdx10ActiveMinus1 = 0;
        hevctype.nRefIdx11ActiveMinus1 = 0;
        hevctype.bEntropyCodingCABAC = OMX_FALSE;
        hevctype.bWeightedPPrediction = OMX_FALSE;
        hevctype.bconstIpred = OMX_FALSE;
        hevctype.bDirect8x8Inference = OMX_FALSE;
        hevctype.bDirectSpatialTemporal = OMX_FALSE;
        hevctype.nCabacInitIdc = 0;
    }

    if (hevctype.nBFrames != 0) {
        hevctype.nAllowedPictureTypes |= OMX_VIDEO_PictureTypeB;
    }

    hevctype.bEnableUEP = OMX_FALSE;
    hevctype.bEnableFMO = OMX_FALSE;
    hevctype.bEnableASO = OMX_FALSE;
    hevctype.bEnableRS = OMX_FALSE;
    hevctype.bFrameMBsOnly = OMX_TRUE;
    hevctype.bMBAFF = OMX_FALSE;
    hevctype.eLoopFilterMode = OMX_VIDEO_HEVCLoopFilterEnable;
...
status_t OMXCodec::setVideoOutputFormat(
    ...
    if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_AVC, mime)) {
        ...
        compressionFormat = OMX_VIDEO_CodingVP6;
...
void OMXCodec::setComponentRole(
    ...
    static const MimeToRole kMimeToRole[] = {
...
#if defined(CC_SUPPORT_TV_STAGEFRIGHT) && !defined(ANDROID_DEFAULT_CODE)
        { MEDIA_MIMETYPE_AUDIO_FLAC, "audio_decoder.flac", "" },
#ifdef CC_SUPPORT_TV_STAGEFRIGHT
#ifdef MTK_AUDIO_APE_SUPPORT
        { MEDIA_MIMETYPE_AUDIO_APE, "audio_decoder.ape", "" },
#endif
        { MEDIA_MIMETYPE_AUDIO_WMA, "audio_decoder.wma", "" },
        { MEDIA_MIMETYPE_AUDIO_WMAPRO, "audio_decoder.wmapro", "" },
#endif
#endif
        ...
        { MEDIA_MIMETYPE_VIDEO_WMV,
            "video_decoder.wmv", "video_encoder.wmv" },  // ToDo: check what is this for???
...
status_t OMXCodec::init() {
    ...
    err = allocateBuffers();
    if (err != (status_t)OK) {
        ALOGV("-init, allocateBuffers fail %d", err);
        return err;
    }
...
#if defined(VDEC_RGB_IOMMU)
    //add for makesure mDealer[portIndex]->allocate()(2048align) size < totalsize(2048align)
    totalSize += 4096*32;
#else
    totalSize += 1024;
#endif
...
status_t OMXCodec::allocateOutputBuffersFromNativeWindow() {
    ...
    CODEC_LOGD ("allocating %lu buffers from a native window of size %lu on output port", def.nBufferCountActual, def.nBufferSize);
...
#if !defined(ANDROID_DEFAULT_CODE) && defined(CC_SUPPORT_TV_STAGEFRIGHT)
        if ((0 != err) && (NULL != info->mMediaBuffer)){
            info->mMediaBuffer->setObserver(NULL);
            ALOGD ("@@@ info->mMediaBuffer(0x%08x)->release();", info->mMediaBuffer);
...
status_t OMXCodec::cancelBufferToNativeWindow(BufferInfo *info) {
    ...
    CODEC_LOGI("Calling cancelBuffer on buffer %p", info->mBuffer);
...
void OMXCodec::on_message(const omx_message &msg) {
#if defined(ANDROID_DEFAULT_CODE) || !defined(CC_SUPPORT_TV_STAGEFRIGHT) // remove Android 4.0 default code , Legis.
    if (mState == ERROR) {
        /*
    ...
    switch (msg.type) {
        ...
        {
            //Driver send empty,framework fill and send data to driver
            #ifdef OMXCODEC_DEBUG_SUPPORT  //For debug
            static int32_t EMPTY_BUFFER_DONE =0;
                
            ++EMPTY_BUFFER_DONE;
                
            ALOGD("FRM_NUM_DBG: EMPTY_BUFFER_DONE %d",EMPTY_BUFFER_DONE);
            #endif
        ...
        {
            //Driver send FILL,framework get data to show
            #ifdef OMXCODEC_DEBUG_SUPPORT  //For debug
            static int32_t FILL_BUFFER_DONE =0;
                
            ++FILL_BUFFER_DONE;
                
            ALOGD("FRM_NUM_DBG: FILL_BUFFER_DONE %d",FILL_BUFFER_DONE);
#endif
            ...
            if (mPortStatus[kPortIndexOutput] == DISABLING) {
                ...
                if (msg.u.extended_buffer_data.flags & OMX_BUFFERFLAG_SYNCFRAME) {
                    CODEC_LOGI("Sync Frame Received!");
                    buffer->meta_data()->setInt32(kKeyIsSyncFrame, true);
                }
                bool isCodecSpecific = false;
                if (msg.u.extended_buffer_data.flags & OMX_BUFFERFLAG_CODECCONFIG) {
                    buffer->meta_data()->setInt32(kKeyIsCodecConfig, true);
                    CODEC_LOGI("Config Frame Received!");
                ...
                if (msg.u.extended_buffer_data.flags & OMX_BUFFERFLAG_EOS) {
                    CODEC_LOGI("No more output data.");
                    mNoMoreOutputData = true;
#if defined (CC_SUPPORT_TV_STAGEFRIGHT) && !defined(ANDROID_DEFAULT_CODE)   //<---for omx component, the buffer flag is EOS, and buffer is empty.
                    SXLOGD("OMXCodec::on_message EOS received!!!!");
                    if (0==msg.u.extended_buffer_data.range_length) {
                        SXLOGD("On EOS Send Signal!!!!");
                ...
                if (mTargetTimeUs >= 0) {
#if !defined(ANDROID_DEFAULT_CODE) && defined(CC_SUPPORT_TV_STAGEFRIGHT)
                    CODEC_LOGI("Key time=%lld, Target time=%lld", msg.u.extended_buffer_data.timestamp, mTargetTimeUs);
#else
                ...
                if (0)//(!mIsEncoder && mIsVideo)
                {
                    CODEC_LOGI("FBD %lld us %d",
                        msg.u.extended_buffer_data.timestamp,
                        mFilledBuffers.size());
                }
                mBufferFilled.signal();
...
void OMXCodec::onEvent(OMX_EVENTTYPE event, OMX_U32 data1, OMX_U32 data2) {
    switch (event) {
        ...
        case OMX_EventError:
        {
            ...
            {
                ALOGE("OMXCodec::onEvent--0x%x", (OMX_S32)data1);
                setState(ERROR);
            }
#else
            setState(ERROR);
        ...
        {
            CODEC_LOGD("OMX_EventPortSettingsChanged(port=%ld, data2=0x%08lx)",
                       data1, data2);
        ...
        default:
        {
            CODEC_LOGI("EVENT(%d, %ld, %ld)", event, data1, data2);
            break;
        }
    }
}
...
void OMXCodec::onCmdComplete(OMX_COMMANDTYPE cmd, OMX_U32 data) {
    switch (cmd) {
        ...
        case OMX_CommandPortDisable:
        {
            ...
            CODEC_LOGD("PORT_DISABLED(%ld)", portIndex);

            //DTV00628005
            //CHECK(mState == EXECUTING || mState == RECONFIGURING);
            if(!(mState == EXECUTING || mState == RECONFIGURING)) {
                CODEC_LOGE("---Error handle---,Error mState = %d", (int)mState);
                return;
            }
        ...
        case OMX_CommandFlush:
        {
            ...
            if (mState == RECONFIGURING) {
                ...
                if((mState == ERROR) && (!strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.APE"))) {
                    LOGE("Dropping complete - we're in APE ERROR state.");
                    return;
                }
#endif
...
#if !defined(ANDROID_DEFAULT_CODE) && defined(CC_SUPPORT_TV_STAGEFRIGHT)
                // modify Android 4.0 default code , Legis.
                if (mState != ERROR)
#endif
                {
...
size_t OMXCodec::countBuffersWeOwn(const Vector<BufferInfo> &buffers) {
    ...
    for (size_t i = 0; i < buffers.size(); ++i) {
        ...
        if (buffers[i].mStatus == OWNED_BY_NATIVE_WINDOW)
        {
            ++m;
        }
#endif
...
#if !defined(ANDROID_DEFAULT_CODE) && defined(CC_SUPPORT_TV_STAGEFRIGHT)
    LOGE("countBuffersWeOwn CLIENT %d, US %d, WINDOW %d, not component %d", j , kk, m, n);
#endif
...
{// for source insight parsing
    ...
    for (;;) {
        ...
        if (mSeekTimeUs >= 0) {
            ...
            CODEC_LOGI("Seek Read from source, seekTime:%lld us, mode=%d", mSeekTimeUs, mSeekMode);
            ...
            if (err == OK) {
                ...
                if (srcBuffer->meta_data()->findInt64(
                        ...
                        && targetTimeUs >= 0) {
                    CODEC_LOGI("targetTimeUs = %lld us", targetTimeUs);
                ...
                CODEC_LOGI("Seek Read from source not OK: %d", err);
        ...
        else
        {
            ...
            if(err!=OK)
            {
                CODEC_LOGE("Linear Read from source: %d", err);
            }
        }
        ...
        if (err != OK) {
            ...
            mFinalStatus = err;
            mSignalledEOS = true;
            CODEC_LOGE("SignaledEOS-->mfinalStatus:%d!!", mFinalStatus);
            mBufferFilled.signal();
            break;
        }
...
#if !defined(ANDROID_DEFAULT_CODE) && defined(CC_SUPPORT_TV_STAGEFRIGHT)// wait for review
    if((mState == ERROR) && (!strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.APE")))
    {
...
status_t OMXCodec::start(MetaData *meta) {
    ...
    if (mState != LOADED) {
        ...
        ALOGV("-start, wrong state %d", mState);
        return UNKNOWN_ERROR;
    }
    ...
    FILE *fp;
    int name_index;
    CODEC_LOGD("Es Dump Start>>>>>>>\n");
    sprintf(name, "/usbdisk/%d.raw", name_index);
    ...
    fp = fopen(name, "wb");
    size_t DataIndex;
    while(DataIndex < mCodecSpecificData.size()) {
        CHECK(!(mFlags & kUseSecureInputBuffers));
        const CodecSpecificData *specific = mCodecSpecificData[DataIndex];
        size_t size = specific->mSize;
        if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_AVC, mMIME) && !(mQuirks & kWantsNALFragments)) {
            ...
            size += 4;
            CODEC_LOGD("Write AVC StartCode to %s size:%d", name, 4);
            fwrite(kNALStartCode, 1, 4, fp);
            CODEC_LOGD("Write AVC Specific Data to %s size:%d", name, specific->mSize);
            fwrite(specific->mData, 1, specific->mSize, fp);
        } else {
            CODEC_LOGD("Write Specific Data to %s size: %d", name, specific->mSize);
            fwrite(specific->mData, 1, specific->mSize, fp);
        }
        ++DataIndex;
    }
    ...
    status_t temp_err;
    while ((temp_err = mSource->read(&TempBuffer)) == OK){
        // if not getting a valid buffer from source, then exit
        if (TempBuffer == NULL) {
            ALOGE("TempBuffer == NULL");
            break;
        }
        CODEC_LOGD("Write ES to %s", name);
        fwrite(TempBuffer->data() + TempBuffer->range_offset(), 1, TempBuffer->range_length(), fp);
        TempBuffer->release();
        TempBuffer = NULL;
    }
    fclose(fp);
    CODEC_LOGD("<<<<<<<Es Dump Done\n");
#endif

    ALOGV("-start, call init");
    return init();
}
...
status_t OMXCodec::stop() {
    ...
    CODEC_LOGI("-stop stopped in state %d", mState);
    return err;
}
...
status_t OMXCodec::read(
    ...
    *buffer = NULL;
    ...
    Mutex::Autolock autoLock(mLock);
    CODEC_LOGV("autoLock(mLock) Line %d",__LINE__);
    
#ifdef CC_SUPPORT_TV_STAGEFRIGHT
    if (mIsPreempted && (mState == LOADED))
    {
        ALOGD("preempt resume!!");
        init();
        mIsPreempted = false;
    }
#endif
    ...
    if (mFilledBuffers.empty()) {
        ALOGE("mSignalledEOS :%d mFinalStatus:%d", mSignalledEOS, mFinalStatus);
        ...
        case OMX_PortDomainAudio:
        {
            ...
            if (audio_def->eEncoding == OMX_AUDIO_CodingPCM) {
                ...
                if(!strncmp(mComponentName, "OMX.MTK.AUDIO.DECODER.", 22))
                {
                    if(!strcmp(mComponentName,"OMX.MTK.AUDIO.DECODER.AAC")
                        #ifdef CC_SUPPORT_TV_STAGEFRIGHT
                        || !strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.DSPMP3")
                        || !strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.DSPAC3")
                        || !strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.DSPWMA")
                        || !strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.DSPDTS")
                        || !strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.DSPMP1")
                        || !strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.DSPAAC")
                        || !strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.DSPFLAC")
                        || !strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.DSPVORBIS")
                        || !strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.DSPLPCM")
                        || !strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.DSPADPCM")
                        || !strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.DSPPCM")
                        || !strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.DSPAMRNB")
                        || !strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.DSPAMRWB")
                        || !strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.DSPWMAPRO")
                        #endif
                        )
                    {
