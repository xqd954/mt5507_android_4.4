...
status_t LiveSession::getStreamFormat(StreamType stream, sp<AMessage> *format) {
    ...
    if (!(mStreamMask & stream)) {
        ALOGD("getStreamFormat UNKNOWN_ERROR");
    ...
    if (meta == NULL) {
        ALOGD("getStreamFormat EAGAIN");
        return -EAGAIN;
    }
    ALOGD("getStreamFormat EAconvertMetaDataToMessageGAIN");

    return convertMetaDataToMessage(meta, format);
}

void LiveSession::setUrlBeforeSeek(const char *url){
	mSeekURL = url;
}
...
status_t LiveSession::seekTo(int64_t timeUs) {
	...
	ALOGD("seekTo timeUs %lld, (%lld m: %lld s)", timeUs,TimeMin, TimeSec);
...
ssize_t LiveSession::fetchFile(
    ...
    for (;;) {
        ...
        ALOGD("readAt size (%lld)%d + %d ", size, (int)buffer->size(), (int)bufferRemaining);
        if (n < 0) {
			ALOGD("fetchfile-read error n = %d",n);
            return n;
        }
...
sp<M3UParser> LiveSession::fetchPlaylist(
    ...
    *unchanged = false;
    ...
    if (err <= 10) {
        ALOGD("fetchPlaylist size = %d URL='%s'", err,url);
		return mPlaylist;
    }
    ...
    if (curPlaylistHash != NULL && !memcmp(hash, curPlaylistHash, 16)) {
        ...
        *unchanged = true;
        ...
        ALOGV("Playlist unchanged, refresh state is now");
        ALOGE("return NULL Line %d",__LINE__);
        return NULL;
    }
