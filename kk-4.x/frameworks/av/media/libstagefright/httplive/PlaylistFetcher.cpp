...
const int32_t PlaylistFetcher::kDownloadBlockSize = -1;
...
PlaylistFetcher::PlaylistFetcher(
    ...
    : mNotify(notify),
      ...
      mNumRetries(0),
      mStartup(true),
      mNumRetriessequnumber(0),
      mNumRetriesplaylist(0),
      mPrepared(false),
      mNextPTSTimeUs(-1ll),
...
int64_t PlaylistFetcher::getSegmentStartTimeUs(int32_t seqNumber) const {
	...
	{
		ALOGE("getSegmentStartTimeUs mPlaylist->meta is null");
		return 0;
	}
...
int64_t PlaylistFetcher::delayUsToRefreshPlaylist() const {
	...
	if(mPlaylist->meta()==NULL)
	{
		ALOGE("delayUsToRefreshPlaylist mPlaylist->meta is null");
		return 0ll;
	}
...
void PlaylistFetcher::onMonitorQueue() {
    ...
    if (mPlaylist != NULL) {
		if(mPlaylist->meta()==NULL)
		{
			ALOGE("onMonitorQueue mPlaylist->meta is null");
			targetDurationUs = 3 * 1000000ll;
		}
        CHECK(mPlaylist->meta()->findInt32("target-duration", &targetDurationSecs));
    ...
    // signal start if buffered up at least the target size(fast play)
    if (!mPrepared && (bufferedDurationUs > 1000000) && downloadMore) {
        mPrepared = true;
...
void PlaylistFetcher::onDownloadNext() {
    ...
    if( mPlaylist==NULL){
		ALOGD("onDownloadNext1 return for mPlaylist is null!");
		return;
	}
	...
	if(mPlaylist->meta()==NULL)
	{
		ALOGD("onDownloadNext1 mPlaylist->meta is null!");
		if (refreshPlaylist() != OK) {
			ALOGD("onDownloadNext mPlaylist->meta is null");
			return;
		}
	}
    if (mPlaylist->meta() == NULL || !mPlaylist->meta()->findInt32(
    ...
    if (mSeqNumber < firstSeqNumberInPlaylist
        ...
        if (!mPlaylist->isComplete() && mNumRetries < kMaxNumRetries) {
            ...
            if (mSeqNumber > lastSeqNumberInPlaylist) {
				...
				if(mPlaylist->meta()==NULL)
				{
					ALOGI("postMonitorQueue mPlaylist->meta is null");
					postMonitorQueue();
					return ;
				}
                ...
                int64_t itemDurationUs;
                CHECK(itemMeta->findInt64("durationUs", &itemDurationUs));
                int64_t delayUs = mPlaylist->size() * itemDurationUs *
                        1000000ll / (1 + mNumRetries);
                if (delayUs > itemDurationUs/2) {
                    delayUs = itemDurationUs/2;
                }
    ...
    do {
        bytesRead = mSession->fetchFile(
         ...
         if (bytesRead < 0) {
            ...
            if(bytesRead == -1004)
            {
                 ALOGE("can't connect web!");
                 postMonitorQueue();
                 return;
            }
            if(bytesRead == -1001)
            {
                 ALOGE("had disconnect web!");
                 return;
            }
            if(mNumRetriesplaylist < kMaxNumRetries-3)
            {
                 ALOGE("failed to fetch .ts segment at url times =%d", mNumRetriesplaylist);
                 ++mNumRetriesplaylist;
                 postMonitorQueue();
            }
            else if(mNumRetriessequnumber < kMaxNumRetries-3)
            {
                ALOGE("failed to fetch .ts segment at url times big than twice");
                ++mSeqNumber;
                postMonitorQueue();
                mNumRetriesplaylist = 0;
                ++mNumRetriessequnumber;
            }
            else
            {
                notifyError(err);
        ...
        if (mStartup || seekDiscontinuity || explicitDiscontinuity) {
            ...
            if (mPlaylist->isComplete() || mPlaylist->isEvent()) {
				...
				ALOGV("mNextPTSTimeUs time = %lld us", mNextPTSTimeUs);
            }
        ...
        if (bufferStartsWithTsSyncByte(buffer)) {
            ...
            err = extractAndQueueAccessUnitsFromTs(tsBuffer,explicitDiscontinuity);
        }
...
int32_t PlaylistFetcher::getSeqNumberForTime(int64_t timeUs) const {
    ...
    int64_t TimeMin=timeUs/(1000000ll * 60ll);
    int64_t TimeSec=(timeUs/1000000ll)%60;
    
    ALOGI("getSeqNumberForTime seektime = %lld us.(%lld m: %lld s)", timeUs,TimeMin, TimeSec);
	if(mPlaylist->meta()==NULL)
	{
		ALOGI("getSeqNumberForTime mPlaylist->meta is null");
		firstSeqNumberInPlaylist = 0;
		return firstSeqNumberInPlaylist;
	}
    if (mPlaylist->meta() == NULL || !mPlaylist->meta()->findInt32(
                "media-sequence", &firstSeqNumberInPlaylist)) {
        firstSeqNumberInPlaylist = 0;
	...
	ALOGI("getSeqNumberForTime index = %d ", index);
...
status_t PlaylistFetcher::extractAndQueueAccessUnitsFromTs(const sp<ABuffer> &buffer
	...
	if (mTSParser == NULL) {
        ...
        LOGI("new ATSParser;");
        // Kling Wang change from ALIGNED_VIDEO_DATA to NONE for DTV00619133
    ...
    if (mNextPTSTimeUs >= 0ll) 
	{
		...
		ALOGI("extractAndQueueAccessUnitsFromTs time = %lld us", mNextPTSTimeUs);
		if(true == discontinuity)
		{
			ALOGI("DISCONTINUITY_FORMATCHANGE discontinuity (explicit=%d)", discontinuity);
			mTSParser->signalDiscontinuity(
					ATSParser::DISCONTINUITY_FORMATCHANGE, extra);
		}
		else
		{
			ALOGI("DISCONTINUITY_SEEK discontinuity (explicit=%d)", discontinuity);
		    mTSParser->signalDiscontinuity(
	...
	{
        mTSParser->signalDiscontinuity(
                ATSParser::DISCONTINUITY_NONE, NULL);
	}
        ...
        while (source->hasBufferAvailable(&finalResult)
            ...
            if (mMinStartTimeUs > 0) {
                if (timeUs < mMinStartTimeUs) {
					...
					if(mPlaylist->meta()==NULL)
					{
						ALOGI("extractAndQueueAccessUnitsFromTs mPlaylist->meta is null");
