...
status_t OggSource::read(
   ...
   if(isSeeking)
   {
#if 0   //Discard first packet
    ssize_t nlength = packet->range_length();
    LOGD("Read :%x bytes and discard",nlength);
    packet->release();
    packet = NULL;
...
#endif	
#if 0
    LOGD("Read length:%x bytes",nlength);
...
status_t MyVorbisExtractor::seekToTime(int64_t timeUs) {
    ...
    if (mTableOfContents.isEmpty() || (mTocDone == false)) {
    	  ...
    	  if(findGranulePositionofPage(mFileSize,&ts2) != OK)
    	  	return seekToOffset(0);
    	  LOGD("bitrate seek--pos:%lld,ts1:%lld,ts2:%lld",timeUs * mVi.rate /1000000,ts1,ts2);
   ...
   if(mTableOfContents.itemAt(mTableOfContents.size()-1).mTimeUs <= timeUs)
   	 return mTableOfContents.itemAt(mTableOfContents.size()-1).mPageOffset;
    off64_t os = 0,oe = 0;
    ...
    /*for(size_t i = 0 ; i< mTableOfContents.size() ;i ++)
    {
      ...
      findGranulePositionofPage(mTableOfContents.itemAt(i).mPageOffset,&ts);
    	LOGD("mTableOfContents.itemAt(%d)--mPageOffset:%lld,mTimeUs:%lld,ts:%lld",i,mTableOfContents.itemAt(i).mPageOffset,mTableOfContents.itemAt(i).mTimeUs,ts);
    }*/
    uint64_t ts1,ts2,ts;
    ...
    LOGD("seektable seek--pos:%lld,ts1:%lld,ts2:%lld,pos:%lld,ts:%lld",timeUs * mVi.rate /1000000,ts1,ts2,pos,ts);
    return seekToOffset(pos);	
#endif    
}
#if defined (CC_SUPPORT_TV_STAGEFRIGHT) && !defined(ANDROID_DEFAULT_CODE)
...
off64_t MyVorbisExtractor::findAccuratePageOffset(uint64_t samples,off64_t ofStart,off64_t ofEnd)
{
	  ...
	  LOGD("ofStart:%lld,sgranulePosition:%lld,ofEnd:%lld,egranulePosition:%lld,samples:%lld",ofStart,sgranulePosition,ofEnd,egranulePosition,samples);
	  ...
	  {
	    LOGD("offsetGuess is abnormal,return the start offset");
	    return ofStart;
	  }
	  else
    LOGD("offsetGuess:%lld,granulePositionGuess:%lld",offsetGuess,granulePositionGuess); 
...
status_t MyVorbisExtractor::readNextPacket(MediaBuffer **out) {
    *out = NULL;
    ...
    for (;;) {
        ...
        {
            mCurrentPageSamples = 0;
            LOGD("revise the timestamp to page granule position");
        }
#endif
    ...
    switch (type) {
        case 1:
        {
            ...
            if(mVi.channels > 2)
            {
                ...
                LOGE("Vorbis SWIP does not enable multi channel");
                return ERROR_UNSUPPORTED;
#endif
#else
                LOGE("Tremolo does not support multi channel");
                return ERROR_UNSUPPORTED;
#endif
            }
#endif 
...
OggExtractor::OggExtractor(const sp<DataSource> &source)
    : mDataSource(source),
      mInitCheck(NO_INIT),
      mImpl(NULL) {
    LOGI("OggExtractor::OggExtractor()");
...
#if defined (CC_SUPPORT_TV_STAGEFRIGHT) && !defined(ANDROID_DEFAULT_CODE)
    if (mInitCheck != OK)
       LOGE("MyVorbisExtractor init error");
#endif
}
...
bool SniffOgg(
    ...
    if (source->readAt(0, tmp, 4) < 4 || memcmp(tmp, "OggS", 4)) {
        LOGI("SniffOgg(%s) return false", mimeType->string());
        return false;
    }

    mimeType->setTo(MEDIA_MIMETYPE_CONTAINER_OGG);
    *confidence = 0.2f;
    LOGI("SniffOgg(%s) return true(%f)", mimeType->string(), *confidence);
    return true;
}
