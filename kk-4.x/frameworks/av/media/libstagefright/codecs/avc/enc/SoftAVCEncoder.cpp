...
//Function Convert RGBA_8888 to RGB_888
//Output color format of Open GL is RGBA_8888
inline static void ConvertARGB8888ToRGB(
uint8_t *srcbuffer, uint8_t *dstbuffer, int32_t width, int32_t height){
    int32_t x,y,i,j;
    
    if((srcbuffer == NULL) || (dstbuffer == NULL))
    {
        ALOGE("ConvertARGB8888ToRGB:Buffer is NULL");
    }
...
//Function Convert RGB_888 to YUV420Planar
#define MY(a,b,c) (( a*0.277479  + b*0.529059  + c*0.102135 + 7.6 )) //7.6 //7.6 //7.6
#define MU(a,b,c) (( a*(-0.16245) + b*(-0.30553) + c*0.439084 + 132)) //132 //132 //131.9444
#define MV(a,b,c) (( a*0.474741  + b*(-0.32253) + c*(-0.06498) + 115)) //115 //116 //116.4082

#define DY(a,b,c) (MY(a,b,c) > 255 ? 255 : (MY(a,b,c) < 0 ? 0 : MY(a,b,c)))
#define DU(a,b,c) (MU(a,b,c) > 255 ? 255 : (MU(a,b,c) < 0 ? 0 : MU(a,b,c)))
#define DV(a,b,c) (MV(a,b,c) > 255 ? 255 : (MV(a,b,c) < 0 ? 0 : MV(a,b,c)))

inline static void ConvertRGBToYUV420Planar(
uint8_t *srcbuffer, uint8_t *dstbuffer, int32_t width, int32_t height){
    int32_t i,j,s,x,y;
    uint8_t *Y = NULL;
    uint8_t *U = NULL;
    ...
    if((srcbuffer == NULL) || (dstbuffer == NULL))
    {
        ALOGE("ConvertRGBToYUV420: Buffer is NULL");
    }

    Y = dstbuffer;
    U = dstbuffer + width*height;
    V = U + ((width*height)>>2);
    //ALOGE("Y_Addr: 0x%x, U_Addr: 0x%x, V_Addr: 0x%x", Y, U, V);
    ...
    for(y=0; y < height; y++)
    {
        for(x=0; x < width; x++)
        {
            j = y*width + x;
            i = j*3;
            Y[j] = (uint8_t)(DY(srcbuffer[i], srcbuffer[i+1], srcbuffer[i+2]));

            if(x%2 == 1 && y%2 == 1)
            {
                U[s] = (uint8_t)
                        ((DU(srcbuffer[i  ], srcbuffer[i+1], srcbuffer[i+2]) + 
                        DU(srcbuffer[i-3], srcbuffer[i-2], srcbuffer[i-1]) +
                        DU(srcbuffer[i  -width*3], srcbuffer[i+1-width*3], srcbuffer[i+2-width*3]) +
                        DU(srcbuffer[i-3-width*3], srcbuffer[i-2-width*3], srcbuffer[i-1-width*3]))/4);
                
                V[s] = (uint8_t)
                        ((DV(srcbuffer[i  ], srcbuffer[i+1], srcbuffer[i+2]) + 
                        DV(srcbuffer[i-3], srcbuffer[i-2], srcbuffer[i-1]) +
                        DV(srcbuffer[i  -width*3], srcbuffer[i+1-width*3], srcbuffer[i+2-width*3]) +
                        DV(srcbuffer[i-3-width*3], srcbuffer[i-2-width*3], srcbuffer[i-1-width*3]))/4);
                s++;
            }
        }
    }
    //ALOGE("s: 0x%x", s); //s is used to count U/V address, it should be in [0,width*height/4)
}
...
OMX_ERRORTYPE SoftAVCEncoder::initEncParams() {
    ...
    if(mVideoColorFormat == OMX_COLOR_Format32bitARGB8888){
        // Color conversion is needed.
        // ARGB to RGB, then RGB to YUV420 Planar
        if(mInputFrameData == NULL){
            mInputFrameData = (uint8_t *)malloc(mVideoWidth * mVideoHeight * 3);
        }
        CHECK(mInputFrameData != NULL);

        if(mInputFrameData_YUV420 == NULL){
            mInputFrameData_YUV420 =
                (uint8_t *) malloc((mVideoWidth * mVideoHeight * 3 ) >> 1);
        }
        CHECK(mInputFrameData_YUV420 != NULL);
    }
    else if(mVideoColorFormat == OMX_COLOR_FormatYUV420SemiPlanar) {
        // Color conversion is needed.
        // YUV420 SemiPlanar to YUV420 Planar
        if(mInputFrameData_YUV420 == NULL){
            mInputFrameData_YUV420 =
                (uint8_t *) malloc((mVideoWidth * mVideoHeight * 3 ) >> 1);
        }
...
void SoftAVCEncoder::initPorts() {
    ...
    const size_t kOutputBufferSize = (352*288*3)>>1; //Max resolution Baseline Level2.0 can support
...
OMX_ERRORTYPE SoftAVCEncoder::internalSetParameter(
    ...
    switch (indexFull) {
        ...
        case OMX_IndexParamPortDefinition:
        {
            ...
            err = SimpleSoftOMXComponent::internalSetParameter(index, params);
        ...
        case kStoreMetaDataExtensionIndex:
        {
            ...
            if (mStoreMetaDataInBuffers) {
                // Actually it's ABGR_8888 from open GL surface
                mVideoColorFormat = OMX_COLOR_Format32bitARGB8888;
                if (mInputFrameData == NULL) {
                    mInputFrameData =
                            (uint8_t *) malloc(mVideoWidth * mVideoHeight * 3);
                }
...
OMX_ERRORTYPE SoftAVCEncoder::internalSetPortParams(
        const OMX_PARAM_PORTDEFINITIONTYPE* port) {
    if (port->nPortIndex == 0) {
        mVideoWidth = port->format.video.nFrameWidth;
        mVideoHeight= port->format.video.nFrameHeight;
        ...
        if ((port->format.video.eColorFormat == OMX_COLOR_FormatYUV420Planar) ||
            ...
            mVideoColorFormat = port->format.video.eColorFormat;
        } else {
        ...
        def->format.video.eColorFormat = port->format.video.eColorFormat;

        return OMX_ErrorNone;
    } else if (port->nPortIndex == 1) {
        if ((port->format.video.eCompressionFormat != OMX_VIDEO_CodingAVC) ||
            (port->format.video.eColorFormat != OMX_COLOR_FormatUnused)) {
            return OMX_ErrorUnsupportedSetting;
        }

        mVideoBitRate = port->format.video.nBitrate;
        return OMX_ErrorNone;
    } else {
        ...
        if (mReadyForNextFrame) {
            ...
            if (inHeader->nFilledLen > 0) {
                ...
                if (mVideoColorFormat != OMX_COLOR_FormatYUV420Planar) {
                    if(mStoreMetaDataInBuffers){
                        ConvertARGB8888ToRGB(
                            inputData, mInputFrameData, videoInput.pitch, videoInput.height);
                        ConvertRGBToYUV420Planar(
                            mInputFrameData, mInputFrameData_YUV420, videoInput.pitch, videoInput.height);
                        inputData = mInputFrameData_YUV420;
                    }
                    else{
                        ConvertYUV420SemiPlanarToYUV420Planar(
                            inputData, mInputFrameData_YUV420, mVideoWidth, mVideoHeight);
                        inputData = mInputFrameData_YUV420;
                    }
                }
