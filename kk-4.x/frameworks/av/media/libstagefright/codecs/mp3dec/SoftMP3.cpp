    ...
    while (!inQueue.empty() && !outQueue.empty()) {
        ...
        if (inHeader->nFilledLen == 0) {
...
#ifdef DEBUG_AUDIO_DUMP    
            {
                ...
                property_get("debug.audio.softmp3", value, "0");

                if (atoi(value)) 
                {
                    FILE* fp = fopen("/tmp/mp3_in.mp3","ab");
                    if (fp)
                    {
                        fwrite(inHeader->pBuffer, 1, inHeader->nOffset, fp);
                        fclose(fp);
                    }
                }
            }
...
#ifdef DEBUG_AUDIO_DUMP    
        {
            ...
            if (atoi(value)) 
            {
                FILE* fp = fopen("/tmp/mp3_out.pcm","ab");
                if (fp)
                {
                    fwrite(outHeader->pBuffer, 1, outHeader->nFilledLen, fp);
                    fclose(fp);
                }
            }
        }
