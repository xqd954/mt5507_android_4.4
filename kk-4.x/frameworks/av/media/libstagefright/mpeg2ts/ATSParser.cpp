    ...
    if (extra != NULL) {
        ...
        if (extra->findInt64("hls-FirstTimeUs", &timeUs)) {
            ...
            if (mPendingTimestamp.pendingtimes > 0) {
                ...
                ALOGD("mFirstTimestamp = %lld", timeUs);
...
status_t ATSParser::Program::parseProgramMap(ABitReader *br) {
    ...
    /*CHECK_EQ(table_id, 0x02u); handle ts data error*/
    if (table_id != 0x02u) {
        ALOGD("PMT data error!");
    ...
    /*CHECK_EQ(section_syntax_indicator, 1u); handle TS data error*/
    ...
    while (infoBytesRemaining > 0) {
		...
		if ((ES_info_length & 0xc00) != 0u) {
			ALOGE("%s(%d) data error", __FUNCTION__, __LINE__);
			return ERROR_MALFORMED; 
		}
        ...
        while (info_bytes_remaining >= 2) {
			...
			/*STREAMTYPE_PRIVATE_DATA may indicate different stream types
			 descriptor tag should be checked*/
		 	if (streamType == STREAMTYPE_PRIVATE_DATA) {
				switch (descTag) {
					case 0x6A:
					case 0x7A: {/*EAC3*/
						/*combine two AC3 Type values to one*/
						streamType = STREAMTYPE_AC3_AUDIO;
						ALOGV("change Type from %02X to %02X",
							STREAMTYPE_PRIVATE_DATA, STREAMTYPE_AC3_AUDIO);
						br->skipBits(descLength * 8);
						break;
					}
					case 0x05: {/*need to check descriptor data*/
						/*0x48455643 is format_identifier value of HEVC
						 designated by ISO/IEC JTC 1/SC 29*/
						if (0x48455643 == br->getBits(32)) {
							#ifdef MTK_VIDEO_HEVC_SUPPORT
							streamType = STREAMTYPE_HEVC;
							ALOGV("change Type from %02X to %02X",
							STREAMTYPE_PRIVATE_DATA, STREAMTYPE_HEVC);
							#endif
						}
...
void ATSParser::Program::notifyFlush() {
    if (mPendingTimestamp.us != -1) {
        ...
        if (mPendingTimestamp.pendingtimes == 0) {
            ...
            mPendingTimestamp.us = -1;
            mFirstPTSValid = false;
            ALOGD("reset firstPTS, firstTimestamp = %lld", mFirstTimestamp);
        } else {
            ALOGD("wait another stream to flushed, pendingtimes = %d", mPendingTimestamp.pendingtimes);
        } 
    }
}
#endif
int64_t ATSParser::Program::convertPTSToTimestamp(uint64_t PTS) {
    if (!(mParser->mFlags & TS_TIMESTAMPS_ARE_ABSOLUTE)) {
        ...
        {    
		   ...
		   if ((PTS*20) < mFirstPTS) {
		       ALOGD("[%s] mFirstPTS:%lld changed to %lld", __FUNCTION__, mFirstPTS, PTS);
			   mFirstPTS = PTS;
		   }
...
#ifndef ANDROID_DEFAULT_CODE
        /*check if timeUs should be modified after discontinuity*/
		ALOGV("[%s]----mAbsoluteTimeAnchorUs:%lld, timeUs:%lld", __FUNCTION__,
		mParser->mAbsoluteTimeAnchorUs, timeUs);

		int64_t threshold = mParser->mAbsoluteTimeAnchorUs/9ll;
		if ((mParser->mAbsoluteTimeAnchorUs - timeUs) > threshold) {
...
//      mFirstPTS(0xFFFFFFFF),
   //   mFirstPTSValid(false),
	...
	case STREAMTYPE_AC3_AUDIO:
            mQueue = new ElementaryStreamQueue(ElementaryStreamQueue::AC3);
            break;
		...
		case STREAMTYPE_DTS_AUDIO:
			mQueue = new ElementaryStreamQueue(ElementaryStreamQueue::DTS);
            break;
#endif
        ...
        case STREAMTYPE_AVS_VIDEO:
            mQueue = new ElementaryStreamQueue(
                ElementaryStreamQueue::AVS_VIDEO);
            break;
...
status_t ATSParser::Stream::parsePES(ABitReader *br) {
    ...
    if (stream_id != 0xbc  // program_stream_map
        ...
        if (PTS_DTS_flags == 2 || PTS_DTS_flags == 3) {
            ...
            /*CHECK_EQ(br->getBits(4), PTS_DTS_flags); for ts data error handling*/
            if (br->getBits(4) != PTS_DTS_flags) {
                ALOGD("PES data Error!");
                return ERROR_MALFORMED;
            }
...
#ifndef ANDROID_DEFAULT_CODE
				if((int64_t)PTS - (int64_t)DTS > 270000)
				{
					PTS = DTS;
					ALOGV("Check PTS = %llu, DTS = %llu", PTS,DTS);
				}
#endif
...
void ATSParser::parseProgramAssociationTable(ABitReader *br) {
    unsigned table_id = br->getBits(8);
    ALOGV("  table_id = %u", table_id);
    /*CHECK_EQ(table_id, 0x00u); handle TS data error*/
    if (table_id != 0x00u) {
        ALOGD("PAT data error!");
        return ;
    }
