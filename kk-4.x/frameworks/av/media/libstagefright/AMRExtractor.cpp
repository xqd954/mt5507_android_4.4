...
bool SniffAMR(
    ...
    if (source->readAt(0, header, sizeof(header)) != sizeof(header)) {
        LOGI("SniffAMR(%s) return false(line=%d)", mimeType->string(), __LINE__);
        return false;
    }
    ...
    if (!memcmp(header, "#!AMR\n", 6)) {
        ...
        *confidence = 0.5;
        ...
        LOGI("SniffAMR(%s) return true #!AMR (%f)", mimeType->string(), *confidence);
        return true;
    } else if (!memcmp(header, "#!AMR-WB\n", 9)) {
        *mimeType = MEDIA_MIMETYPE_AUDIO_AMR_WB;
        *confidence = 0.5;

        LOGI("SniffAMR(%s) return true #!AMR-WB (%f)", mimeType->string(), *confidence);
        return true;
    }
