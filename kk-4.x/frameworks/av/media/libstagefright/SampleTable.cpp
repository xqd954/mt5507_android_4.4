...
#ifndef ANDROID_DEFAULT_CODE
		if (U32_AT(buffer) < 1) {
			LOGE("Chunk index is less than 1 in stsc");
			return ERROR_MALFORMED;
		}
#else
...
status_t SampleTable::setSampleSizeParams(
	...
	LOGD("setSampleSizeParams(type=0x%x, data_offset=%lld, data_size=%d)", type, data_offset, data_size);
	...
	LOGD("version=%d, mDefaultSampleSize=%d, mNumSampleSizes=%d)", U32_AT(header), mDefaultSampleSize, mNumSampleSizes);
	...
	LOGD("sampleCount=%d", mNumSampleSizes);
...
status_t SampleTable::findSampleAtTime(
	...
	LOGD("findSampleAtTime: req_time=%d, flags=%d", req_time, flags);
    ...
    *sample_index = mSampleTimeEntries[closestIndex].mSampleIndex;
	LOGD("findSampleAtTime: closestIndex=%d, *sample_index=%d", closestIndex, *sample_index);
...
status_t SampleTable::findSyncSampleNear(
    ...
    *sample_index = x;
	LOGD("findSyncSampleNear: start_sample_index=%d, *sync_sample_near_index=%d", start_sample_index, *sample_index);
    return OK;
}
