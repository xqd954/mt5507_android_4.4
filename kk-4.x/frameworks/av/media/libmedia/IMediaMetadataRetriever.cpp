...
//MTK Begin
    status_t  setThumbnail(bool IsThumbnail)
    {
        Parcel data, reply;
        data.writeInterfaceToken(IMediaMetadataRetriever::getInterfaceDescriptor());
        data.writeInt32((int)IsThumbnail);
        remote()->transact(SET_THUMBNAIL_MODE, data, &reply);
        return reply.readInt32();
    }

    status_t  setRetriveverSvctxPath(const char *pu1_svctx_path)
    {
...
//MTK Begin
        case SET_THUMBNAIL_MODE: {
            CHECK_INTERFACE(IMediaMetadataRetriever, data, reply);
            bool isThumbnail = (bool)data.readInt32();
            reply->writeInt32(setThumbnail(isThumbnail));
            return NO_ERROR;
        } break;
        case SET_SVCTX_PATH: {
            ...
            reply->writeInt32(setRetriveverSvctxPath(pu1_svctx_path));
            return NO_ERROR;
        } break;
//MTK End		
        default:
