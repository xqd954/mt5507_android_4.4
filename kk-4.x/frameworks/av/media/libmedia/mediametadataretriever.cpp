...
//MTK Begin
status_t MediaMetadataRetriever::setThumbnail(bool IsThumbnail)
{
    LOGI("MediaMetadataRetriever::setThumbnail");
    Mutex::Autolock _l(mLock);
    if (mRetriever == 0) {
        ALOGE("retriever is not initialized");
        return NULL;
    }
    return mRetriever->setThumbnail(IsThumbnail);
}

status_t MediaMetadataRetriever::setRetriveverSvctxPath(const char *pu1_svctx_path){
    if (mRetriever == NULL) {
        ALOGE("retriever is not initialized");
        return NO_INIT;
    }
