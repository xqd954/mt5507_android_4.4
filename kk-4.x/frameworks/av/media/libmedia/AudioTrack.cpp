...
#define DEBUG_AUDIO_DUMP

#ifdef DEBUG_AUDIO_DUMP
...
ssize_t AudioTrack::write(const void* buffer, size_t userSize)
{
	...
	property_get("debug.audio.track01", value, "0");	
	if (atoi(value)) {
		char fileName[PATH_MAX];

		// dump Track info
		snprintf(fileName, PATH_MAX, "/tmp/track01_%08X.inf", mSessionId);
		...
		// dump audio data
		snprintf(fileName, PATH_MAX, "/tmp/track01_%08X.pcm", mSessionId);
		int fd = ::open(fileName, O_WRONLY | O_CREAT | O_APPEND, 0777);
		if (fd != -1) {
			ALOGI("line %d: debug.audio.track01::write %d\n", __LINE__, userSize);
			::write(fd, buffer, userSize);
			::close(fd);
		}
	}
#endif
...
#ifdef DEBUG_AUDIO_DUMP
		char value[PROPERTY_VALUE_MAX];
		property_get("debug.audio.track02", value, "0");
		if (atoi(value)) {			
			ALOGI("line %d: mCbf(%d/%d)", __LINE__, writtenSize, reqSize);
			char fileName[PATH_MAX];

			// dump Track info
			snprintf(fileName, PATH_MAX, "/tmp/track02.inf");
			if (access(fileName, F_OK) != 0) {
				int fd = ::open(fileName, O_WRONLY | O_CREAT, 0777);
				if (fd != -1) {
					Vector<String16> args;
			...
			// dump audio data
			snprintf(fileName, PATH_MAX, "/tmp/track02.pcm");
			int fd = ::open(fileName, O_WRONLY | O_CREAT | O_APPEND, 0777);
			if (fd != -1) {
				ALOGI("line: %d debug.audio.track02::write %d", __LINE__, audioBuffer.size);
				::write(fd, audioBuffer.raw, audioBuffer.size);
				::close(fd);
			}
		}
#endif
