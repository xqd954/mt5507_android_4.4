...
{
    return new MediaProfiles::VideoEncoderCap(
        VIDEO_ENCODER_HEVC, /*250000*/75000, 9000*1000, 96, 
        1280, 96, 720, 15, 30);
}
#endif
#endif
...
MediaProfiles::createDefaultExportVideoProfiles(MediaProfiles *profiles)
{
    ...
    profiles->mVideoEditorExportProfiles.add(
        new ExportVideoProfile(VIDEO_ENCODER_HEVC,
            OMX_VIDEO_HEVCProfileBaseline, OMX_VIDEO_HEVCLevel13));
#endif
#endif
}
...
{
	// MTK add for camera from /etc/media_profiles.xml
	char value[PROPERTY_VALUE_MAX];
	property_get("ro.mtk.isSupportHwVenc", value, "0");
	if('0' == value[0]){					
		ALOGV("Set the media_profiles.xml cameraId=1 because of not support Hw Venc.\n");
		cameraId = 1;
	}else{
		ALOGV("The camera vieo recording support HW Venc.\n");
	}
