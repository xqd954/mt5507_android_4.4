...
int CameraClient::getOrientation(int degrees, bool mirror) {
	...
	#if 1
	/* MTK_CAMERA For CTS must set the facing=back, but in the normal use need set the facing=front,
	* If set the facing=back, the picute will be as the mirror image. 
	* If you hand is left but in the preview the hand is right.
	* Now Camera HAL set the cameraInfo->facing = BACK forever. if the CTS_TEST=true, the video need do a mirror.
	* sys_build/mtk_android/mtxx_android/Makefile
	* kk-4.x/device/mtk/mt53xx/system.mk
	* mediatek/device/libcamera/android.mk, 
	* MtkCameraHal.cpp/HAL_getCameraInfo()
	* kk-4.x/frameworks/av/services/camera/libcameraservice/api1/CameraCLient.cpp/getOrientation()
	*/	
	/*	ro.mtk.camera.facingback	  CTS_TEST		   cameraInfo->facing		  mirror		actually_display
	*			   '0'				   false			 CAMERA_FACING_BACK		  true		 front(mirror)
	*			   '1'				   true		       CAMERA_FACING_BACK		  false			back
	*/

	char value[PROPERTY_VALUE_MAX];
	property_get("ro.mtk.camera.facingback", value, "0");
    if('0' == value[0]){					
		//	MTK HAL set the Facing is BACK, but the actually using is Front, so we need mirror it by handler.
		mirror = 1;
    }else{
