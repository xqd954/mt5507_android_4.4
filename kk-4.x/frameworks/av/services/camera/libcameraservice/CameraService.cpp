...
void CameraService::onFirstRef()
{
    ...
    if (hw_get_module(CAMERA_HARDWARE_MODULE_ID,
		...
		// MTK_CAMERA for dynamic detect camera whether is existed or not.
...
int32_t CameraService::getNumberOfCameras() {
	...
	if(!mModule)
	{
		ALOGE("Camera HAL module not loaded, return num = 0");
		mNumberOfCameras = 0;
		return 0;
	}
	mNumberOfCameras = mModule->get_number_of_cameras();
	ALOGI("getNumberOfCameras num=%d", mNumberOfCameras);
