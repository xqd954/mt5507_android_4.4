...
// MTK_TEST: start : non-AOSP: TODO: review unshare(CLONE_NEWNS)
	#ifdef HAVE_ANDROID_OS
	    if (prctl(PR_SET_NO_NEW_PRIVS, 1, 0, 0, 0) < 0) 
            {
                if (errno == EINVAL) 
                {
                    SLOGE("MT_TEST: PR_SET_NO_NEW_PRIVS failed. "
                 ...
                 {
                     SLOGE("MT_TEST: PR_SET_NO_NEW_PRIVS failed: %s", strerror(errno));
                     return -1;
                 }
            }
            else 
            {
                SLOGE(" !!! MT_TEST: prctl(PR_SET_NO_NEW_PRIVS): ok !!! ");
                SLOGE(" !!! MT_TEST: prctl(PR_SET_NO_NEW_PRIVS): ok !!! ");
                SLOGE(" !!! MT_TEST: prctl(PR_SET_NO_NEW_PRIVS): ok !!! ");
            }
