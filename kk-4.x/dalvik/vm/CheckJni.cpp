...
static jint Check_AlreadyOneWithVM(JavaVM* vm, JNIEnv** p_env) {
    ScopedCheck sc(false, __FUNCTION__);
    sc.check(true, "vp", vm, p_env);
    return CHECK_JNI_EXIT("I", baseVm(vm)->AlreadyOneWithVM(vm, p_env));
}
