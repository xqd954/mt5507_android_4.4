...
static inline int wpa_drv_set_ptk(struct wpa_supplicant *wpa_s,
				   u8 *ptk, int ptk_len)
{
	if (!wpa_s->driver->set_ptk)
		return -1;
	return wpa_s->driver->set_ptk(wpa_s->drv_priv, ptk, ptk_len);
}
static inline int wpa_drv_set_replay_counter(struct wpa_supplicant *wpa_s,
					   u8 *replay_counter, int replay_counter_len)
{
	if (!wpa_s->driver->set_replay_counter)
		return -1;
	return wpa_s->driver->set_replay_counter(wpa_s->drv_priv, replay_counter, replay_counter_len);
}
