...
static void wpas_p2p_group_remove_readd(void *eloop_ctx, void *timeout_ctx);
#endif
				 ...
				 enum p2p_group_removal_reason removal_reason)
{
	...
	case P2P_GROUP_REMOVAL_FREQ_CONFLICT:
		...
		break;
	case P2P_GROUP_REMOVAL_AND_READD:
		reason = " reason=GROUP_READD";		
		break;
...
{
#ifdef CONFIG_DISALLOW_P2P_GO_FIND
    struct wpa_supplicant *orig_wpa_s = wpa_s;
    for (wpa_s = wpa_s->global->ifaces; wpa_s; wpa_s = wpa_s->next) {
        struct wpa_ssid *ssid = wpa_s->current_ssid;
        if (ssid == NULL)
            continue;
        ...
        {
            wpa_printf(MSG_INFO, "AP-like interface with mode %d detected for %s", ssid->mode, wpa_s->ifname);
            wpa_s = orig_wpa_s;
            wpas_p2p_stop_find_oper(wpa_s);
            return -1;
        }
    ...
    wpa_s = orig_wpa_s;
#endif /* CONFIG_DISALLOW_P2P_GO_FIND */
...
static void wpas_p2p_group_remove_readd(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_printf(MSG_DEBUG, "P2P: GO Frequency conflict - terminate group THEN re-add it");
	wpas_p2p_group_delete(wpa_s, P2P_GROUP_REMOVAL_AND_READD);
}
	...
	for (iface = wpa_s->global->ifaces; iface; iface = iface->next) {
		if ((iface->current_ssid) &&
			...
			if(!wpas_is_p2p_prioritized(iface)) {
				...
				if ((iface->p2p_group_interface == P2P_GROUP_INTERFACE_GO)  ||
			    (iface->current_ssid->mode == WPAS_MODE_P2P_GO)) {
			    	/*just for GO to remove and then add it again by framework*/
					eloop_register_timeout(0, 0, wpas_p2p_group_remove_readd,
						       iface, NULL);
				}
				else{
				...
				if (wpa_s == iface)
					...
					/*update GO operation channle following AP channle any*/
       					u8 reg_class = 0, new_channel = 0, cfg_op_channel = 0;
        				unsigned int r;
       					//extern int p2p_freq_to_channel(unsigned int freq, u8 *reg_class, u8 *channel);
				        struct p2p_data *p2p = wpa_s->global->p2p;        
        
			            if (freq)
			            {
/*
			                    !p2p_set_listen_channel(p2p, reg_class, new_channel))
			                    wpa_printf(MSG_DEBUG, "Set P2P Listen Channel to AP Channel (%d-ch%d)\n",
			                               freq, new_channel);
*/
			                p2p_freq_to_channel(freq, &reg_class, &new_channel);
