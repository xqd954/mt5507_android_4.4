		...
		const u8 *bssid, const u8 *ie, size_t ie_len)
{
...
#ifdef CONFIG_MTK_HE_SUPPORT
        p2p_dbg(p2p, "%s: not sent(Not-In-Listen)", __FUNCTION__);
#endif
	...
	if (ieee802_11_parse_elems((u8 *) ie, ie_len, &elems, 0) ==
	    ParseFailed) {
#ifdef CONFIG_MTK_HE_SUPPORT
        p2p_dbg(p2p, "%s: not sent(Malformed)", __FUNCTION__);
#endif
	...
	if (elems.p2p == NULL) {
#ifdef CONFIG_MTK_HE_SUPPORT
        p2p_dbg(p2p, "%s: not sent(NotP2P)", __FUNCTION__);
#endif
...
#ifdef CONFIG_MTK_HE_SUPPORT
        p2p_dbg(p2p, "%s: not sent(Not-To-Me)", __FUNCTION__);
#endif
...
#ifdef CONFIG_MTK_HE_SUPPORT
        p2p_dbg(p2p, "%s: not sent(Not-Wildcard-Bssid)", __FUNCTION__);
#endif
	...
	if (elems.ssid == NULL || elems.ssid_len != P2P_WILDCARD_SSID_LEN ||
	    ...
	    0) {
#ifdef CONFIG_MTK_HE_SUPPORT
        p2p_dbg(p2p, "%s: not sent(Not-P2PWildcard-Ssid)", __FUNCTION__);
#endif
...
#ifdef CONFIG_MTK_HE_SUPPORT
        p2p_dbg(p2p, "%s: not sent(11b-Rates-Only)", __FUNCTION__);
#endif
...
#ifdef CONFIG_MTK_HE_SUPPORT
        p2p_dbg(p2p, "%s: not sent(Parse-P2P-Failed)", __FUNCTION__);
#endif
...
#ifdef CONFIG_MTK_HE_SUPPORT
        p2p_dbg(p2p, "%s: not sent(DeviceId-Not-Match)", __FUNCTION__);
#endif
...
#ifdef CONFIG_MTK_HE_SUPPORT
        p2p_dbg(p2p, "%s: not sent(DeviceType-Not-Match)", __FUNCTION__);
#endif
...
#ifdef CONFIG_MTK_HE_SUPPORT
        p2p_dbg(p2p, "%s: not sent(Force-Not-Send)", __FUNCTION__);
#endif
