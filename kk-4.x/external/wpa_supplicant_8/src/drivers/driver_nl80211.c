...
/* WIPHY_FLAG_HAVE_AP_SME only available in kernel version 3.3 and above */
#ifndef CONFIG_FORCE_AP_SME_SUPPORT
				      ...
				      void *global_priv)
{
	...
	if (drv->rfkill == NULL) {
		...
		if (!os_strncmp(bss->ifname, "p2p-ap0-", 8))
			drv->device_ap_sme = 1;
#endif
                                ...
                                size_t buf_len )
{
    ...
    if (os_strcasecmp(cmd, "STOP") == 0) {
        ...
        wpa_msg(drv->ctx, MSG_INFO, WPA_EVENT_DRIVER_STATE "HANGED");
#ifdef CONFIG_P2P_AUTO_GO_AS_SOFTAP
		} else if (os_strncasecmp(cmd, "P2P_GO_SOFTAP ", 14) == 0) {
				int htbw = 0;
				htbw = atoi(cmd+14);
				ret = linux_set_p2p_go_softap(drv->global->ioctl_sock, bss->ifname, htbw);
		...
		*/
		...
		unsigned char IP[16];
		int i = 0, cmd_length = os_strlen(cmd);
		i = 11;
		os_strlcpy(IP,cmd+11,16);
		if(!os_strncmp(IP,"any",3))
		{
			ret = linux_set_wow_ip(drv->global->ioctl_sock, bss->ifname, &IP, 3);
            ret = os_snprintf(buf, buf_len,
                "Set IP to be any\n");
		}
		else
		{
			ret = linux_set_wow_ip(drv->global->ioctl_sock, bss->ifname, &IP, cmd_length - 11);
			ret = os_snprintf(buf, buf_len,
                "Set IP to %s\n",IP);
		}
...
#endif
#ifdef CONFIG_WOW_SUPPORT
	.set_ptk = wpa_driver_nl80211_set_ptk,
	.set_replay_counter = wpa_driver_nl80211_set_replay_counter,
#endif /* CONFIG_WOW_SUPPORT */
#ifdef ANDROID
