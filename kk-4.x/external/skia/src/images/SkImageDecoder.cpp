...
bool SkImageDecoder::decode(SkStream* stream, SkBitmap* bm,
	...
	#ifdef CC_MTK_ANDROID_AUTO_TEST
	{
		...
		char* bitmap = NULL;
		bm->lockPixels();
		bitmap = (char*)bm->getAddr(0,0);
		if (bitmap == NULL)
		{
			__android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, "bitmap is null");
			bm->unlockPixels();
			return true;
		}
		...
		__android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, "image_format:%d, checkSum:0x%x", fmt, checkSum);
	}
	#endif
    return true;
}

bool SkImageDecoder::decodeMtkGif(SkStream* stream, mtkSkBitmap* bm,
                            SkBitmap::Config pref, Mode mode) {
    // pass a temporary bitmap, so that if we return false, we are assured of
    // leaving the caller's bitmap untouched.
    bm->setFrameNumber(1);
    ...
    if (!this->onMtkDecodeGif(stream, bm, mode)) {
        return false;
    }
...
bool SkImageDecoder::decodeAnimationGif(SkStream* stream, mtkSkBitmap* bm,
                            SkBitmap::Config pref, Mode mode, int frameIdx) {
    // pass a temporary bitmap, so that if we return false, we are assured of
    // leaving the caller's bitmap untouched.
    ...
    if (!this->onMtkDecodeAnimationGif(stream, bm, mode, frameIdx)) {
        return false;
    }
