    ...
    do {
        ...
        case IMAGE_DESC_RECORD_TYPE: {
            ...
            if ((imageTop | imageLeft) > 0 ||
                ...
                sampler = SkScaledBitmapSampler(innerWidth, innerHeight, sampleSize);
...
bool SkImageDecoder::onMtkDecodeGif(SkStream* sk_stream, mtkSkBitmap* bm, Mode mode) {
#if GIFLIB_MAJOR < 5
		GifFileType* gif = DGifOpen(sk_stream, DecodeCallBackProc);
#else
		GifFileType* gif = DGifOpen(sk_stream, DecodeCallBackProc, NULL);
		...
		if (gif->SWidth * gif->SHeight > 7000*7000)
	    {
	        SkDebugf("GIF : Too large dimension (%d %d)", gif->SWidth, gif->SHeight);

	        return error_return(*bm, "Pixels > 24M");
	    }
		...
		do {
			...
			case IMAGE_DESC_RECORD_TYPE: {
				...
				if (SkImageDecoder::kDecodeBounds_Mode == mode) {
					gif->SavedImages[gif->ImageCount-1].ImageDesc.delay_time = delay;
	                if (DGifSkipContent(gif) != 0)
	                {
	                    return error_return(*bm, "outOfMemory");
	                }
					break;
				}
...
#if GIFLIB_MAJOR < 5
            if (temp_save.Function == 0xF9)/* graphic control extension */
#else
			if (extFunction == 0xF9)/* graphic control extension */		
#endif

				{
					delay = (extData[2] + (extData[3] << 8)) * 10;
				}
		...
		SkDebugf("[gif] It is gif, frame_count=%d.\n", gif->ImageCount);
		if (gif->ImageCount > 1)
	    {
	        if (gif->ImageCount > 100)
	        {
	            return error_return(*bm, "Image count is more than 100");
	        }
	        bm->setFrameNumber(gif->ImageCount);

	        int * frameDelay = (int*)malloc(gif->ImageCount * sizeof(int));
	        for (int i = 0; i < gif->ImageCount; i++)
	        {
	            frameDelay[i] = (gif->SavedImages[i].ImageDesc.delay_time < 50) ? 100 : gif->SavedImages[i].ImageDesc.delay_time;
	        }
	        bm->setFrameDelayArray(frameDelay);
	        free(frameDelay);
	    }
...
bool SkImageDecoder::onMtkDecodeAnimationGif(SkStream* sk_stream, mtkSkBitmap* bm, Mode mode, int frameIdx) {
#if GIFLIB_MAJOR < 5
		GifFileType* gif = DGifOpen(sk_stream, DecodeCallBackProc);
#else
		GifFileType* gif = DGifOpen(sk_stream, DecodeCallBackProc, NULL);
	    ...
	    if (gif->SWidth * gif->SHeight > 500 * 500)
	    {
	        SkDebugf("GIF : Too large dimension (%d %d)", gif->SWidth, gif->SHeight);
		...
		GifByteType ui1_byte, ui1_dispose_method;
		...
		do {
			...
			case IMAGE_DESC_RECORD_TYPE: {
				...
				image->ImageDesc.dispose_method = ui1_dispose_method;
            	image->ImageDesc.transparent_idx = transpIndex;
            	image->ImageDesc.transparent_flag = TransparentFlag;
			...
			case EXTENSION_RECORD_TYPE:
...
#endif
				{
					...
					ui1_dispose_method = ((extData[1] & 0x1C) >> 2);
					transpIndex = extData[4];
					TransparentFlag = (extData[1] & 0x01);
				}
