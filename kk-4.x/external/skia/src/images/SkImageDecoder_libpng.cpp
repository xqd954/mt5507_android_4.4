...
DECODE_AGAIN:
    if((fgHWDecode) && (fgPngIsFree()) && (fgPngGetHwInstance()))
	{
		...
		if (ret == false)
		{
			...
			SkDebugf("*****Error*****Png HW decoder error, try software!!!!\n");
			fgHWDecode = false;
			sk_stream->rewind();
			goto DECODE_AGAIN;
		}
		...
		if((hasAlpha) || ((config != SkBitmap::kARGB_8888_Config) && (config != SkBitmap::kRGB_565_Config)&& (config != SkBitmap::kARGB_4444_Config)))
		{
			/* 
			  *1. using 8888 if have alpha
			  *2. if preconfig is not 565 or not 4444, using 8888 
			  */
		...
		{
			osdMode = JPG_GETIMG_OSDMODE_ARGB4444;
		}
