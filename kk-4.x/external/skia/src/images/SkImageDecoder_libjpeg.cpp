...
bool SkJPEGImageDecoder::onDecode(SkStream* stream, SkBitmap* bm, Mode mode) {
    ...
    SkBitmap::Config cfg = this->getPrefConfig(k32Bit_SrcDepth, false);
    ...
    */
	...
	if (sampleSize == 1 && mode == SkImageDecoder::kDecodeBounds_Mode) {
        cinfo.fgforceswmode = TRUE;
         //SkDebugf("Ondecode color space = %d\n",cinfo.out_color_space);
         SkJpegDebugf("$$$onDecode cinfo.fgforceswmode = TRUE\n");
    }
...
bool SkJPEGImageDecoder::onDecodeSubset(SkBitmap* bm, const SkIRect& region) {
    ...
    */
                ...
                cinfo->out_color_space == JCS_RGB_565)))
    {
        ...
        INTJ32 const bpr = bitmap.rowBytes();
        int rowTotalCount = 0;
