...
static boolean sk_fill_input_buffer(j_decompress_ptr cinfo) {
	...
	if(cinfo->fghwinforead)
	{
		if (src->next_input_byte == NULL && src->bytes_in_buffer == 0)//special case
		{
			bytes = src->fStream->getLength();
		}
		else
		{
			//SkDebugf("read to jpeg  mmap addr............");
		 	bytes = src->fStream->read((void*)src->next_input_byte, src->bytes_in_buffer);
		}
	}
	else
	{
		//SkDebugf("read to fBuffer  addr............");
...
void skjpeg_de_error_exit(j_common_ptr cinfo) {
