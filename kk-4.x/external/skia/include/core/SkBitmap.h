...
class SK_API mtkSkBitmap : public SkBitmap{
public:
    mtkSkBitmap() {frameNumber = 1;}
    int getFrameNumber() { return frameNumber;}
    int* getFrameDelay() {return frameDelayArray;}
    /*
    * Set if it is animation gif
    */
    void setFrameNumber(int frame) {frameNumber = frame;}

    void setFrameDelayArray(int* frameDelay) {for (int i = 0; i < frameNumber; i++) frameDelayArray[i] = frameDelay[i];}
    
private:
    int         frameNumber;
    int         frameDelayArray[100];
};
