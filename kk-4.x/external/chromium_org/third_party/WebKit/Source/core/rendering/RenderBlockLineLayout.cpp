...
void RenderBlockFlow::layoutRunsAndFloatsInRange(LineLayoutState& layoutState, InlineBidiResolver& resolver, const InlineIterator& cleanLineStart, const BidiStatus& cleanLineBidiStatus, unsigned consecutiveHyphenatedLines)
{
    ...
    while (!endOfLine.atEnd()) {
        ...
        if (layoutState.lineInfo().isEmpty()) {
            ...
            /// M:
            isText = endOfLine.object() ? endOfLine.object()->isText() : false;
            bidiRuns.deleteRuns();
            resolver.markCurrentRunEmpty(); // FIXME: This can probably be replaced by an ASSERT (or just removed).
        ...
        if (doTextWrap && hasTextToWrap) {
            ...
            if (!isConstrained) {
                ...
                if (textWrapWidth > 0 && width() > (textWrapWidth + padding)) {
                    ...
                    setWidth(min(width().ceil(), maxWidth));
                    m_minPreferredLogicalWidth = min(m_minPreferredLogicalWidth.ceil(), maxWidth);
                    m_maxPreferredLogicalWidth = min(m_maxPreferredLogicalWidth.ceil(), maxWidth);
