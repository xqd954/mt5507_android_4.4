        ...
        if (cWidth > 2 && cHeight > 2) {
            ...
            if (!m_altText.isEmpty()) {
                ...
                if (errorPictureDrawn) {
                    if (cWidth >= textWidth && fontMetrics.height() <= imageOffset.height())  /// M: use cWidth instead of usableWidth
                        context->drawText(font, textRunPaintInfo, textOrigin);
                } else if (cWidth >= textWidth && usableHeight >= fontMetrics.height())  /// M: use cWidth instead of usableWidth
                    context->drawText(font, textRunPaintInfo, textOrigin);
            }
        }
