...
void WMLSelectElement::dispatchFocusEvent(Element* oldFocusedElement, FocusDirection direction)
{
    SelectElement::dispatchFocusEvent(m_data, this);
    WMLFormControlElement::dispatchFocusEvent(0, FocusDirectionPage);
}
...
RenderObject* WMLSelectElement::createRenderer(RenderStyle*)
{
    ...
    return new RenderWMLListBox(this);
    /// @}
}

/// M: @{
bool WMLSelectElement::childShouldCreateRenderer(const Node& child) const
{
    if (!m_data.usesMenuList())
        return child.hasTagName(WMLNames::optionTag) || isWMLOptGroupElement(&child);
    return false;
}
/// @}
