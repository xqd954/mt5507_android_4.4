...
public class ContentViewCore
    ...
    private void updateFrameInfo(
        ...
        if (mUpdateFrameInfoListener != null) {
            mUpdateFrameInfoListener.onFrameInfoUpdated(pageScaleFactor);
        }
        /// @}
    }
    /// M: added for CTS testFindNext @{
    ...
    private void onSelectionBoundsChanged(Rect anchorRectDip, int anchorDir, Rect focusRectDip,
            int focusDir, Rect inputBounds, boolean isAnchorFirst) {
        ...
        if (x1 != x2 || y1 != y2 ||
            ...
            /// M: hide selection handles while out of input bounds @{
            boolean startIsOut = false;
            boolean endIsOut = false;
            if (!inputBounds.isEmpty()) {
                int offsetY = inputBounds.height() >> 1;
                if (isAnchorFirst) {
                    startIsOut = !inputBounds.contains(x1, y1 - offsetY);
                    endIsOut = !inputBounds.contains(x2, y2 - offsetY);
                } else {
                    startIsOut = !inputBounds.contains(x2, y2 - offsetY);
                    endIsOut = !inputBounds.contains(x1, y1 - offsetY);
                }
            }
            getSelectionHandleController().setOutOfInputBounds(startIsOut, endIsOut);
            /// @}
