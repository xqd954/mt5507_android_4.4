...
SynchronousCompositorImpl::SynchronousCompositorImpl(WebContents* contents)
    : compositor_client_(NULL),
      ...
      contents_(contents),
      input_handler_(NULL),
      /// M: add has_updated_frame
      has_updated_frame_(false)
      {
  DCHECK(contents);
}
...
void SynchronousCompositorImpl::UpdateFrameMetaData(
  ...
  /// M: defer the DidUpdateContent until UpdateFrameMetaData @{
  if (!has_updated_frame_) {
    has_updated_frame_ = true;
    LOG(INFO)<<"[UpdateFrameMetadata] ["<<this<<"]["<<gettid()<<"] retrigger DidActivatePendingTree";
...
void SynchronousCompositorImpl::DidActivatePendingTree() {
   /// M: defer the DidUpdateContent until UpdateFrameMetaData @{
   if (!has_updated_frame_) {
	   LOG(INFO)<<"[DidActivatePendingTree] ["<<this<<"]["<<gettid()<<"] defer DidUpdateContent";
	   return;
   }
	/// @}
