...
#include "ui/events/keycodes/keyboard_codes_posix.h" // for keycode
...
void RenderWidget::OnHandleInputEvent(const blink::WebInputEvent* input_event,
  ...
  if (WebInputEvent::isKeyboardEventType(input_event->type)) {
    ...
    uint32_t FLAG_FROM_SYSTEM = 0x08;
    char flagValue[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("webviewchromium.tv_enterKeyFlag", flagValue, "0");
    int enterKeyFlag = atoi(flagValue);
    LOG(ERROR) << "[ESTALENT] enterKeyFlag: " << enterKeyFlag;

    if ((key_event.windowsKeyCode == ui::VKEY_RETURN) &&
        ((enterKeyFlag & FLAG_FROM_SYSTEM) != 0)) {
        ui::TextInputType _type = GetTextInputType(); // must edit
        LOG(ERROR) << "[ESTALENT] TextInputType: " << _type;

        if (_type == ui::TEXT_INPUT_TYPE_NONE || 
            _type == ui::TEXT_INPUT_TYPE_DATE || 
            _type == ui::TEXT_INPUT_TYPE_DATE_TIME ||
            _type == ui::TEXT_INPUT_TYPE_DATE_TIME_LOCAL || 
            _type == ui::TEXT_INPUT_TYPE_MONTH ||
            _type == ui::TEXT_INPUT_TYPE_TIME || 
            _type == ui::TEXT_INPUT_TYPE_WEEK) {
            prevent_default = WillHandleKeyEvent(key_event);
        } else {
            LOG(ERROR) << "[ESTALENT] enterKey and editable input";
            if(key_event.type == WebInputEvent::KeyUp) {
               LOG(ERROR) << "[ESTALENT] enter keyUp, resetInputMethod and show IME";
               resetInputMethod();
...
void RenderWidget::UpdateSelectionBounds() {
  ...
  if (selection_anchor_rect_ != params.anchor_rect ||
    ...
    selection_focus_rect_ = params.focus_rect;
    /// M: input field bounds
    params.input_bounds = webwidget_->textInputInfo().inputBounds;
