...
#undef XLOG
#define XLOG(...) android_printLog(ANDROID_LOG_DEBUG, "WebMediaPlayerAndroid", __VA_ARGS__)
...
void WebMediaPlayerAndroid::load(LoadType load_type,
  ...
  XLOG("WebMediaPlayerAndroid::load() type=%d,url=%s,cors_mode=%d",load_type,url.string().utf8().data(),cors_mode);
...
void WebMediaPlayerAndroid::play() {
  XLOG("WebMediaPlayerAndroid::play()");
#if defined(VIDEO_HOLE)
...
void WebMediaPlayerAndroid::pause() {
  XLOG("WebMediaPlayerAndroid::pause()");
  pause(true);
}
...
void WebMediaPlayerAndroid::seek(double seconds) {
  ...
  XLOG("WebMediaPlayerAndroid::seek(%f)", seconds);
...
void WebMediaPlayerAndroid::setVolume(double volume) {
  XLOG("WebMediaPlayerAndroid::setVolume(%f)", volume);
...
void WebMediaPlayerAndroid::OnVideoSizeChanged(int width, int height) {
  ...
  /// M: If duration is 0, means it's live video.
  ///    since the initial video size of live streaming is 0x0,
  ///    we notify player client size is changed. Client can update the status@{
  if (duration() == 0)
      client_->sizeChanged();
  /// @}
}
...
void WebMediaPlayerAndroid::OnTimeUpdate(const base::TimeDelta& current_time) {
  ...
  char auto_test[PROPERTY_VALUE_MAX];
  memset(auto_test, 0, sizeof(auto_test));

  property_get("mtk.auto_test", auto_test, NULL);

  // If current time is great than 5000 ms, determine the html5 video auto test is ok
  if ((0 == strcmp(auto_test, "1")) && (current_time_ > 5)) {
    XLOG("[MTK_WEBKIT] Browser_html5_video_is_OK");
  }
...
void WebMediaPlayerAndroid::enterFullscreen() {
  XLOG("WebMediaPlayerAndroid::enterFullscreen()");
...
void WebMediaPlayerAndroid::exitFullscreen() {
  XLOG("WebMediaPlayerAndroid::exitFullscreen()");
