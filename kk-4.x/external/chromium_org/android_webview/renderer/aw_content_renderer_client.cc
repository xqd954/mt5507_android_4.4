...
void AwContentRendererClient::GetNavigationErrorStrings(
  ...
  if (error_description) {
    if (error.localizedDescription.isEmpty())
      /// M: empty the description. The error message will be generated in WebViewContentsClientAdpter
      *error_description = ASCIIToUTF16(""); //ASCIIToUTF16(net::ErrorToString(error.reason));
    else
