...
#undef XLOG
#define XLOG(...) android_printLog(ANDROID_LOG_DEBUG, "InProcessViewRenderer", __VA_ARGS__)
...
void RequestProcessGLOnUIThread() {
  ...
  if (!renderer || !renderer->RequestProcessGL()) {
    LOG(ERROR) << "Failed to request GL process. Deadlock likely: "
               << "renderer=[" << renderer << "] AllowGL=[" << ScopedAllowGL::IsAllowed() << "]";
  }
}
...
void InProcessViewRenderer::CalculateTileMemoryPolicy() {
  ...
  char lowMemoryMode[128];
  memset(lowMemoryMode, 0, sizeof(lowMemoryMode));

  property_get("mtk.browser.lowMemoryMode", lowMemoryMode, NULL);

  if (0 == strcmp(lowMemoryMode, "1")) {
    g_memory_multiplier = 8;
  }
#endif
...
#if 1 //def MTK_WEBKIT_FIX
  //char lowMemoryMode[PROPERTY_VALUE_MAX];
  //memset(lowMemoryMode, 0, sizeof(lowMemoryMode));

  //property_get("mtk.browser.lowMemoryMode", lowMemoryMode, NULL);

  if (0 == strcmp(lowMemoryMode, "1")) {
      const char kDefaultTileSize[] = "256";
...
void InProcessViewRenderer::TrimMemory(int level) {
  ...
  XLOG("InProcessViewRenderer::TrimMemory");
