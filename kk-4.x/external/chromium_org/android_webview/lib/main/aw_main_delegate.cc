...
#undef XLOG
#define XLOG(...) android_printLog(ANDROID_LOG_DEBUG, "AwMainDelegate", __VA_ARGS__)
...
bool AwMainDelegate::BasicStartupComplete(int* exit_code) {
  ...
  /// M: Enable WebGL for non-LCA project @ {
#if defined(MTK_LCA_RAM_OPTIMIZE) || defined(MTK_LCA_ROM_OPTIMIZE)
  ...
  cl->AppendSwitch(switches::kDisableWebRTC);
	...
	if (0 == strcmp(lowMemoryMode, "1")) {
	  XLOG("disable WebGL for low memory mode");
	  cl->AppendSwitch(switches::kDisableExperimentalWebGL);
	}
#else
	/// M: Enable WebGL for SQC @ {
	//cl->AppendSwitch(switches::kDisableExperimentalWebGL);
	/// @ }
#endif
