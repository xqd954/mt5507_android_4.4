...
static const char* const platform_media_codecs[] = {
    // MPEG-4
    "mp4v",
...
/// M: platform media type @{
  for (size_t i = 0; i < arraysize(platform_media_types); ++i) {
    ...
    non_image_map_.insert(platform_media_types[i]);
  }
/// @}
...
/// M: platform media codec@{
  for (size_t i = 0; i < arraysize(platform_media_codecs); ++i)
    codecs_map_.insert(platform_media_codecs[i]);
/// @}
...
/// M: platform media types/codecs @{
  for (size_t i = 0; i < arraysize(platform_media_types); ++i) {
    ...
    non_image_map_.erase(platform_media_types[i]);
  }
  for (size_t i = 0; i < arraysize(platform_media_codecs); ++i)
    codecs_map_.erase(platform_media_codecs[i]);
/// @}
}
