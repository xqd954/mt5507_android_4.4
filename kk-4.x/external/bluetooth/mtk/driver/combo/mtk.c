...
int write_comm_port(int fd, unsigned char *buffer, unsigned long len)
{
    ...
    while (bytesToWrite > 0){
    	LOG_DBG("write(fd=%d, buffer=%p, bytestowrite=%d)", fd, buffer, bytesToWrite);
        nWritten = write(fd, buffer, bytesToWrite);
        LOG_DBG("write ret : %d", nWritten);
        if (nWritten < 0){
            LOG_DBG("erno=%d", errno);
...
int read_comm_port(int fd, unsigned char *buffer, unsigned long len)
{
    ...
    LOG_DBG("read(fd=%d, buffer=%p, bytestowrite=%d)\n", fd, buffer, bytesToRead);
    nRead = read(fd, buffer, bytesToRead);
    LOG_DBG("read return %d", nRead);
    if (nRead < 0){
    	LOG_DBG("errno=%d\n", errno);
        if(errno == EINTR || errno == EAGAIN)
            return 0;
