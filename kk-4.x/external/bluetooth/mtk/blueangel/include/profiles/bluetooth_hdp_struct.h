...
typedef enum
{
	BT_CONNECTION_TYPE_CONTROL = 0x00,
	BT_CONNECTION_TYPE_DATA 	= 0x01,
	BT_CONNECTION_TYPE_INVALID = 0xFF,
} bt_connection_type;
...
typedef enum 
{
	BT_HDP_DATA_CONN_TYPE_DEFAULT			= 0x00,	//based on main channels
	BT_HDP_DATA_CONN_TYPE_INDEX			= 0x01,	//based on index
	BT_HDP_DATA_CONN_TYPE_MDL_ID			= 0x02,	//based on MDL ID
} bt_hdp_data_CONN_type;
...
typedef struct 
{
	...
	bt_addr_struct 	bdaddr;
	U8				con_type;
	U8				data_chn_type;     //optional, valid when conn type is data channel type
	U32				value;    		   //optional, valid when conn type is data channel type
...
typedef struct 
{
	LOCAL_PARA_HDR
	bt_addr_struct 	bdaddr;
	U8				data_chn_type;     //bt_hdp_data_conn_type
	U16				value;
