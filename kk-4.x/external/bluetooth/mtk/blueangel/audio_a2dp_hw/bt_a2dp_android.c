...
static BtStatus A2DP_Android_SendMessageToItself(msg_type msg_id, ilm_struct * ilm, U16 size);
static BtStatus A2DP_Android_Send_StopCnf_ToItself (VOID);
...
void a2dp_cleanup(void *ptr)
{
	...
	bt_a2dp_data->is_cleanup = TRUE;
        ...
        {//send STOP fail
			...
			A2DP_Android_Send_StopCnf_ToItself();
            return;
        }
    }
	//else
...
static BtStatus a2dpdata_handle_message(BT_A2DP_DATA_CTX *bt_a2dp_data, ilm_struct *message)
{
	...
	case MSG_ID_BT_A2DP_STREAM_DATA_START_CNF:
            if (bt_a2dp_data->is_streaming == FALSE)
            {
...
static BtStatus A2DP_Android_SendMessageToItself(msg_type msg_id, ilm_struct * ilm, U16 size)
{
	...
	BT_A2DP_DATA_CTX *data_ctx = (BT_A2DP_DATA_CTX *)btA2dpAdress;
	socket_make_sockaddr_un(BT_SOCK_NAME_EXT_ADP_A2DP_DATA, ANDROID_SOCKET_NAMESPACE_ABSTRACT,(struct sockaddr_un*)&address,&address_len);
	bt_ext_err("[A2DP][A2DP_Android_SendMessageToItself] socket address:%s",address.sun_path);
	/* Set src_mod to MOD_MMI */
    ilm->src_mod_id = MOD_MMI;
    ...
    // cal whole ilm size
    size = sizeof(ilm_struct) - MAX_ILM_BUFFER_SIZE + size;
	if(data_ctx->srvcsock >= 0)
	{
		ret = sendto(data_ctx->srvcsock,ilm,size,0,(const struct sockaddr_un *)&address,address_len);
		if(ret < 0)
        {
            bt_ext_err("[A2DP][A2DP_Android_SendMessageToItself]sendto fail : %s, %d", strerror(errno), errno);
			return BT_STATUS_FAILED;
        }
        else
        {
            bt_ext_log("[A2DP][A2DP_Android_SendMessageToItself]sendto success : %d", ret);
			return BT_STATUS_SUCCESS;
        }
	}
	else
	{		
        bt_ext_err("[A2DP][A2DP_Android_SendMessageToItself]server socket uninitialized");
		return BT_STATUS_FAILED;
	}
...
static BtStatus A2DP_Android_Send_StopCnf_ToItself (VOID)
{
    ...
    bt_ext_log("[A2DP] Send Stop Confirm to Itself!");
    return A2DP_Android_SendMessageToItself(MSG_ID_BT_A2DP_STREAM_DATA_STOP_CNF,ilmptr, sizeof(bt_a2dp_stream_data_stop_cnf_struct));
}
