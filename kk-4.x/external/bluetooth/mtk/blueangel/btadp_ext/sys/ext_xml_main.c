...
//    FHANDLE fh;
	...
	if( FALSE != btmtk_fs_is_folder_exist(CONST_XML_FOLDER) )
    {
 ...
 static kal_int32 xml_process_cdata(XML_PARSER_STRUCT *parser, kal_int32 length)
{
    ...
    while (i > 0)   /* i is used as the rest length of content data */
    {
        kal_int32 len_read = 0, len_write;
        ...
        ret = btmtk_fs_write_ucs2(handle, parser->ap_temp, len_read, (kal_uint32*)&len_write);
        if (ret < 0)
        {
            free_buffer(parser, parser->ap_temp);
            parser->ap_temp = NULL;
...
{   
    ...
    if (parser->source != 0)
    {
        ...
        btmtk_fs_get_position(parser->source, &pos);
        return pos;
    }
    else
        return 0;
