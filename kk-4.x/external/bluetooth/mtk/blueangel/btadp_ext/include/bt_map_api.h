...
typedef struct
{
	...
	MBT_CHAR					Subject[MBT_MAP_MAX_MSG_LIST_ATTR_LEN + 1];		// [M] Title, the first words of the message, or "" (This length shall be used according to the requested value in GetMessagesListing)
	MBT_CHAR					DateTime[MBT_MAP_MAX_DATETIME_LEN + 1];			// [M] The sending time or the reception time in format "YYYYMMDDTHHMMSS"
	MBT_CHAR					SenderName[MBT_MAP_MAX_MSG_LIST_ATTR_LEN + 1];	// [C]
	MBT_CHAR					SenderAddr[MBT_MAP_MAX_MSG_LIST_ATTR_LEN + 1];	// [C] The senders email address or phone number
	MBT_CHAR					ReplyToAddr[MBT_MAP_MAX_MSG_LIST_ATTR_LEN + 1];	// [C] This shall be used only for emails to deliver the sender's reply-to email address.
	MBT_CHAR					RecipientName[MBT_MAP_MAX_MSG_LIST_ATTR_LEN + 1];	// [C] The recipient's email address, a list of email addresses, or phone number
	MBT_CHAR					RecipientAddr[MBT_MAP_MAX_MSG_LIST_ATTR_LEN + 1];	// [M] If the recipient is not known this may be left empty.
...
{
	MBT_BYTE					Mode;
	MBT_CHAR					Time[MBT_MAP_MAX_DATETIME_LEN + 1]; // "yyyymmddTHHMMSSZ", or "" if none
	MBT_UINT					Size;
	MBT_CHAR					Name[MBT_MAX_FILE_NAME_LEN + 1];	
...
{	
	...
	MBT_CHAR					FolderPath[MBT_MAP_MAX_FOLDER_PATH_LEN + 1]; // The folder name including the path
	// [<bmessage-originator>]*
...
{	// Response Out
	...
	union
	{
		...
		struct	// StorageType = MBT_MAP_STORAGE_TYPE_FILE or MBT_MAP_STORAGE_TYPE_RAW_FILE
		{
			...
			MBT_UINT			Size;
			MBT_CHAR			FileName[MBT_MAX_FILE_NAME_LEN + 1]; // The file name including the full path
		} File;
	} Data;
} MBT_MAP_GET_MSG_RSP;
...
typedef struct
{
	...
	MBT_CHAR					FolderPath[MBT_MAP_MAX_FOLDER_PATH_LEN + 1];
	MBT_CHAR					OldFolderPath[MBT_MAP_MAX_FOLDER_PATH_LEN + 1];
	MBT_MAP_MSG_TYPE			MsgType;
