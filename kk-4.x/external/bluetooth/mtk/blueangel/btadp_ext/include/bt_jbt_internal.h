...
typedef struct _jbt_subsession {
	...
	kal_uint8	rxbuf[JSR82_GET_DATA_SHARED_BUFFER_SIZE];
	kal_uint8	txbuf[JSR82_PUT_DATA_SHARED_BUFFER_SIZE];
#ifdef __USE_BT_RING_BUF_API__
...
typedef struct{
	...
	kal_uint32 twin_session_id; //valid for server role
	...
	//sdp 
	JSR82_SDP_REGISTRATION  sdpinfo;
	JSR82_SDP_QUERY			queryinfo;
...
typedef struct 
{
	...
	U16 			monitorid;
	JBT_SPP_CNTX 	spp;
	JBT_L2CAP_CNTX 	l2cap;

	Session_Mutex	*mutex;
} JBT_JSR82_CNTX;

void jbt_l2cap_init(JBT_L2CAP_CNTX *);
void jbt_l2cap_deinit(JBT_L2CAP_CNTX *);

void jbt_spp_init(JBT_SPP_CNTX *);
void jbt_spp_deinit(JBT_SPP_CNTX *);
...
jbt_subsession *jbt_find_subsession_entry(kal_uint8 session_type, U8 session_inx, kal_uint16 subsession_id);
...
void jbt_jsr82_session_deregistered(jbt_session_info *session);
