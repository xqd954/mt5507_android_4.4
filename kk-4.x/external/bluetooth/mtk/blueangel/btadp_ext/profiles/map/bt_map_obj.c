...
extern wise_map_cntx_struct g_pi_map_cntx;
...
static const char *btmtk_map_xml_get_bool_string(MBT_BOOL value)
{
    switch(value)
    {
    case KAL_TRUE:
...
{
    return ext_fs_write(handle, MAP_XML_HEADER, strlen(MAP_XML_HEADER));
}
...
//    BTMTK_ASSERT(count <= MAP_ADP_BUFFER_SIZE);
    ...
    count = ext_fs_write(handle, buf, count);
    return count;
}
                                                        ...
                                                        S32 seq_write_fhandle)
{
    ...
    else if ((fhandle = ext_fs_open((const U8*)filename, EXT_FS_APPEND)) < 0)
    {
        return 0;
    }
    ...
    for (count = 0; count < FolderNum; count++)
    {
        ...
        sprintf(n_buf, "%lu", list[count].Size);
        attr_list[i++] = n_buf;
...
{
    char output[BT_MAP_MAX_TEMP_FILE_NAME_LEN + 1];
    S32 fhandle, seq_write_fhandle;
    S32 entry_size = sizeof(MBT_MAP_FOLDER_LIST_ENTRY), index = 0, total;
    U32 count, size = 0;

    sprintf(output, BT_MAP_FOLDER_LIST_FILE, mas_index);

    if ((seq_write_fhandle = ext_fs_open((const U8*)output, EXT_FS_APPEND)) < 0)
    {
        return 0;
    }
    
    if ((fhandle = ext_fs_open((const U8*)filename, EXT_FS_READ_ONLY)) < 0)
    {
        return 0;
    }
    ...
    {
        if (ext_fs_read(fhandle, g_pi_map_cntx.file_buffer, entry_size) == entry_size)
        {
    ...
    ext_fs_close(fhandle);
    ext_fs_close(seq_write_fhandle);
    return size;
}
                                                    ...
                                                    S32 seq_write_fhandle)
{
    ...
    //    sprintf(n_buf, "%llu", list[count].MsgHandle);
   	sprintf(n_buf, "%llx", list[count].MsgHandle);
        ...
        /* size [M] */
        attr_list[i++] = MAP_XML_ATTR_SIZE;
        sprintf(n_buf2, "%lu", list[count].OriginMsgSize);
        attr_list[i++] = n_buf2;
        /* text [O] */
        ...
        sprintf(n_buf3, "%lu", list[count].AttachSize);
        attr_list[i++] = n_buf3;
        /* priority [O] */
...
U32 btmtk_map_xml_create_message_list_by_file(S8 mas_index, char *filename, U32 filesize)
{
    ...
    sprintf(output, BT_MAP_MESSAGE_LIST_FILE, mas_index);
...
U32 btmtk_map_xml_create_event_report(S8 mas_index, MBT_MAP_SEND_EVENT_REQ *event)
{
    ...
    sprintf(filename, BT_MAP_EVENT_REPORT_FILE, mas_index);
    if ((fhandle = ext_fs_open((const U8*)filename, EXT_FS_READ_WRITE | EXT_FS_CREATE)) < 0)
    {
        return 0;
    }
...
static U8 btmtk_vcard_cmp_field(S8 *srcString, const S8 *refString)
{
    ...
    if (btmtk_util_strnicmp(srcString, (S8*) refString, refLen))
    {
        return FALSE;
    }
    else
...
static S32 btmtk_map_vcard_get_single_line(S8 *buf, S32 fh)
{
    ...
    nBytes = ext_fs_read(fh, buf, VCARD_MAX_LINE_BUFF_SIZE - 1);
	...
	/*in this case, strlen is smaller than VCARD_MAX_LINE_BUFF_SIZE - 1, even if is just a signle line*/
        ...
        ext_fs_seek(fh, (S32) (strLen - nBytes + 1), EXT_FS_SEEK_CURRENT);
        nBytes = strLen;
    }
...
{
    if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_VERSION_VALUE_10, strlen(MAP_VCARD_PROP_VERSION_VALUE_10)) == 0)
    {
...
{
    if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_STATUS_VALUE_READ, strlen(MAP_VCARD_PROP_STATUS_VALUE_READ)) == 0)
    {
...
{
    if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_TYPE_VALUE_EMAIL, strlen(MAP_VCARD_PROP_TYPE_VALUE_EMAIL)) == 0)
    {
        return MBT_MAP_MSG_TYPE_EMAIL;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_TYPE_VALUE_SMS_GSM, strlen(MAP_VCARD_PROP_TYPE_VALUE_SMS_GSM)) == 0)
    {
        return MBT_MAP_MSG_TYPE_SMS_GSM;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_TYPE_VALUE_SMS_CDMA, strlen(MAP_VCARD_PROP_TYPE_VALUE_SMS_CDMA)) == 0)
    {
...
{
    if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_ENCODING_VALUE_8BIT, strlen(MAP_VCARD_PROP_ENCODING_VALUE_8BIT)) == 0)
    {
        return MBT_MAP_MSG_ENCOD_8BIT;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_ENCODING_VALUE_G7BIT, strlen(MAP_VCARD_PROP_ENCODING_VALUE_G7BIT)) == 0)
    {
        return MBT_MAP_MSG_ENCOD_G7BIT;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_ENCODING_VALUE_G7BITEXT, strlen(MAP_VCARD_PROP_ENCODING_VALUE_G7BITEXT)) == 0)
    {
        return MBT_MAP_MSG_ENCOD_G7BITEXT;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_ENCODING_VALUE_GUCS2, strlen(MAP_VCARD_PROP_ENCODING_VALUE_GUCS2)) == 0)
    {
        return MBT_MAP_MSG_ENCOD_GUCS2;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_ENCODING_VALUE_G8BIT, strlen(MAP_VCARD_PROP_ENCODING_VALUE_G8BIT)) == 0)
    {
        return MBT_MAP_MSG_ENCOD_G8BIT;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_ENCODING_VALUE_C8BIT, strlen(MAP_VCARD_PROP_ENCODING_VALUE_C8BIT)) == 0)
    {
        return MBT_MAP_MSG_ENCOD_C8BIT;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_ENCODING_VALUE_CEPM, strlen(MAP_VCARD_PROP_ENCODING_VALUE_CEPM)) == 0)
    {
        return MBT_MAP_MSG_ENCOD_CEPM;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_ENCODING_VALUE_C7ASCII, strlen(MAP_VCARD_PROP_ENCODING_VALUE_C7ASCII)) == 0)
    {
        return MBT_MAP_MSG_ENCOD_C7ASCII;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_ENCODING_VALUE_CIA5, strlen(MAP_VCARD_PROP_ENCODING_VALUE_CIA5)) == 0)
    {
        return MBT_MAP_MSG_ENCOD_CIA5;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_ENCODING_VALUE_CUNICODE, strlen(MAP_VCARD_PROP_ENCODING_VALUE_CUNICODE)) == 0)
    {
        return MBT_MAP_MSG_ENCOD_CUNICODE;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_ENCODING_VALUE_CSJIS, strlen(MAP_VCARD_PROP_ENCODING_VALUE_CSJIS)) == 0)
    {
        return MBT_MAP_MSG_ENCOD_CSJIS;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_ENCODING_VALUE_CKOREAN, strlen(MAP_VCARD_PROP_ENCODING_VALUE_CKOREAN)) == 0)
    {
        return MBT_MAP_MSG_ENCOD_CKOREAN;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_ENCODING_VALUE_CLATINHEB, strlen(MAP_VCARD_PROP_ENCODING_VALUE_CLATINHEB)) == 0)
    {
        return MBT_MAP_MSG_ENCOD_CLATINHEB;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_ENCODING_VALUE_CLATIN, strlen(MAP_VCARD_PROP_ENCODING_VALUE_CLATIN)) == 0)
    {
...
{
    if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_CHARSET_VALUE_UTF8, strlen(MAP_VCARD_PROP_CHARSET_VALUE_UTF8)) == 0)
    {
...
{
    if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_LANGUAGE_VALUE_ENGLISH, strlen(MAP_VCARD_PROP_LANGUAGE_VALUE_ENGLISH)) == 0)
    {
        return MBT_MAP_MSG_LANG_ENGLISH;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_LANGUAGE_VALUE_FRENCH, strlen(MAP_VCARD_PROP_LANGUAGE_VALUE_FRENCH)) == 0)
    {
        return MBT_MAP_MSG_LANG_FRENCH;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_LANGUAGE_VALUE_SPANISH, strlen(MAP_VCARD_PROP_LANGUAGE_VALUE_SPANISH)) == 0)
    {
        return MBT_MAP_MSG_LANG_SPANISH;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_LANGUAGE_VALUE_JAPANESE, strlen(MAP_VCARD_PROP_LANGUAGE_VALUE_JAPANESE)) == 0)
    {
        return MBT_MAP_MSG_LANG_JAPANESE;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_LANGUAGE_VALUE_KOREAN, strlen(MAP_VCARD_PROP_LANGUAGE_VALUE_KOREAN)) == 0)
    {
        return MBT_MAP_MSG_LANG_KOREAN;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_LANGUAGE_VALUE_CHINESE, strlen(MAP_VCARD_PROP_LANGUAGE_VALUE_CHINESE)) == 0)
    {
        return MBT_MAP_MSG_LANG_CHINESE;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_LANGUAGE_VALUE_HEBREW, strlen(MAP_VCARD_PROP_LANGUAGE_VALUE_HEBREW)) == 0)
    {
        return MBT_MAP_MSG_LANG_HEBREW;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_LANGUAGE_VALUE_TURKISH, strlen(MAP_VCARD_PROP_LANGUAGE_VALUE_TURKISH)) == 0)
    {
        return MBT_MAP_MSG_LANG_TURKISH;
    }
    else if (btmtk_util_strnicmp(str, MAP_VCARD_PROP_LANGUAGE_VALUE_PORTUGUESE, strlen(MAP_VCARD_PROP_LANGUAGE_VALUE_PORTUGUESE)) == 0)
    {
...
static BT_BOOL btmtk_map_vcard_is_object_begin(S8 *str, btmtk_map_vcard_object_enum obj)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (btmtk_vcard_cmp_field(str, MAP_VCARD_PROP_BEGIN))
    {
        ...
        if (btmtk_util_strnicmp(value, obj_str, strlen(obj_str)) == 0)
        {
            return TRUE;
        }
    }
...
static BT_BOOL btmtk_map_vcard_is_object_end(S8 *str, btmtk_map_vcard_object_enum obj)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
static S32 btmtk_map_vcard_read_vcard(S8 *buf, S32 fh, BT_BOOL add_header)
{
    int len;
													  ...
													  )
{
    ...
    {
 #ifdef BTMTK_ON_WISE
        MBT_MAP_STATUS *status = (MBT_MAP_STATUS *) mbt_sdc_getrecord(MBTSDC_REC_MAP_STATUS);
 #else
	...
	/*	if(msg->FolderPath){
		...
		{
		    prop_list[i++] = btmtk_map_util_get_virtual_folder_path(status->CurFolderPath, (MBT_CHAR*)status->masInfo.RootPath);
		} else {
...
//        prop_list[i++] = msg->FolderPath;
        ...
        {
            ext_fs_write(fhandle, msg->Originator, msg->OriginatorSize);
            ext_fs_write(fhandle, VCARD_LINE_BREAK, 2);
        }
	    ...
	    level++;
            ...
            {
            	ext_fs_write(fhandle, msg->Envelope[index].Recipient, msg->Envelope[index].RecipientSize);
                ext_fs_write(fhandle, VCARD_LINE_BREAK, 2);
            }
            else
            {
    ...
    btmtk_map_vcard_create_object_start(fhandle, MAP_VCARD_OBJ_VALUE_BBODY);
    {
        ...
        if (frac_deliv != MBT_MAP_FRAC_DELIVER_NO)
        {
			...
			value = btmtk_map_vcard_get_encoding_string(msg->Encoding);
			if (value != NULL)
			{
        ...
        prop_list[i++] = MAP_VCARD_PROP_CHARSET;
        	prop_list[i++] = btmtk_map_vcard_get_charset_string(msg->Charset);
			value = btmtk_map_vcard_get_language_string(msg->Lang);
			if (value != NULL)
			{
        ...
        prop_list[i++] = MAP_VCARD_PROP_LENGTH;
        	sprintf(n_buf2, "%lu", msg->ContentSize + (MAP_VCARD_OBJ_VALUE_MSG_SHELL_LEN) * msg->FragmentNum);
        prop_list[i++] = n_buf2;
        prop_list[i] = NULL;
        btmtk_map_vcard_create_object_properties(fhandle, prop_list);
    }
    ...
    if (rhandle >= 0)
    {
        	int read, write, buffersize, remain = msg->FragmentSize[msgcount];

        while (remain > 0)
        {
        		buffersize = (remain < MAP_ADP_BUFFER_SIZE) ? remain : MAP_ADP_BUFFER_SIZE;
            	if((read = ext_fs_read(rhandle, g_pi_map_cntx.file_buffer, buffersize)) < 0)
            {
                ext_fs_close(fhandle);
                return FALSE;
            }
            ...
            remain -= read;
            if((write = ext_fs_write(fhandle, g_pi_map_cntx.file_buffer, read)) < 0)
            {
                ext_fs_close(fhandle);
                return FALSE;
            }
    ...
    {
    		if (ext_fs_write(fhandle, (msg->Content + msglength), msg->FragmentSize[msgcount]) < 0)
    		{
				return FALSE;
    }
    else
                                                    ...
                                                    char *file_name)
{
    ...
    if ((fhandle = ext_fs_open((const U8*)file_name, EXT_FS_READ_ONLY)) < 0)
    {
        return FALSE;
    }

    if (btmtk_map_vcard_create_message_by_buffer(mas_index, frac_req, frac_deliv, msg, fhandle, file_size) == FALSE)
    {
        ext_fs_close(fhandle);
        return FALSE;
    }
...
BT_BOOL btmtk_map_vcard_extract_content_body_by_single_line(S32 fhandle,MBT_MAP_MSG_OBJECT *msg, char *to_file)
{
	S32 whandle;
	S32 count = 0;
	BT_BOOL ret = TRUE;
	BT_BOOL endOfLine = FALSE;
	bt_ext_log("[MAP][EXT]btmtk_map_vcard_extract_content_body_by_single_line");
	if ((msg == NULL)
		|| (to_file == NULL) 
		|| (whandle = ext_fs_open((const U8*)to_file, BTMTK_FS_APPEND | EXT_FS_CREATE)) < 0)
	{
		return FALSE;
	}
    ...
    {            
        ...
        if ((count < VCARD_MAX_LINE_BUFF_SIZE - 1))
        {
			...
			if(msg->ContentSize < MAP_VCARD_OBJ_VALUE_MSG_SHELL_LEN)
			{
				msg->ContentSize = 0;
				bt_ext_err("[MAP][EXT]the message length is incorrent");
			} 
			else
			{			
		...
		else
		{
        	...
        	ext_fs_seek(fhandle, -1, EXT_FS_SEEK_CURRENT);
			count--;
			msg->FragmentSize[msg->FragmentNum] += count;
			bt_ext_log("keep going");
		}
		if(ext_fs_write(whandle, g_pi_map_cntx.file_buffer, count) < 0 || endOfLine)
        {
        	break;
        }	
	}
...
BT_BOOL btmtk_map_vcard_extract_content_body_by_size(S32 fhandle,MBT_MAP_MSG_OBJECT *msg, char *to_file, U32 *msg_buf_size)
{
	BT_BOOL ret = TRUE;	
	U32 size;
	msg->ContentSize -= MAP_VCARD_OBJ_VALUE_MSG_SHELL_LEN;
	...
	if (*msg_buf_size < size)
	{
		...
		if ((whandle = ext_fs_open((const U8*)to_file, BTMTK_FS_APPEND | EXT_FS_CREATE)) >= 0)
		{
			while (remain > 0)
			{
				current = (remain < MAP_ADP_BUFFER_SIZE)? remain : MAP_ADP_BUFFER_SIZE;
				bt_ext_log("remain is %d", remain);
				if((read = ext_fs_read(fhandle, g_pi_map_cntx.file_buffer, current)) < 0)
				{
					break;
				}
				remain -= read;
				if(ext_fs_write(whandle, g_pi_map_cntx.file_buffer, read) < 0)
				{
				   break;
				}
			}   
	...
	{
		S32 length, offset;
		length = ext_fs_read(fhandle, msg->Content, size);
		//*msg_buf_size -= length;
		if ((S32)(*msg_buf_size) < length)
		{
		   *msg_buf_size = 0;
		}else
		{
                                                ...
                                                char *to_file)
{
    ...
    if (strlen(to_file) == 0)
	{
    	sprintf(to_file, BT_MAP_TMP_FILE, mas_index);
    }
    btmtk_fs_delete((const U8*)to_file);
    ...
    sprintf(filename, BT_MAP_RCV_MESSAGE_PUSH_FILE, mas_index);
    if ((fhandle = ext_fs_open((const U8*)filename, EXT_FS_READ_ONLY)) < 0)
    {
        return FALSE;
    }
    ...
    while ((count = btmtk_map_vcard_get_single_line(g_pi_map_cntx.file_buffer, fhandle)) > 0)
    {
        ...
        if (btmtk_map_vcard_is_object_begin(g_pi_map_cntx.file_buffer, MAP_VCARD_OBJECT_BMSG))
        {
            ...
            while (in_bmsg && (count = btmtk_map_vcard_get_single_line(g_pi_map_cntx.file_buffer, fhandle)) > 0)
            {
                ...
                case MAP_VCARD_BMSG_PROP_FOLDER:
                    ...
                    break;
                default:
                    ext_fs_seek(fhandle, 0 - count - 1, EXT_FS_SEEK_CURRENT);
                    in_bmsg = FALSE;
                    break;
                }
            }
        ...
        else if (btmtk_map_vcard_is_object_begin(g_pi_map_cntx.file_buffer, MAP_VCARD_OBJECT_VCARD))
        {
            ...
            int len;
            ext_fs_seek(fhandle, 0 - count - 1, EXT_FS_SEEK_CURRENT);
            len = btmtk_map_vcard_read_vcard(g_pi_map_cntx.file_buffer, fhandle, FALSE);
		...
		msg->Originator[len] = '\0';
            ...
            else
            {
				...
				msg->Envelope[benv_level - 1].Recipient = calloc(len+1, sizeof(MBT_CHAR));
        ...
        {	
            ...
            while (in_bbody && (count = btmtk_map_vcard_get_single_line(g_pi_map_cntx.file_buffer, fhandle)) > 0)
            {
                ...
                case MAP_VCARD_BBODY_PROP_PARTID:
                    msg->PartId = (MBT_SHORT) atoi(prop_value);
                    break;
         ...
         //           msg->ContentSize += (MBT_UINT) atoi(prop_value);
		 //   msg->FragmentSize[msg->FragmentNum++] = (MBT_UINT) atoi(prop_value);
		      msg->ContentSize = (MBT_UINT) atoi(prop_value);
                    break;
                default:
                    ext_fs_seek(fhandle, 0 - count - 1, EXT_FS_SEEK_CURRENT);
