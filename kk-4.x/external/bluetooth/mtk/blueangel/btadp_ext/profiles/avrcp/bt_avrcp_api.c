...
static U8 g_avrcp_browse_connected = 0;

// copy from bt_pbap_api.c
static U8 BT_SendMessage(msg_type msg_id, module_type mod_id, ilm_struct * ilm, U16 size)
{
    int ret;
    int sockfd;
    U16 *field;
	
    bt_ext_log("[AVRCP] := msg=%u, ptr=0x%X, size=%d", msg_id, (unsigned int)ilm, size);
    ...
    *(++field) = size;
    ...
    if(mod_id == MOD_BT)
    {
        btmtk_sendmsg(PROFILE_AVRCP, ilm, size);
    }
    else
    {   // MOD_MMI
    	btmtk_sendmsg(PROFILE_AVRCP, ilm, size);
...
EXP_AVAPI U8 AVRCP_SendMessage(void *cntx, U16 id, void *data, U32 len){
    ...
    ret = BT_SendMessage(id, MOD_BT, &ilm, len);
    if( ret >= 0)
    {
        return 0 ;
    }
...
EXP_AVAPI BT_BOOL btmkt_avrcp_init_context()
{
	...
	avrcp_input_init( (const char *) "AVRCP");
	
	g_avrcp_browse_connected = 0;
...
EXP_AVAPI void btmtk_avrcp_send_activate_req(AvrcpChannelContext *cntx, U8 chnl_num, U8 local_role, U8 version, U8 sdpfeature )
{
	...
	if( FALSE == btmkt_avrcp_is_activate() ){
		bt_ext_log("btmtk_avrcp_send_activate_req init context ");
		btmkt_avrcp_init_context();
	}
...
void btmtk_avrcp_send_reg_notievent_track_changed_rsp(AvrcpChannelContext *cntx, U8 error, U8 interim, U8 *identifier){
	...
	U8 allzero[8] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
	if( 0 == cntx ){
		cntx = &g_avrpcAdpCntx;
	}
	...
	if( error == 0 ){
		...
		// avrcp only !
		if( 0 == g_avrcp_browse_connected ){
		    if( 0 == ext_os_memcmp( &msg.identifier, allff, 8) ){
		        // do nothing
		    }else{
		        ext_os_memcpy( &msg.identifier, allzero, 8);
		        bt_ext_log("AVRCP send_get_element change to 0x00000000");
		    }
		}
	}else{
		msg.reject = TRUE;
...
void btmtk_avrcp_send_reg_notievent_volume_changed_req(AvrcpChannelContext *cntx){
	bt_avrcp_register_notification_req_struct msg;
	if( 0 == cntx ){
		cntx = &g_avrpcAdpCntx;
	}
	...
	msg.event_id = AVRCP_NOTIFY_EVENT_VOLUME_CHANGED;
    msg.playback_interval = 0;
		
	AVRCP_SendMessage(cntx, MSG_ID_BT_AVRCP_REGISTER_NOTIFICATION_REQ, &msg, sizeof(bt_avrcp_register_notification_req_struct));	

}

void btmtk_avrcp_send_set_absolute_volume_req(AvrcpChannelContext *cntx, U8 volume){
	bt_avrcp_set_absolute_volume_req_struct msg;
	if( 0 == cntx ){
		cntx = &g_avrpcAdpCntx;
	}    
	...
	AVRCP_SendMessage(cntx, MSG_ID_BT_AVRCP_SET_ABSOLUTE_VOLUME_REQ, &msg, sizeof(bt_avrcp_set_absolute_volume_req_struct));	
}
