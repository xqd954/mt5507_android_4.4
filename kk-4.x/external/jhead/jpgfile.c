...
int getThumbnailLen(int* len)
{
    ...
    *len =  ImageInfo.ThumbnailSize;

    return TRUE;
}

int SaveThumbnailToBuffer(char * buf, int len)
{
    if (ImageInfo.ThumbnailOffset == 0 || ImageInfo.ThumbnailSize == 0){
        fprintf(stderr,"Image contains no thumbnail\n");
        return FALSE;
    ...
    if (len < ImageInfo.ThumbnailSize)
    {
        fprintf(stderr, "buffer len is not enough\n");
        return FALSE;
    }
    ...
    if (buf)
    {
        ...
        //check if the thumbnaildata is a valid jpg format file. 
        //Some special jpg file has wrong thumbnail offset which would cause decoder fail.
        //Modify it for fixing DTV00578726.
        if (ThumbnailPointer[0] == 0xFF && ThumbnailPointer[1] == 0xD8)
        {
            memcpy(buf, ThumbnailPointer, ImageInfo.ThumbnailSize );
        }
        else
        {
            //invalid thumbnail
