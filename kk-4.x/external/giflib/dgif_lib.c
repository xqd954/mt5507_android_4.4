...
DGifDecompressLine(GifFileType *GifFile, GifPixelType *Line, int LineLen)
{
    ...
    if (StackPtr != 0) {
        ...
        {
        	if (GifFile->SavedImages[GifFile->ImageCount-1].ImageDesc.transparent_idx == Stack[--StackPtr] &&
				GifFile->SavedImages[GifFile->ImageCount-1].ImageDesc.transparent_flag)
        	{
            	i++;
        	}
			else
    ...
    while (i < LineLen) {    /* Decode LineLen items. */
	    ...
	    return GIF_ERROR;
            ...
            if (CrntCode < ClearCode) {
                ...
                if (GifFile->SavedImages[GifFile->ImageCount-1].ImageDesc.transparent_idx == CrntCode &&
				GifFile->SavedImages[GifFile->ImageCount-1].ImageDesc.transparent_flag)
	        	{
	            	i++;
	        	}
...
int DGifSkipContent(GifFileType * GifFile)
{
	GifByteType* buf = NULL;
	GifByteType ui1_byte = 0;
	
	while (1)
    {
        if (READ(GifFile, &ui1_byte, 1) <= 0)
       	{
       		return -1;
        }
        ...
        {
        	buf = (GifByteType*)malloc(ui1_byte);
			if (buf == NULL)
			{
				return -1;
			}
        	if (READ(GifFile,buf ,ui1_byte) <= 0)
        	{
        		free(buf);
        		return -1;
        	}
