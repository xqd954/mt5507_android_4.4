			 ...
			 d_derived_tbl ** pdtbl)
{
  ...
  for (l = 1; l <= 16; l++) {
    if (htbl->bits[l]) {
       ...
       */
      dtbl->valoffset[l] = (INTJ32) p - (INTJ32) huffcode[p];
      p += htbl->bits[l];
...
/* Load up the bit buffer to a depth of at least nbits */
{
  ...
  if (cinfo->unread_marker == 0) {	/* cannot advance past a marker */
    ...
    {   
        #if 0
        //some corrupt jpg file has no EOI tag. So force it to end when data is over.
        //Fix DTV00584607 by xiuqin.wang
        cinfo->unread_marker = 0xD9;//EOI
        goto no_more_bytes;
        #endif
	    return FALSE;
    }
