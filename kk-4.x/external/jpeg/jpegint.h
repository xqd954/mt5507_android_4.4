 ...
 * RIGHT_SHIFT provides a proper signed right shift of an INTJ32 quantity.
 * It is only applied with constant shift counts.  SHIFT_TEMPS must be
 * included in the variables of any routine using RIGHT_SHIFT.
 */

#ifdef RIGHT_SHIFT_IS_UNSIGNED
#define SHIFT_TEMPS	INTJ32 shift_temp;
#define RIGHT_SHIFT(x,shft)  \
	((shift_temp = (x)) < 0 ? \
	 (shift_temp >> (shft)) | ((~((INTJ32) 0)) << (32-(shft))) : \
	 (shift_temp >> (shft)))
#else
#define SHIFT_TEMPS
...
JEXTERN(void) jinit_phuff_encoder JPP((j_compress_ptr cinfo));
...
JEXTERN(void) jinit_huff_decoder_no_data JPP((j_decompress_ptr cinfo));
JEXTERN(void) jinit_phuff_decoder JPP((j_decompress_ptr cinfo));
...
JEXTERN(void) jpeg_decompress_per_scan_setup (j_decompress_ptr cinfo);
...
JEXTERN(long) jmin JPP((long a, long b));
...
JEXTERN(void) jset_input_stream_position JPP((j_decompress_ptr cinfo,
                    int offset));
JEXTERN(void) jset_input_stream_position_bit JPP((j_decompress_ptr cinfo,
                    int byte_offset, int bit_left, INTJ32 buf));

JEXTERN(int) jget_input_stream_position JPP((j_decompress_ptr cinfo));
