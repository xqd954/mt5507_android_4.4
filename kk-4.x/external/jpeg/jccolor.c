...
typedef struct {
  ...
  INTJ32 * rgb_ycc_tab;		/* => table for RGB to YCbCr conversion */
} my_color_converter;
...
#define CBCR_OFFSET	((INTJ32) CENTERJSAMPLE << SCALEBITS)
#define ONE_HALF	((INTJ32) 1 << (SCALEBITS-1))
#define FIX(x)		((INTJ32) ((x) * (1L<<SCALEBITS) + 0.5))
...
rgb_ycc_start (j_compress_ptr cinfo)
{
  ...
  cconvert->rgb_ycc_tab = rgb_ycc_tab = (INTJ32 *)
    (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_IMAGE,
				(TABLE_SIZE * SIZEOF(INTJ32)));
		 ...
		 JDIMENSION output_row, int num_rows)
{
  my_cconvert_ptr cconvert = (my_cconvert_ptr) cinfo->cconvert;
  register int r, g, b;
  register INTJ32 * ctab = cconvert->rgb_ycc_tab;
...
static void copyquads(const UINTJ32 in[], UINTJ32 out0[], UINTJ32 out1[], UINTJ32 out2[], int col4)
{
    do {
        UINTJ32 src0 = *in++;
        UINTJ32 src1 = *in++;
  ...
  JDIMENSION num_cols = cinfo->image_width;
    ...
    if (1 == num_rows && 3 == nc && num_cols > 0) {
        ...
        if (col4 > 0 && ptr_is_quad(inptr) && ptr_is_quad(outptr0) &&
            ...
            const UINTJ32* in = (const UINTJ32*)inptr;
            UINTJ32* out0 = (UINTJ32*)outptr0;
            UINTJ32* out1 = (UINTJ32*)outptr1;
            UINTJ32* out2 = (UINTJ32*)outptr2;
