...
public class BrowserSettings implements OnSharedPreferenceChangeListener,
    ...
    private void syncStaticSettings(WebSettings settings) {
        ...
        // -- MTK_WEBKIT_FIX START --
        int openNewLinkInSameTab = 1;

        if (0 != SystemProperties.get("mtk.browser.openLinkInSameTab").length()) {
            openNewLinkInSameTab = Integer.parseInt(SystemProperties.get("mtk.browser.openLinkInSameTab"));
        }

        if (openNewLinkInSameTab == 0) {
    ...
    // -- MTK_WEBKIT_FIX START --
    public void disablePluginIfNeed(String url, WebView view) {
        if (view == null) {
            return;
        }
        ...
        // do nothing if current UA is not iPad
        if (USER_AGENTS[getUserAgent()] == null || IPAD_USERAGENT.compareTo(USER_AGENTS[getUserAgent()]) != 0) {
            return;
        }

        if (MtkBlacklist.disablePlugin(url)) {
        ...
        Log.i("browserSetting", "url = " + url + ", PluginState is " + settings.getPluginState());
    }

    public void adjustUAStringIfNeed(String url, WebView view) {
        if (view == null) {
            return;
        }
        ...
        int skipUABlackList = 0;

        if (0 != SystemProperties.get("mtk.browser.skipUABlackList").length()) {
            skipUABlackList = Integer.parseInt(SystemProperties.get("mtk.browser.skipUABlackList"));
        }

        if (skipUABlackList == 0) {
            if (MtkBlacklist.useDesktopUA(url)) {
                settings.setUserAgentString(DESKTOP_USERAGENT);
            } else if (MtkBlacklist.useAndroidUA(url)) {
                // set null mean use Android's default UA
                settings.setUserAgentString(null);
            } else if (MtkBlacklist.useIPadUA(url)) {
                settings.setUserAgentString(IPAD_USERAGENT);
            } else {
                settings.setUserAgentString(USER_AGENTS[getUserAgent()]);
            }
        } else {
        ...
        Log.i("browserSetting", "url = " + url + ", Current UA String is " + settings.getUserAgentString());
    }
    // -- MTK_WEBKIT_FIX END --
}
