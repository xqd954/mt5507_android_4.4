...
public class SoftKeyboardView extends View {
	...
	private BalloonHint mCaretOnKey;
	...
	private int mActivekeyIndex;

	private int mAllKeyNum;

	private List<SoftKey> mKeysList = new ArrayList<SoftKey>();

	List<List<SoftKey>> mKeyRows = new ArrayList<List<SoftKey>>();
    ...
    public boolean setSoftKeyboard(SoftKeyboard softSkb) {
		...
		for (int row = 0; row < rowNum; row++) {
			...
			if (null == keyRow || keyRow.mSoftKeys.size() == 0) {
				continue;
			}
			List<SoftKey> softKeys = keyRow.mSoftKeys;
			mKeyRows.add(softKeys);
			mKeysList.addAll(softKeys);

		}
		if (null != mCaretOnKey) {
			mCaretOnKey.delayedDismiss(0);
		}
		mAllKeyNum = mKeysList.size();
		mActivekeyIndex = -1;
        return true;
    }
	...
	private int calcRowIndex(int keyIndex) {
		int startIdx = 0, endIdx = 0;
		List<SoftKey> rowKeys;
		for (int row = 0; row < mKeyRows.size(); row++) {
			rowKeys = mKeyRows.get(row);
			startIdx = endIdx;
			endIdx += rowKeys.size();
			if (keyIndex >= startIdx && keyIndex < endIdx) {
				return row;
			}
		}
		return 0;
	...
	private int calcIndexInKeyRow(int keyIndex) {
		int rowIdx = calcRowIndex(keyIndex);
		for (int row = 0; row < rowIdx; row++) {
			keyIndex -= mKeyRows.get(row).size();
		}
		return keyIndex;
	}

	private int calcActiveIndex(int rowIdx, int idxInRow) {
		for (int i = 0; i < rowIdx; i++) {
			idxInRow += mKeyRows.get(i).size();
		}
		return idxInRow;
	}

	public SoftKey onKeyActive(int keyCode) {
		int oldIdx = mActivekeyIndex;
		if (-1 == mActivekeyIndex && KeyEvent.KEYCODE_DPAD_CENTER != keyCode) {
			mActivekeyIndex = 0;
		} else {
			switch (keyCode) {
			case KeyEvent.KEYCODE_DPAD_LEFT:
				mActivekeyIndex = (mActivekeyIndex + mAllKeyNum - 1)
						% mAllKeyNum;
				break;

			case KeyEvent.KEYCODE_DPAD_RIGHT:
				mActivekeyIndex = (mActivekeyIndex + 1) % mAllKeyNum;
				break;

			case KeyEvent.KEYCODE_DPAD_UP:
			case KeyEvent.KEYCODE_DPAD_DOWN: {
				int rowNum = mKeyRows.size();
				int rowIdx = calcRowIndex(mActivekeyIndex);
				int idxInRow = calcIndexInKeyRow(mActivekeyIndex);
				if (keyCode == KeyEvent.KEYCODE_DPAD_UP) {
					rowIdx = (rowIdx + rowNum - 1) % rowNum;
				} else {
					rowIdx = (rowIdx + 1) % rowNum;
				}

				if (idxInRow > mKeyRows.get(rowIdx).size() - 1) {
					idxInRow = mKeyRows.get(rowIdx).size() - 1;
				}
				mActivekeyIndex = calcActiveIndex(rowIdx, idxInRow);
				break;
			}
		...
		SoftKey activeKey = mKeysList.get(mActivekeyIndex);
		if (oldIdx != mActivekeyIndex && mActivekeyIndex >= 0) {
			showCaret(activeKey);
		}
	...
	private void showCaret(SoftKey activeKey) {
		if (null == activeKey) {
			return;
		}
		Drawable keyHlBg = activeKey.getKeyHlBg();
		mCaretOnKey.setBalloonBackground(keyHlBg);
		...
		int desired_width;
		int desired_height;
		desired_width = activeKey.width() - 2 * keyXMargin;
		desired_height = activeKey.height() - 2 * keyYMargin;
		float textSize;
		Environment env = Environment.getInstance();
		textSize = env
				.getKeyTextSize(SoftKeyType.KEYTYPE_ID_NORMAL_KEY != activeKey.mKeyType.mKeyTypeId);
		Drawable icon = activeKey.getKeyIcon();
		if (null != icon) {
			mCaretOnKey.setBalloonConfig(icon, desired_width, desired_height);
		} else {
			mCaretOnKey
					.setBalloonConfig(activeKey.getKeyLabel(), textSize, true,
							activeKey.getColorHl(), desired_width,
							desired_height);
		}

		mHintLocationToSkbContainer[0] = mPaddingLeft + activeKey.mLeft
				- (mCaretOnKey.getWidth() - activeKey.width()) / 2;
		mHintLocationToSkbContainer[0] += mOffsetToSkbContainer[0];
		mHintLocationToSkbContainer[1] = mPaddingTop
				+ (activeKey.mBottom - keyYMargin) - mCaretOnKey.getHeight();
		mHintLocationToSkbContainer[1] += mOffsetToSkbContainer[1];
		showBalloon(mCaretOnKey, mHintLocationToSkbContainer, false);
	}
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int row = 0; row < rowNum; row++) {
            ...
            List<SoftKey> softKeys = keyRow.mSoftKeys;
			if (keyRow.mSoftKeys.size() != 0 && !mKeyRows.contains(softKeys)) {
				mKeyRows.add(softKeys);
				mKeysList.addAll(softKeys);
				mAllKeyNum = mKeysList.size();
			}
