...
public class MediaScannerReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Intent.ACTION_BOOT_COMPLETED.equals(action)) {
            ...
            /*
	            ...
	            if(Environment.MEDIA_MOUNTED.equals(Environment.getStorageState(Environment.getExternalStorageDirectory())))
	            {
	                scan(context, MediaProvider.EXTERNAL_VOLUME);
	            }
			*/
			
		//Added by zhaojun@tcl.com for mediascan start/stop
			...
			Log.w(TAG, "-------------------"+action+"---------------------");
			Log.w(TAG, "-------------------"+uri+"---------------------");
			updatescan(context, path);    //update database
        } else {
	...
	private void updatescan(Context context, String path) {
		Bundle args = new Bundle();
		args.putString("updatapath", path);
		context.startService(
				new Intent(context, MediaScannerService.class).putExtras(args));
	}
}
