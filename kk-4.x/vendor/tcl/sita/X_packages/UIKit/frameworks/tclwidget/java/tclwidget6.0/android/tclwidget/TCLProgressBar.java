...
public class TCLProgressBar extends ProgressBar {
	...
	private void drawableOfStyleCircle(Context context, AttributeSet attrs) {
		...
		mOnlyIndeterminate = a.getBoolean(
				com.android.internal.R.styleable.ProgressBar_indeterminateOnly, mOnlyIndeterminate);
		if (drawable == null && mOnlyIndeterminate) {
			drawable = a
					.getDrawable(com.android.internal.R.styleable.ProgressBar_indeterminateDrawable);
			if (drawable != null) {
		...
		if (attrs.getStyleAttribute() == 0) {
			...
			if(attr_style != null){
				if(attr_style.indexOf("tvos:attr/tclprogressBarStyleHorizontal") != -1) {
					setTclStyle(context, TclStyle_Horizontal);
				}else if(attr_style.indexOf("tvos:attr/tclprogressBarStyleCircle") != -1){
					setTclStyle(context, TclStyle_Circle);
				}else if(attr_style.indexOf("tvos:attr/tclprogressBarStyleHorizontalWhite") != -1){
