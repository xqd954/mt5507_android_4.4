...
static unsigned char *search_fat_label(struct vfat_dir_entry *dir, int count)
{
	...
	for (i = 0; i < count; i++) {
		if (dir[i].name[0] == 0x00) {
			LOGI("dir[%d].name[0] == 0x00 and Break\n",i);
			break;
		}
		...
		if ((dir[i].name[0] == FAT_ENTRY_FREE) ||
			...
			LOGI("i=%d,dir.name=0x%x,cluster_high=0x%x,cluster_low=0x%x,attr=0x%x\n",i,dir[i].name[0],dir[i].cluster_high,dir[i].cluster_low,dir[i].attr);
			continue;
		}

		if ((dir[i].attr & (FAT_ATTR_VOLUME_ID | FAT_ATTR_DIR)) ==
		    FAT_ATTR_VOLUME_ID) {
			LOGI("Find label name.i=%d,dir.name=%s,cluster_high=0x%x,cluster_low=0x%x,attr=0x%x\n",i,dir[i].name,dir[i].cluster_high,dir[i].cluster_low,dir[i].attr);
			return dir[i].name;
		}
	}
	return 0;
		      ...
		      unsigned char *buf)
{
	...
	if (ms->ms_fat_length) {
		...
		LOGI("vol_label=%s,ms->ms_label=%s\n",vol_label,ms->ms_label);
		...
		while (next && --maxloop) {
			...
			LOGI("count=%d,buf_size=%d,sizeof=%d,sector_size=%d,next_off=%x\n",count,buf_size, sizeof(struct vfat_dir_entry),sector_size,next_off);
			...
			LOGI("vol_label=%s,search_fat_label unexpectedly returns NULL\n",vol_label);
			/* get FAT entry */
		...
		if (!vol_label || !memcmp(vol_label, no_name, 11)) {
			LOGI("vol_label=%s,vs->vs_label=%s\n", vol_label, vs->vs_label);
			vol_label = vs->vs_label;
		}
	...
	LOGI("********Get label=%s, uuid=%s from buffer\n",label, serno);
	...
	while (1) {
		...
		if (attr_type == MFT_RECORD_ATTR_VOLUME_NAME) {
			...
			/* Modify by Sarah Zhang(zxia@tcl.com)
			   Fix: ntfs Chinese name is random code. */
			for (i=0, cp=label_str; i < val_len; i+=2,cp+=2) {
...
blkid_dev blkid_verify(blkid_cache cache, blkid_dev dev)
{
	...
	/* Modify by Sarah Zhang(zxia@tcl.com)
	 * Add diff verify, otherwise enter if loop and load only one vold */
...
try_again:
	...
	for (id = type_array; id->bim_type; id++) {
		...
		if (!buf)
			continue;
		LOGI("Maybe need to check this buffer\n");
