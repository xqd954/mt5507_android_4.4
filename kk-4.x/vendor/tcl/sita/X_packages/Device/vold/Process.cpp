...
void Process::killProcessesWithOpenFiles(const char *path, int action) {
    ...
    while ((de = readdir(dir))) {
        ...
        if (checkFileDescriptorSymLinks(pid, path, openfile, sizeof(openfile))) {
			...
			if (!strcmp(name, "./TvServer")){
				SLOGE("Warning Sarah: Process is ./TvServer and can't kill. Waiting...");
				continue;
			}
        ...
        if (action == 1) {
            SLOGW("Sending SIGHUP to process %d, name=%s", pid, name);
            kill(pid, SIGTERM);
        } else if (action == 2) {
            SLOGE("Sending SIGKILL to process %d, name=%s", pid, name);
            kill(pid, SIGKILL);
        }
    }
    closedir(dir);
