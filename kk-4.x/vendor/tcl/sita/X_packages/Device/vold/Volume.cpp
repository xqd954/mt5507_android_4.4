...
int Volume::mountVol() {
	...
	/*modified by cuixk for check others' filesystems*/		
		VolumeInfoManager *vim = VolumeInfoManager::Instance();

		if(!vim->getVolFstype(getMountpoint())){
			SLOGW("%s does not any filesystems\n", devicePath);
			continue;
		}
		
		if(!strncmp(vim->getVolFstype(getMountpoint()), "vfat", 4)){
		...
		}else if(!strncmp(vim->getVolFstype(getMountpoint()), "ntfs", 4)){
		...
		/* modify by zxia@tcl.com and support hfsplus */
		}else if(!strncmp(vim->getVolFstype(getMountpoint()), "hfsplus", 7)){
			if(Hfsplus::check(devicePath)){
				if(errno == ENODATA){
					SLOGW("%s does not contain a hfsplus filesystem\n", devicePath);
					continue;
				}
				errno = EIO;
			...
			SLOGD("Skpping filesytem checking.\n");
		}

		/*end modified by cuixk*/
       ...
       /* Begin modified by Sarah Zhang(zxia@tcl.com) */
       // if (primaryStorage) {
        if (1) {
      ...
      /* End modified by Sarah Zhang(zxia@tcl.com) */
	/*modified by cuixk for mount others' filesystems*/
		if(!strncmp(vim->getVolFstype(getMountpoint()), "vfat", 4)){
			if(primaryStorage){
        		if (Fat::doMount(devicePath, "/mnt/secure/staging", false, false, false,
               		 AID_SYSTEM, gid, 0002, true)) {
            		SLOGE("%s failed to mount @/mnt/secure/staging via VFAT (%s)\n", devicePath, strerror(errno));
            		continue;
        		}
			}else{
				if (Fat::doMount(devicePath, getMountpoint(), false, false, false,
               		 AID_SYSTEM, gid, 0002, true)) {
            		SLOGE("%s failed to mount @%s via VFAT (%s)\n", devicePath, getMountpoint(), strerror(errno));
            		continue;
				}
			}
		}else if(!strncmp(vim->getVolFstype(getMountpoint()), "ntfs", 4)){
			if(primaryStorage){
        		if (Ntfs::doMount(devicePath, "/mnt/secure/staging", false, false, AID_SYSTEM, gid, 0002, true)) {
            		SLOGE("%s failed to mount @/mnt/secure/staging via ntfs (%s)\n", devicePath, strerror(errno));
            		continue;
        		}
			}else{
        		if (Ntfs::doMount(devicePath, getMountpoint(), false, false, AID_SYSTEM, gid, 0002, true)) {
            		SLOGE("%s failed to mount @%s via ntfs (%s)\n", devicePath, getMountpoint(), strerror(errno));
            		continue;
        		}
			}
		/* modify by zxia@tcl.com and support hfsplus */
			...
			if(primaryStorage){
				if (Hfsplus::doMount(devicePath, "/mnt/secure/staging", false, false, AID_SYSTEM, gid, 0002, true)) {
					SLOGE("%s failed to mount @/mnt/secure/staging via hfsplus (%s)\n", devicePath, strerror(errno));
					continue;
				}
			}else{
				if (Hfsplus::doMount(devicePath, getMountpoint(), false, false, AID_SYSTEM, gid, 0002, true)) {
					SLOGE("%s failed to mount @%s via hfsplus (%s)\n", devicePath, getMountpoint(), strerror(errno));
					continue;
				}
			}
		}else if(!strncmp(vim->getVolFstype(getMountpoint()), "ext2", 4)){
			if(primaryStorage){
        		if (Ext2::doMount(devicePath, "/mnt/secure/staging", false, false,false)) {
            		SLOGE("%s failed to mount @/mnt/secure/staging via ext2 (%s)\n", devicePath, strerror(errno));
            		continue;
        		}
			}else{
        		if (Ext2::doMount(devicePath, getMountpoint(), false, false,false)) {
            		SLOGE("%s failed to mount @%s via ext2 (%s)\n", devicePath, getMountpoint(), strerror(errno));
            		continue;
        		}
			}

		}else if(!strncmp(vim->getVolFstype(getMountpoint()), "ext3", 4)){
			if(primaryStorage){
        		if (Ext3::doMount(devicePath, "/mnt/secure/staging", false, false,false)) {
            		SLOGE("%s failed to mount @/mnt/secure/staging via ext3 (%s)\n", devicePath, strerror(errno));
            		continue;
        		}
			}else{
        		if (Ext3::doMount(devicePath, getMountpoint(), false, false,false)) {
            		SLOGE("%s failed to mount @%s via ext3 (%s)\n", devicePath, getMountpoint(), strerror(errno));
            		continue;
        		}
			}
		}else if(!strncmp(vim->getVolFstype(getMountpoint()), "ext4", 4)){
			if(primaryStorage){
        		if (Ext4::doMount(devicePath, "/mnt/secure/staging", false, false,false)) {
            		SLOGE("%s failed to mount @/mnt/secure/staging via ext4 (%s)\n", devicePath, strerror(errno));
            		continue;
        		}
			}else{
        		if (Ext4::doMount(devicePath, getMountpoint(), false, false,false)) {
            		SLOGE("%s failed to mount @%s via ext4 (%s)\n", devicePath, getMountpoint(), strerror(errno));
            		continue;
        		}
			}
		}else{
			SLOGW("not support mounting this filesysem.\n");	
			continue;
		}
	...
	/*end modified by cuixk*/
         ...
         */
         /* Sarah Zhang(zxia@tcl.com):not primaryStorage will be error for udisk */
        /*modified by cuixk for only primaryStorage can mount via /mnt/secure/staging*/
        if (primaryStorage && doMoveMount("/mnt/secure/staging", getMountpoint(), false)) {
        /*end modified by cuixk*/
