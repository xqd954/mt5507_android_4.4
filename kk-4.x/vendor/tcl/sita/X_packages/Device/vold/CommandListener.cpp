...
int CommandListener::VolumeCmd::runCommand(SocketClient *cli,
    ...
    if (!strcmp(argv[1], "list")) {
        ...
        return 0;
/*modified by cuixk for get volume info(label.uuid.fstype)*/
    } else if(!strcmp(argv[1], "label")){
        if (argc != 3) {
            cli->sendMsg(ResponseCode::CommandSyntaxError, "Usage: volume label <path>", false);
            return 0;
        }
		return vim->VolumeInfoLabel(cli, argv[2]);
	} else if(!strcmp(argv[1], "fstype")){
        if (argc != 3) {
            cli->sendMsg(ResponseCode::CommandSyntaxError, "Usage: volume fstype <path>", false);
            return 0;
        }
		return vim->VolumeInfoFstype(cli, argv[2]);
	} else if(!strcmp(argv[1], "uuid")){
        if (argc != 3) {
            cli->sendMsg(ResponseCode::CommandSyntaxError, "Usage: volume uuid <path>", false);
            return 0;
        }
		return vim->VolumeInfoUuid(cli, argv[2]);
/*end modified by cuixk*/
	/* Begin to modify by Sarah Zhang(zxia@tcl.com) */
	} else if(!strcmp(argv[1], "devpath")){
        if (argc != 3) {
            cli->sendMsg(ResponseCode::CommandSyntaxError, "Usage: volume devpath <path>", false);
            return 0;
        }
		return vim->VolumeInfoDevpath(cli, argv[2]);
...
int CommandListener::AsecCmd::runCommand(SocketClient *cli,
    ...
    // Sarah Zhang(zxia@tcl.com) Begin
