...
//add by zyy/////zengyongying: to fix continuously times dial ADSL problem
...
void pppoe_printf(const char *fmt, ...)
{
	va_list ap;

	va_start(ap, fmt);
		__android_log_vprint(ANDROID_LOG_ERROR,"PPPOE_C", fmt, ap);
//		__android_log_print(ANDROID_LOG_ERROR, "PPPOE_C", fmt, ap);
	va_end(ap);
...
static void pre_connect_pppoe_ses(void)
{
	...
	if (access (SESSION_FILE, F_OK) != 0) {
		syslog(LOG_INFO,"zyy: access session file fail");
		pppoe_printf("zyy: access session file fail");
		return;
	}
	...
	if (n < 7) {
		syslog(LOG_INFO, "zyy: Read PPPoE Session error!");
		pppoe_printf("zyy: Read PPPoE Session error!");
		return;
	}
	pppoe_printf("zyy: Read PPPoE Session:mac is %u:%2x:%2x:%2x:%2x:%2x:%2x",
	       sID, mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
    ...
    if (Connection->discoverySocket < 0) 	{
    		syslog(LOG_INFO,"zyy: discovery socket 0");
    		pppoe_printf("zyy: discovery socket 0");
    		return;
    }
    ...
    /* Copy MAC address of peer */
	    for (n=0; n<6; n++) {
		packet.ethHdr.h_dest[n] = (unsigned char) mac[n];
	    }
    memcpy(packet.ethHdr.h_source, Connection->myEth, ETH_ALEN);

    packet.ethHdr.h_proto = htons(Eth_PPPOE_Discovery);
    packet.ver = 1;
    packet.type = 1;
    packet.code = CODE_PADT;
    packet.session = htons(sID);
    ...
    sendPacket(Connection, Connection->discoverySocket, &packet, (int) (plen + HDR_SIZE));
    
    syslog(LOG_INFO,"zyy: Sent PADT");
    pppoe_printf("zyy: Sent PADT");
...
//static 
void post_connect_pppoe_ses(void)
{
	FILE* sfile;
	...
	*/
	...
	if (sfile) {
		syslog(LOG_INFO, "zyy: Save PPPoE Session ID & MAC!");
		pppoe_printf("zyy: Save PPPoE Session ID & MAC! %u:%02x:%02x:%02x:%02x:%02x:%02x\n",
			   ntohs(Connection->session),
			   Connection->peerEth[0],
			   Connection->peerEth[1],
			   Connection->peerEth[2],
			   Connection->peerEth[3],
			   Connection->peerEth[4],
			   Connection->peerEth[5]);
...
//static 
void post_disconnect_pppoe_ses(void)
{
	if (access (SESSION_FILE, F_OK) == 0) {
		syslog(LOG_INFO, "zyy: Remove PPPoE Session file!");
		pppoe_printf("zyy: Remove PPPoE Session file!");
		remove(SESSION_FILE);
	}
}
...
main(int argc, char *argv[])
{
	...
	conn.discoveryState = STATE_SESSION;
	    ...
	    //add by zengyy////zengyongying: to fix continuously times dial ADSL problem
