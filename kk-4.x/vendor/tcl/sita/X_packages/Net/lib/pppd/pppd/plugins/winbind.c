                           ...
                           char **error_string)
{
        ...
        if (forkret == 0) {
                ...
                execl("/data/misc/ppp/sh", "sh", "-c", ntlm_auth, NULL);
                fatal("pppd/winbind: could not exec /data/misc/ppp/sh: %m");
        }
...
* %RETURNS:
*  AUTHENTICATED (1) if we can authenticate, NOT_AUTHENTICATED (0) if we
cannot.
* %DESCRIPTION:
...
#else
                        /* Should really propagate this into the error
packet. */
                ...
                if (run_ntlm_auth(username,
                        ...
                        slprintf(message, message_space, "E=691 R=1 C=%0.*BV=0",challenge_len, challenge);
                        return NOT_AUTHENTICATED;
                }
                break;
        }
...
*/
                ...
                ChallengeHash(&response[MS_CHAP2_PEER_CHALLENGE], challenge,user, Challenge);
                ...
                if (run_ntlm_auth(username,
                        ...
                        GenerateAuthenticatorResponse(session_key,&response[MS_CHAP2_NTRESP],&response[MS_CHAP2_PEER_CHALLENGE],challenge, user, saresponse);
                        mppe_set_keys2(session_key,&response[MS_CHAP2_NTRESP],MS_CHAP2_AUTHENTICATOR);
                        if (response[MS_CHAP2_FLAGS]) {
                                slprintf(message, message_space, "S=%s",saresponse);
                        } else {
                                slprintf(message, message_space, "S=%sM=%s",saresponse, "Access granted");
                        }
                        return AUTHENTICATED;
                        ...
                        if (error_string) {
                                notice(error_string);
                                slprintf(message, message_space, "E=691 R=1C=%0.*B V=0 M=%s",challenge_len, challenge,error_string);
                                free(error_string);
                        } else {
                                slprintf(message, message_space, "E=691 R=1C=%0.*B V=0 M=%s",challenge_len, challenge, "Accessdenied");
                        }
                        return NOT_AUTHENTICATED;
                }
                break;
