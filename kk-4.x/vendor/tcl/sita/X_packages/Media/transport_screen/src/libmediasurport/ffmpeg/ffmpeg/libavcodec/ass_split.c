...
//sujuan 2012-03-22
//get ass/ssa fontsize and fontcolor
void ff_ass_get_style_font_size_color(ASSSplitContext *ctx, int *size, unsigned int *color)
{
	unsigned int rgb; 
    if (ctx) 
	{
		if (ctx->ass.styles->font_size > 0)
		{
			*size = ctx->ass.styles->font_size;
		}
		else
		{
			*size = 18;
		...
		if (ctx->ass.styles->primary_color >=0 && ctx->ass.styles->primary_color <= 0xffffff)
		{
			rgb = ctx->ass.styles->primary_color;
