...
#define EARLYSUSPEND_AW_SYS_POWER_STATE "/sys/power/suspend"
...
#define AVOID_WAKELOCK
static int aw_sPowerStatefd;
static const char *pwr_state_mem = "mem";
static const char *pwr_state_on = "on";
static const char *aw_pwr_state_mem = "1";
static const char *aw_pwr_state_on = "0";
...
#else
    ret = write(aw_sPowerStatefd, aw_pwr_state_mem, strlen(aw_pwr_state_mem));
    if (ret < 0) {
        strerror_r(errno, buf, sizeof(buf));
        ALOGE("Error writing to %s: %s\n", EARLYSUSPEND_AW_SYS_POWER_STATE, buf);
        goto err;
    }
#endif
...
#else
    ret = write(aw_sPowerStatefd, aw_pwr_state_on, strlen(aw_pwr_state_on));
    if (ret < 0) {
        strerror_r(errno, buf, sizeof(buf));
        ALOGE("Error writing to %s: %s\n", EARLYSUSPEND_AW_SYS_POWER_STATE, buf);
        goto err;
...
struct autosuspend_ops *autosuspend_earlysuspend_init(void)
{
    ...
    aw_sPowerStatefd = open(EARLYSUSPEND_AW_SYS_POWER_STATE, O_RDWR);

    if (aw_sPowerStatefd < 0) {
        strerror_r(errno, buf, sizeof(buf));
        ALOGW("Error opening %s: %s\n", EARLYSUSPEND_AW_SYS_POWER_STATE, buf);
        return NULL;
    }
