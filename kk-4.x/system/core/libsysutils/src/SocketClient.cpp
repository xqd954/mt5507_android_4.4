...
int SocketClient::sendMsg(int code, const char *msg, bool addErrno, bool useCmdNum) {
    ...
    if (addErrno) {
        if (useCmdNum) {
        	...
        	if(code != ExternalVolumeInfo::VolumeMounted && code != ExternalVolumeInfo::VolumeUmounted
            && code != Device::DeviceProductConnected && code != Device::DeviceProductDisconnected
            && code != Device::DeviceWifiConnected && code != Device::DeviceWifiDisconnected
            && code != Device::DeviceConnected && code != Device::DeviceDisconnected
            && code != Device::DeviceIsoMountFailed){
