...
static int write_symlink_file(const char *path, const char *value)
{
    int fd, ret, len;
...
int do_exec(int nargs, char **args)
{
    ...
    for (i = nargs; i < 5; i++)
    {
        //reset the rest args as NULL to avoid EFAULT error when execl()
        args[i] = NULL;
    }
    pid = fork();
    switch (pid){
    case -1:
        ...
        break;
    case 0:  /* child */
        /* when exec dtv_svc, open console */
        if (((args[2] != NULL) && (strstr(args[2], "dtv_svc") != NULL)) || ((args[2] != NULL) && (strstr(args[2], "mount_data") != NULL)))
        {
            int fd;
            if ((fd = open("/dev/ttyMT0", O_RDWR)) < 0) {
    ...
    default: /* parent */
        ...
        if ((args[2] != NULL) && (strstr(args[2], "dtv_svc") != NULL))
        {
            /* dtv_svc.sh is executed, let's go */
            break;
        }
		
		if ((args[2] != NULL) && (strstr(args[2], "mount_data") != NULL))
		{
		    if (WIFEXITED(status)) {
				...
				ERROR("child_return_val = %s\n", child_return_val);
			}
			break;
		}
		
        
        if ((args[1] != NULL) && (strstr(args[1], "mknod_util") != NULL))
        {
            /* mknod_util is executed, let's go */
            break;
        }
                        ...
                        switch (pid_super){
                            ...
                            default: /* parent */
                                ...
                                if (WIFEXITED(status)){
                                    child_return_val = WEXITSTATUS(status);
                                    if (child_return_val != 0 && child_return_val != 1){
                                        printf("we need reboot and run e2fsck again\n");
                                        //this may lead to loop reboot, for example partition super block bad
                                        //bud don't has group 1
                                        //android_reboot(ANDROID_RB_RESTART, 0, 0);
                                    }
                                }
                                break;
...
int file_exist_at(const char *path)
{
    int ret = 0;
...
int do_system(int nargs, char **args)
{
    char cmd[128] = {0};
    int n = 1, ret = 0;
    ...
    if(ret != 0)
    {
        ERROR("%s(%d): %s failed, ret %d, errno %d(%s)\n", __FUNCTION__, __LINE__, cmd, ret, errno, strerror(errno));
    return -1;
    }
    else
    {
...
int do_write_symlink(int nargs, char **args)
{
    const char *path = args[1];
    const char *value = args[2];
    char prop_val[PROP_VALUE_MAX];
    ...
    return write_symlink_file(path, prop_val);
}
