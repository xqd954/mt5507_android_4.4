...
typedef android::List<char *> ExclusiveMountName;
...
public:
    ExclusiveMountName    *mExclusiveMountStrs;
    virtual ~VolumeManager();

    int start();
    int stop();

    void handleWifiProduct(NetlinkEvent * evt);
    void handleWifiEvent(NetlinkEvent *evt);
    void handleBlockEvent(NetlinkEvent *evt);
#ifndef ANDROID_DEFAULT_CODE
    //2012/8/6,added by Daniel Hsiao,for USB Audio detection support
    void handleSoundEvent(NetlinkEvent *evt);
#endif
    ...
    void setDMBroadcaster(SocketListener *dmsl) {mDMBroadcaster = dmsl;}
    SocketListener *getDMBroadcaster() { return mDMBroadcaster; }
    static VolumeManager *Instance();

    static char *asecHash(const char *id, char *buffer, size_t len);

    void doExternalCommand(int argc, char** argv);
