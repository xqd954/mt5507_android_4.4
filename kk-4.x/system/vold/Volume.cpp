...
const char *Volume::stateToStr(int state) {
...
int Volume::handleWifiProduct(NetlinkEvent * evt) {
    errno = ENOSYS;
    return -1;
}

int Volume::handleWifiEvent(NetlinkEvent * evt) {
    errno = ENOSYS;
    return -1;
}
...
int  Volume::umountVolumeCmd(const char *label, bool force, bool revert) {
    errno = ENOSYS;
    return -1;   
}
...
int Volume::handleSdcardMountVol() {
    errno = ENOSYS;
    return -1;
}
...
int Volume::handleSoundEvent(NetlinkEvent * evt) {
    errno = ENOSYS;
    return -1;
}
#endif
...
void Volume::setDevNode(const char * devNode)
{
...
int Volume::formatVol(bool wipe) {
    ...
    if (formatEntireDevice) {
        sprintf(devicePath, "/dev/block/vold/%s",
                getDevNode());
...
int Volume::mountVol() {
    ...
    for (i = 0; i < n; i++) {
		...
		if (doMoveMount("/mnt/secure/staging", getMountpoint(), false)) {
            ...
            umount2("/mnt/secure/staging", MNT_FORCE | MNT_DETACH);
            setState(Volume::State_Idle);
            return -1;
        }
...
int Volume::mountVolumeByNtfsKo(const char* devpath, const char* mntpath, int mntflags)
{
    int    ret = 0;
    char   cmd[128] = {0};
...
int Volume::mountVolumeByTuxeraNtfsKO(const char* devpath, const char* mntpath, int mntflags, const char* mntdata)
{
    int    ret = 0;
    char   cmd[128] = {0};
    if(access(TUXERA_NTFS_MODULE_PATH, F_OK) != 0)
    {
        sprintf(cmd, "insmod %s", TUXERA_NTFS_KO_PATH);
        ret = system(cmd);
        if(ret == -1 || ret == 127)
		{
			SLOGE("%s fail\n", cmd);
    ...
    ret = mount(devpath, mntpath, "tntfs", mntflags, mntdata);  
    return ret;    
}
...
int Volume::doUnmount(const char *path, bool force) {
    ...
    while (retries--) {
        if (!umount2(path, MNT_FORCE | MNT_DETACH) || errno == EINVAL || errno == ENOENT) {
