...
VolumeManager::VolumeManager() {
    ...
    mExclusiveMountStrs = new ExclusiveMountName();
    mBroadcaster = NULL;
    mUmsSharingCount = 0;
    mSavedDirtyRatio = -1;
...
VolumeManager::~VolumeManager() {
    ...
    ExclusiveMountName::iterator it;
     for(it = mExclusiveMountStrs->begin(); it != mExclusiveMountStrs->end(); it++)
    {
        delete (*it);        
    }
    delete mExclusiveMountStrs;
}
...
void VolumeManager::handleWifiProduct(NetlinkEvent * evt) {
    ...
    SLOGI("ProductName: %s\n", productName);
    SLOGI("InterfacePara: %s\n", interfacePara);

    VolumeCollection::iterator it;
    bool hit = false;
    for (it = mVolumes->begin(); it != mVolumes->end(); ++it) {
        if (!(*it)->handleWifiProduct(evt)) {
#ifdef NETLINK_DEBUG
            SLOGD("ProductName: '%s' event handled by volume %s\n", productName, (*it)->getLabel());
#endif
            hit = true;
            break;
        }
    ...
    if (!hit) {
#ifdef NETLINK_DEBUG
        SLOGW("No volumes handled WIFI Product with ProductName: %s\n", productName);
#endif
    } 
}

void VolumeManager::handleWifiEvent(NetlinkEvent * evt) {  
    const char *wifiInterface = evt->findParam("INTERFACE");  
    if(wifiInterface == NULL)
    {
        return;
    }
    SLOGI("WIFI Dangle interface: %s\n", wifiInterface);
    /* Lookup a volume to handle this device */
    VolumeCollection::iterator it;
    bool hit = false;
    for (it = mVolumes->begin(); it != mVolumes->end(); ++it) {
        if (!(*it)->handleWifiEvent(evt)) {
#ifdef NETLINK_DEBUG
            SLOGD("WIFI Dangle interface: '%s' event handled by volume %s\n", wifiInterface, (*it)->getLabel());
#endif
            hit = true;
            break;
        }
    ...
    if (!hit) {
#ifdef NETLINK_DEBUG
        SLOGW("No volumes handled WIFI Dangle event for WIFI Dangle interface: %s\n", wifiInterface);
#endif
    }  
}
...
void VolumeManager::handleSoundEvent(NetlinkEvent *evt) {
    const char *devpath = evt->findParam("DEVPATH");
    ...
    bool hit = false;
    for (it = mVolumes->begin(); it != mVolumes->end(); ++it) {
        if (!(*it)->handleSoundEvent(evt)) {
#if 1 //def NETLINK_DEBUG
            SLOGD("handleSoundEvent Device '%s' event handled by volume %s\n", devpath, (*it)->getLabel());
#endif
            hit = true;
            break;
        }
    ...
    if (!hit) {
#if 1 //def NETLINK_DEBUG
        SLOGW("handleSoundEvent No volumes handled block event for '%s'", devpath);
#endif
    }
}
#endif
...
int VolumeManager::mountVolume(const char *label) {
    ...
    if (v->handleSdcardMountVol() != 0) {
        SLOGD("sdcard alread mounted\n");
        return v->mountVol(); //exist
    }else{
        return -1; //not exsit
    }  ;
}

void VolumeManager::doExternalCommand(int argc, char** argv)
{
    VolumeCollection::iterator it;
    //SLOGE("(3)doExternalCommand(%s)", strerror(errno));
    for (it = mVolumes->begin(); it != mVolumes->end(); ++it) 
    {
        ExternalVolume* ex = (ExternalVolume*)(*it);
        ex->doCommand(argc, argv);
    }
}
