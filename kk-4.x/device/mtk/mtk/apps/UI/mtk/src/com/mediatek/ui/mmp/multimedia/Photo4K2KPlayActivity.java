...
import com.mediatek.ui.mmp.util.EffectView;
import com.mediatek.ui.mmp.util.EffectView.ImagePlay;
...
public class Photo4K2KPlayActivity extends MediaPlayActivity {
	...
	private ImagePlay mPlay = new ImagePlay() {
	...
	protected void onPause() {
		if(isBackFlag){
			picjni.nativeExitFromPhotoPlay(false,mLogicManager.getPicIndex());
			is4K2KFlag = false;
		}
	...
	private void initShowPhoto() {
		...
		mLogicManager.initPhotoFor4K2K(display);
		if(0 !=picjni.nativeExitFromPhotoPlay( true ,mLogicManager.getPicIndex())){
...
// 			mLogicManager.setCapturer(vShowView);
	...
	private void loadImageDone(PhotoUtil bitmap) {
		...
		if (null == bitmap || bitmap.getmBitmaps() == null) {
			mCurBitmap = bitmap;
			return;
		}
